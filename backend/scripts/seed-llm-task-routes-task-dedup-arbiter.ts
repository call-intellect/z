/**
 * TZ task-dedup (2026-06-16) — Seed маршрута LLM для `task-dedup-arbiter`
 * (дедуп задачи ПЕРЕД записью в трекер, два уровня входа).
 *
 *   - task-dedup-arbiter — арбитр «та же задача или разные» с обязательным NIL.
 *     Вход: кандидат + KNN-похожие открытые задачи (человеческие заголовки).
 *     Выход: { verdict: nil|same|different, sameWithRef, confidence, rationale }.
 *     Только SUGGEST — авто-merge запрещён (R13). Дешёвый арбитр, JSON Schema
 *     strict. Маршрут должен существовать, иначе вызов поедет по аварийному
 *     DEFAULT_FALLBACK_CHAIN.
 *
 * Цепочка (НЕ anthropic — memory `project_z_infra_and_ai`):
 *   task-dedup-arbiter: deepseek/deepseek-v4-flash → openai-via-proxy/gpt-5.4-mini → ollama/qwen3.5:9b
 *
 * Запуск:
 *   bun run scripts/seed-llm-task-routes-task-dedup-arbiter.ts
 *   bun run scripts/seed-llm-task-routes-task-dedup-arbiter.ts --update-existing
 *
...
async function main(): Promise<void> {
  ...
  console.log(
    `=== seed-llm-task-routes-task-dedup-arbiter START (updateExisting=${updateExisting}) ===`,
  );
  ...
  console.log('=== seed-llm-task-routes-task-dedup-arbiter DONE ===');
}

main()
  .catch((err) => {
     
    console.error('seed-llm-task-routes-task-dedup-arbiter FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
