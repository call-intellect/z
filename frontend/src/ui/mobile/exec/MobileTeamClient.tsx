...
/**
 * MobileTeamClient — мобильный экран «Команда» руководителя (ТЗ B2/Ф3
 ...
 * `GET /dashboard/operations/overview` (тот же, что десктопная COO-панель),
 * через тот же маппер `fromOperationsOverviewApi` → `OperationsOverviewDomain`.
 * Десктоп НЕ меняем.
 *
 * Раскладка (один столбец, glance):
 *   - «Настроение команды» — светофор (доля зелёных чек-инов);
 *   - «Кому помочь» (вверху) — блокеры/трения, поданные нейтрально-заботливо;
 *   - сводные числа (блокеры/трения/перегруз).
 *
 * Р4 (КРИТИЧНО): фокус «кому помочь», БЕЗ публичного «кто провалил». Чистая
 * логика рядов/тона вынесена в `team-rows.ts` (тестируется юнитом). Финансы
 * НЕ показываем.
 */
...
export function MobileTeamClient() {
  ...
  return (
    <div className="mx-auto w-full max-w-md px-4 py-5">
      ...
      <p className="mb-4 text-sm text-fg-secondary">Настроение и кому помочь — за неделю.</p>
      ...
      {!loading && !error && empty && (
        ...
        >
          <HeartHandshake size={24} className="mx-auto mb-2 text-accent" aria-hidden />
          <p className="text-base font-medium text-fg-primary">Пока тихо</p>
          <p className="mt-1 text-sm text-fg-secondary">
            Появятся чек-ины и встречи — здесь будет настроение команды и кому помочь.
          </p>
        </div>
      )}
