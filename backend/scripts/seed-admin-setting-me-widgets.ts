...
const SEEDS: SettingSeed[] = [
  {
    key: 'me.daily_value_widgets.enabled',
    value: true,
    category: 'me',
    section: 'daily_value',
    severity: 'medium',
    description:
      'Kill-switch (ON): 4 виджета ежедневной ценности в /me + self-эндпоинты /me/ideas (судьба моих идей) и /me/recognitions (полученные признания). OFF → эндпоинты отдают пустой ответ (graceful). Только self-данные пользователя, чужого не светим.',
  },
];
...
async function main(): Promise<void> {
  console.log('=== seed-admin-setting-me-widgets START ===');
  ...
  console.log('=== seed-admin-setting-me-widgets DONE ===');
}

main()
  .catch((err) => {
    console.error('seed-admin-setting-me-widgets FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
