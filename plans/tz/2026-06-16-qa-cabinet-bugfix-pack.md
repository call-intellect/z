...
## Статус (реализовано 2026-06-16, ветка fix/qa-cabinet-bugfix-2026-06-16)
- [x] **Фаза 1** — idea_blocks→"IdeaBlock"/tags (фикс ERROR 42P01); chat-v2 X-Org-Id; valueStrip уже исправлен (`a813d1df`)
- [x] **Фаза 2** — Markdown+GFM в боковой панели и /result; never_activated корректный текст
- [x] **Фаза 3** — consistency-checker УЖЕ русский (правка не нужна); фронт скрывает технический context; intake confidence-фильтр; probe дедуп. ⚠ curation chydo_002 — частично (personName для knowledge_profile; источник логина для card-rollup в `card-rollup-v2.service.ts` — вынесено)
- [x] **Фаза 4** — tier_standard→«Стандартный»; убран мёртвый лимит meetings_per_month
- [x] **Фаза 5** — бренд Z→Кора (фронт+сиды+backfill); англицизмы; склонения (pluralRu)
- [x] **Фаза 6** — спринты CTA; дублирующая навигация настроек убрана; диалоги auto-title refetch
- [ ] **Ретест** — ПОСЛЕ prod-выката (кабинет на проде, правки локальны)

### Вне код-фиксов (как планировалось): «( vs )» помощника + метрики встречи (LLM/данные); дубли данных в БД конкретной Org; искажение бренда в графе (ASR). Источник логина chydo_002 (card-rollup-v2) — отдельная под-задача.
