...
import type { MonthlyDigestGoalAlignmentMonthApi } from "@/api/monthly-digest.api";
...
import { MonthGoalPace } from "./MonthGoalPace";

type Direction = MonthlyDigestGoalAlignmentMonthApi["direction"];
...
const DIRECTION_VERDICT: Record<Direction, string> = {
  to_goal: "К цели — за месяц усилия приближали к главной цели",
  drift: "Дрейф — движение есть, но к цели двигала малая часть",
  against: "Против — месяц увёл от цели",
};
...
export function MonthGoalCompass({
  goal,
}: {
  goal: MonthlyDigestGoalAlignmentMonthApi;
}) {
              ...
              <ul className="flex flex-col gap-2">
                {goal.pro.length > 0 ? (
                  ...
                  <li className="text-[13px]" style={{ color: CHART.faint }}>
                    За месяц ничего не приблизило к цели.
                  </li>
                )}
              </ul>
            </div>
