...
function makeService(prisma: PrismaService): IntakeService {
  const cfg = { tracker: { chatboxTasksInTriageEnabled: false } };
  return new IntakeService(prisma, {} as never, {} as never, {} as never, cfg as never);
}
...
describe('IntakeService.findAll — read-union chatbox-Task (Блок B / F10)', () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  function makeChatboxTask(overrides: Partial<ChatboxTaskRow>): ChatboxTaskRow {
  ...
  function buildUnionPrisma(opts: {
    intakes: IntakeRow[];
    tasks: ChatboxTaskRow[];
    issues?: { title: string }[];
    persons?: { userId: string | null; name: string }[];
  }): PrismaService {
    return {
  ...
  function makeUnionService(prisma: PrismaService, flag: boolean): IntakeService {
    const cfg = { tracker: { chatboxTasksInTriageEnabled: flag } };
    return new IntakeService(prisma, {} as never, {} as never, {} as never, cfg as never);
  }

  const DEFAULT_QUERY = { page: 1, limit: 50 } as never;

  it('флаг on: открытая chatbox-Task попадает в items (синтетический id + source chatbox)', async () => {
    const intake = makeIntake({ id: 'i-1', status: 'pending', extractedTitle: 'Из встречи' });
    const task = makeChatboxTask({ id: 't-1', title: 'Из чата' });
    const prisma = buildUnionPrisma({ intakes: [intake], tasks: [task] });
    const svc = makeUnionService(prisma, true);

    const res = await svc.findAll(TENANT, DEFAULT_QUERY, 'user-x');

    const chat = res.items.find((i) => i.id === 'chatbox-task:t-1');
    expect(chat).toBeDefined();
    expect(chat!.source).toBe('chatbox');
    expect(chat!.extractedTitle).toBe('Из чата');
    expect(chat!.status).toBe('pending');
    expect(res.total).toBe(2);
  });

  it('форма адаптера совпадает с обычным элементом (одинаковые ключи)', async () => {
    const intake = makeIntake({ id: 'i-1', status: 'pending', extractedTitle: 'Из встречи' });
    const task = makeChatboxTask({ id: 't-1', title: 'Из чата' });
    const prisma = buildUnionPrisma({ intakes: [intake], tasks: [task] });
    const svc = makeUnionService(prisma, true);
    ...
    const res = await svc.findAll(TENANT, DEFAULT_QUERY, 'user-x');
    const intakeItem = res.items.find((i) => i.id === 'i-1')!;
    const chatItem = res.items.find((i) => i.id === 'chatbox-task:t-1')!;

    expect(Object.keys(chatItem).sort()).toEqual(Object.keys(intakeItem).sort());
  });

  it('флаг off → только IntakeIssue, без chatbox (task.findMany не зван)', async () => {
    const intake = makeIntake({ id: 'i-1', status: 'pending' });
    const task = makeChatboxTask({ id: 't-1' });
    const prisma = buildUnionPrisma({ intakes: [intake], tasks: [task] });
    const svc = makeUnionService(prisma, false);

    const res = await svc.findAll(TENANT, DEFAULT_QUERY, 'user-x');

    expect(res.items.map((i) => i.id)).toEqual(['i-1']);
    expect(prisma.task.findMany as ReturnType<typeof vi.fn>).not.toHaveBeenCalled();
  });

  it('кросс-дедуп: chatbox-Task с title как у открытого IntakeIssue → не дублируется', async () => {
    const intake = makeIntake({
      id: 'i-1',
      status: 'pending',
      extractedTitle: '  Согласовать Бюджёт ',
    });
    const task = makeChatboxTask({ id: 't-dup', title: 'согласовать бюджет' });
    const prisma = buildUnionPrisma({ intakes: [intake], tasks: [task] });
    const svc = makeUnionService(prisma, true);
  ...
  it('кросс-дедуп: chatbox-Task с title как у открытого Issue → не дублируется', async () => {
    const intake = makeIntake({ id: 'i-1', status: 'pending', extractedTitle: 'Из встречи' });
    const task = makeChatboxTask({ id: 't-dup', title: 'Закрыть спринт' });
    const prisma = buildUnionPrisma({
      intakes: [intake],
      tasks: [task],
      issues: [{ title: 'Закрыть спринт' }],
    });
    const svc = makeUnionService(prisma, true);
  ...
  it('пагинация в памяти: сортировка createdAt desc + slice', async () => {
    const older = makeIntake({
      id: 'i-old',
      status: 'pending',
      createdAt: new Date('2026-06-10T00:00:00.000Z'),
    });
    const newerTask = makeChatboxTask({
      id: 't-new',
      title: 'Свежая задача из чата',
      createdAt: new Date('2026-06-25T00:00:00.000Z'),
    });
    const prisma = buildUnionPrisma({ intakes: [older], tasks: [newerTask] });
    const svc = makeUnionService(prisma, true);

    const page1 = await svc.findAll(TENANT, { page: 1, limit: 1 } as never, 'user-x');
    expect(page1.items.map((i) => i.id)).toEqual(['chatbox-task:t-new']);
    expect(page1.total).toBe(2);

    const page2 = await svc.findAll(TENANT, { page: 2, limit: 1 } as never, 'user-x');
    expect(page2.items.map((i) => i.id)).toEqual(['i-old']);
  });

  it('резолв имени исполнителя chatbox-Task из карты person', async () => {
    const task = makeChatboxTask({ id: 't-1', assigneeUserId: 'user-1' });
    const prisma = buildUnionPrisma({
      intakes: [],
      tasks: [task],
      persons: [{ userId: 'user-1', name: 'Иван Петров' }],
    ...
    const res = await svc.findAll(TENANT, DEFAULT_QUERY, 'user-x');
    const chat = res.items.find((i) => i.id === 'chatbox-task:t-1')!;
    expect(chat.suggestedAssigneeId).toBe('user-1');
    expect(chat.suggestedAssigneeName).toBe('Иван Петров');
  });
});

describe('IntakeService.triage — промоут chatbox-Task (Блок B / F10)', () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  it('accept chatbox-task → Issue создан из Task + task.status=done', async () => {
    ...
    expect(issuesCreate).toHaveBeenCalledTimes(1);
    const [projectId, dto] = issuesCreate.mock.calls[0]! as [
      string,
      { title: string; assigneeUserIds: string[] },
    ];
  ...
  it('reject chatbox-task → Issue НЕ создан, task.status=done', async () => {
    ...
    const res = await svc.triage('chatbox-task:t-1', REJECT, TENANT, 'user-1');
    ...
    expect(res.intake.status).toBe('rejected');
    expect(res.createdIssue).toBeNull();
  });

  it('chatbox-task не найдена → NotFoundException', async () => {
    const { svc } = build({ task: null });
    await expect(svc.triage('chatbox-task:missing', ACCEPT, TENANT, 'user-1')).rejects.toThrow(
      NotFoundException,
    );
  });
