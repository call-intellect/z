...
> ТЗ [`plans/tz/2026-06-21-smart-tables-revive-and-autofill.md`](../../plans/tz/2026-06-21-smart-tables-revive-and-autofill.md) (Ф4). Строки 4 системных таблиц (Идеи/Обещания/Цели/Гипотезы) авто-создаются из графовых объектов (IdeaBlock signalType `idea`/`commitment`, Goal, Experiment) через `TableGraphSyncService` + reconcile-крон. Связка таблиц с чатом «Мастер» уже была — рычаг = наличие строк.
>
> **🟢 1 МИГРАЦИЯ PRISMA (аддитивная).** **🟢 НОВЫХ ОБЯЗАТЕЛЬНЫХ ENV НЕТ.** **🟢 1 НОВЫЙ ФЛАГ** — `table.graphsync.enabled` (тип A — kill-switch, ON; реестр `feature-flags.md`). **🟢 1 НОВЫЙ СИД** (`seed-admin-setting-table-graphsync.ts`, 2 ключа, STEPS `phase:'seed-base'`). **🟢 1 НОВЫЙ BACKFILL** (`backfill-table-graphsync.ts`, STEPS `phase:'backfill'`, `skipBootstrap`). **🟢 1 НОВЫЙ CRON** (`table-graphsync-reconcile`, @Cron '25 */3 * * *'). Docker rebuild backend+frontend.

- **Шаг 1 — ENV: новых обязательных нет.** 2 крутилки `table.graphsync.{enabled,min_confidence}` читаются через `getDynamic` с code-fallback (Ship-On: admin→code-fallback `true`/`0.5`). `enabled` — kill-switch (тип A), действий владельца не требует. Реестр флагов — `docs/operations/feature-flags.md` (1 новая строка тип A).
...
- **Шаг 7 — Seed (НОВЫЙ, идемпотентный, STEPS `phase:'seed-base'`):** `scripts/seed-admin-setting-table-graphsync.ts` — 2 ключа `table.graphsync.enabled`=`true` + `table.graphsync.min_confidence`=`0.5` (category `tables`, section `agent`, severity `low`). Защита admin-edited (`updatedBy !== 'system'`); повтор = no-op.
- **Шаг 8 — Backfill (НОВЫЙ, идемпотентный, STEPS `phase:'backfill'`, `skipBootstrap`):** `scripts/backfill-table-graphsync.ts` — (а) проставляет `Table.graphSync` из каталога системным таблицам ideas/promises/okr/hypotheses; (б) `reconcileTenant` по всем Org: создаёт строки из живых Goal/Experiment/IdeaBlock (гейт confidence≥0.5, дедуп по sourceObjectId, fill-empty). Поддержка `--org=<id>`. Идемпотентен (второй прогон = 0 created).
- **Шаг 11 — Docker rebuild** — `docker compose up -d --build backend frontend`. Backend: `TableGraphSyncService` (tables/services) + `TableGraphsyncReconcileCronService` (tables/workers, в `ai/workers.module.ts` providers), `tables-auto-provision.service.ts` (копирует graphSync при провижининге), `system-tables.catalog.ts` (канал graphSync), реестр `admin-setting-schema-registry.ts` (+2 ключа). Frontend: `nav-config.ts` (пункт «Таблицы» в «Справочник»).
- **Шаг 12 — Smoke** (после выката): `docker compose exec backend bun run scripts/apply-prod-deploy.ts --mode update` прогоняет seed (created=2, потом no-op) + backfill (created>0 первый раз, потом 0); 2 крутилки видны в `/admin` (category `tables`); cron зарегистрирован: `docker compose exec backend sh -c 'grep -rn "table-graphsync-reconcile" /app/src/modules/ai/workers.module.ts'`; после backfill в таблицах Идеи/Обещания/Цели/Гипотезы появляются строки (`SELECT COUNT(*) FROM "TableRow" WHERE "sourceObjectType" IS NOT NULL`); пункт «Таблицы» виден в меню «Справочник».
