...
const SEEDS: SettingSeed[] = [
  {
    key: 'proactive.eveningPlanCheckEnabled',
    value: true,
    category: 'ai',
    section: 'proactive',
    severity: 'high',
    description:
      'Вечерняя сверка плана дня: пинг по незакрытым пунктам утреннего плана. Рубильник, по умолчанию вкл (Ship-On).',
  },
  {
    key: 'proactive.planItemOverdueThresholdDays',
    value: 1,
    category: 'ai',
    section: 'proactive',
    severity: 'low',
    description:
      'За сколько последних суток проверять незакрытые пункты плана (вечерняя сверка). По умолчанию 1.',
  },
];
...
async function main(): Promise<void> {
  console.log('=== seed-admin-setting-proactive START ===');
  ...
  console.log('=== seed-admin-setting-proactive DONE ===');
}

main()
  .catch((err) => {
    console.error('seed-admin-setting-proactive FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
