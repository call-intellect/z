...
export function useHostControls(meetingId: string) {
  ...
  return {
    pending,
    mute: (pid: string) =>
      wrap(`mute:${pid}`, () => meetingsApi.muteParticipant(meetingId, pid)),
    unmute: (pid: string) =>
      wrap(`unmute:${pid}`, () =>
        meetingsApi.unmuteParticipant(meetingId, pid),
      ),
    kick: (pid: string) =>
      wrap(`kick:${pid}`, () => meetingsApi.kickParticipant(meetingId, pid)),
