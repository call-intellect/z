...
Транскрипт-фрагмент: «[00:10] Сергей: Запускаем доработку оплаты к пятнице. [00:18] Аня: Я возьму тесты. [00:25] Сергей: Можно было бы потом сделать ещё экспорт в Excel, но это идея на будущее.». Спикеры покрывают примерно 60% реплик.
Вывод (фрагмент tasks + summary_markdown + data_quality): {"tasks": [{"title": "Запустить доработку оплаты", "assigneeRaw": "Сергей", "dueDateIso": null, "sourceQuote": "Запускаем доработку оплаты к пятнице", "confidence": 0.9}, {"title": "Провести тестирование", "assigneeRaw": "Аня", "dueDateIso": null, "sourceQuote": "Я возьму тесты", "confidence": 0.85}], "summary_markdown": "Договорились запустить доработку оплаты к пятнице; тестирование за Аней.", "data_quality": "Спикеры распознаны примерно для 60% реплик; часть имён не атрибутирована."}. Пояснение: «решили ≠ обсудили» — в summary только принятое; «идея ≠ задача» — экспорт в Excel озвучен как идея на будущее, поэтому в tasks НЕ попадает; срок «пятница» относительный, поэтому dueDateIso = null.

Что НЕ делать (edge case — бедный/обрезанный транскрипт и чужое имя):
Транскрипт-фрагмент: «[00:03] Участник: …поэтому давайте Олег займётся отчётом. [запись обрывается]». В переданном списке участников Олега НЕТ.
