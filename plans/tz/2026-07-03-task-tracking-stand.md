...
**Цель:** построить измерительный стенд `task-stand`, который через **реальный конвейер** льёт 110 синтетических
...
в банке), судит смысл панелью LLM и печатает **baseline** — распределение вердиктов по 5 механизмам (создание ·
подзадачи/дедуп · журнал · автозакрытие · **качество текста**) + статус инвариантов INV-1..4 + вердикт по
гипотезам Т1–Т11.
...
backend/scripts/task-stand/            (СТРОИТ агент)
  ...
  report.ts           Ф4/Ф5: scorecard по категориям+механизмам + оси + инварианты + Т1–Т11 + конфиг + было→стало
docs/testing/
...
journal/closure/text-quality); оси C/D/G/J/X/P/**T** (среднее); **статус INV-1..4** (любой FAIL — красный стоп); **таблица Т1–Т11**
...
**Acceptance 4:** судьи majority-of-3 работают; report печатает scorecard + оси + инварианты + Т1–Т11 с
атрибуцией; каждый провал сверяем с сырой трассой.

## 9. Фаза 5 — baseline-прогон + отчёт владельцу

Чистка кэшей/дедупов (§3) → `run` → `judge` → `report` на 110 + свежие 15. Зафиксировать конфигурацию.

**Отчёт владельцу** (`task-stand-report.md` + разбор): scorecard по механизмам; **вердикт по Т1–Т11** (что
...
**Acceptance ТЗ (общая):**
- [ ] `bun run scripts/task-stand/stand.ts all` гоняет 110 сценариев по-фазно и целиком;
- [ ] setup-задачи + манифест ground truth на месте; эмбеддинги готовы;
- [ ] замер Т1 снят (какой извлекатель жив/мёртв) — явно в отчёте;
- [ ] baseline-отчёт: scorecard по 5 механизмам + оси (вкл. T) + **статус INV-1..4** + **таблица Т1–Т11** + конфиг;
...
- **Недетерминизм LLM** ±3-5 п.п. на 110 — дельта такого размера не вывод; целевые сценарии чинятся поимённо.
...
**Исключение — Т11 (структурное описание) фикс УЖЕ оформлен owner-confirmed:**
[plans/tz/2026-07-04-task-text-reformulation.md](2026-07-04-task-text-reformulation.md) — реализуется вместе со
стендом, доказывается категорией K (было→стало); учесть регрессию дедупа (ось D, `buildText` использует описание).

Остальной ТЗ-фикс «Разворот трекинга» — **отдельный файл ПОСЛЕ baseline**, по подтверждённым Т1–Т11 (напр.: Т1 — добить
...
Реализовано: —/— (по фазам). Стенд меряет 5 механизмов трекинга на 110 синтетических входах через реальный
