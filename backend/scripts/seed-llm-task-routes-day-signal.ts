...
const TASK_TYPES = ['day-signal-detect'] as const;
...
async function main(): Promise<void> {
  ...
  console.log(
    `=== seed-llm-task-routes-day-signal START (taskTypes=${TASK_TYPES.join(',')}, updateExisting=${updateExisting}) ===`,
  );
  ...
  // eslint-disable-next-line no-console
  console.log('=== seed-llm-task-routes-day-signal DONE ===');
}

main()
  .catch((err) => {
    // eslint-disable-next-line no-console
    console.error('seed-llm-task-routes-day-signal FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
