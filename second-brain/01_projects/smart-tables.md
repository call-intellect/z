...
- **Новый канал `graphSync`** на `SystemTableTemplate` (`templates/system-tables.catalog.ts`) — параллельно `entitySync`; проставлен для `ideas`/`promises`/`okr`/`hypotheses`/**`risks`**.
- **`TableGraphSyncService`** (`services/table-graph-sync.service.ts`) заводит `TableRow` из графовых объектов: `Goal`→«Цели и метрики» (okr), `Experiment`→«Гипотезы и эксперименты» (hypotheses), `IdeaBlock signalType='idea'`→«Идеи и бэклог» (ideas), `IdeaBlock signalType='commitment'`→«Обещания и обязательства» (promises). **Ф4.5 — «Реестр рисков»:** `IdeaBlock` по НАБОРУ риск-сигналов (`signalTypes[]`: `risk`/`churn_risk`/`blocker`/`pain`/`resource_gap`/`process_friction`/`team_friction` → Описание←name) — детерминированно, без нового LLM-извлекателя (block-ingest уже классифицирует эти signalType). Гейт `table.graphsync.min_confidence` (0.5; `null`=доверяем), детерминированный дедуп по `(tableId, sourceObjectType, sourceObjectId)`, **fill-empty** (ручные правки не перетираются), `TableCellProvenance` на каждую ячейку. Методы `syncObject` (по объекту) + `reconcileTenant` (весь тенант, offset-пагинация).
- **Ф4.6 — draft-пул спорных:** объект `confidence < min` → строка `status='draft'` + `draftExpiresAt` (вне чата/фильтров); усиление (объект дорос до порога) промоутит draft→active (сброс expiry); не промоутнутый за `table.agent.draft_ttl_days` (14) истекает (soft-delete) на reconcile (`expireDrafts`). Чат/семантик-фильтр читают только `status='active'`. Демоции active→draft нет (данные из чата не пропадают).
...
### ⚠️ Замер recall (2026-07-04, «Стрела») — данные есть, лифта НЕТ (ДО фикса; СНЯТО замером ниже)

> **Историческая запись до-фикс состояния.** Числа ниже (14.3% vs 23.8%) сняты на СЛОМАННОМ коде выбора таблицы одиночным LLM-судьёй. После связного слоя + гейтед-извлечения стенд-A/B (детерминированный судья, 3 повтора) дал лифт — см. §«Связный слой» → «Стенд-A/B recall доведён». Оставлено для истории диагноза.

Живьём на «Стреле» graphSync наполнил **99 строк** (Идеи 17 / Обещания 43 / Цели 1 / Гипотезы 11 / Риски 27; 1 черновик Ф4.6). Но **A/B-замер** (21 структурный вопрос банка: risks 8 + blockers 7 + goals 6; graph-only vs с таблицами; LLM-судья, `scripts/_measure-tables-recall.ts`) показал: **лифта нет — CORRECT 3/21 (14.3%) с таблицами против 5/21 (23.8%) graph-only** (в пределах шума LLM, но не выше).

**Корень (проба `fetchTableContext`):** предсуществующий баг выбора таблицы в чате `ChatV2TableContextService.fetchByKeywordTables` — (1) generic-имена колонок перехватывают матч (колонка «Что» у «Обещаний» цепляет вопрос «Что горит?» → в контекст льётся 20 нерелевантных промисов); (2) нет морфологии («риски»≠«рисков», «блокирует»≠«блокер») → «Реестр рисков» не выбирается никогда; (3) залив 20 строк без row-релевантности. Данные-рычаг рабочий, но **доставка в ответ сломана** — мои данные это вскрыли (раньше таблицы были пусты).

**Фикс — follow-up ТЗ** [`2026-07-04-chat-table-context-relevance-selection`](../../plans/tz/2026-07-04-chat-table-context-relevance-selection.md) (draft, owner-gate: трогает chat-retrieval, блэк-радиус на весь банк 111). **Прод-гейт:** не гонять `backfill-table-graphsync.ts` на проде до фикса (наполнение активирует деградирующую чат-ветку). Строка в [[../04_не-сделано/README]].

## Связный слой (строка↔граф) + гейтед извлечение (2026-07-04, dev)

Восстановление исходного замысла A5 «каждая строка — узел графа» + починка доставки в ответ. Плоский graphSync (строка несла только `name`, `entityId=null`, `sourceLink=null`) заменён связным. Анализ: [`plans/analysis/2026-07-04-smart-tables-relational-restore.md`](../../plans/analysis/2026-07-04-smart-tables-relational-restore.md); ТЗ: [`plans/tz/2026-07-04-smart-tables-relational-and-gated-retrieval.md`](../../plans/tz/2026-07-04-smart-tables-relational-and-gated-retrieval.md). Обе цели владельца одним корнем: (1) связность/идентичность как у конкурентов, (2) улучшать ответ — но только релевантным.

- **Ф1 — гейтед извлечение** (`chat-v2-table-context.service.ts` + `chat-v2.service.ts`, БД не трогает): fail-closed gating табличной ветки (подаём таблицы ТОЛЬКО при структурном интенте `queryClass ∈ {list,overview,temporal}`/aggregation ИЛИ точной сущности; иначе не подаём — kill-switch `chat_v2.table_context_enabled`); скоринг таблицы по `name`+`description` БЕЗ generic-имён колонок + стоп-слов (устраняет перехват колонкой «Что»); морфология (prefix-stem ≥4); **дедуп по source-block** (строку, чей IdeaBlock уже в граф-контексте, не подаём второй раз — убирает двойной счёт); row-level релевантность + cap-per-(entity,table)=3 + кап `max_rows`=8. Топиковые описания 10 системных таблиц с синонимами (`system-tables.catalog.ts` `description`).
- **Ф2 — `entityId`-мост** (`table-graph-sync.service.ts`): детерминированный `pickPrimaryEntity` (type-приоритет из `graphSync.preferredEntityTypes` ВЫШЕ роли subject/object/mentioned; тай-брейк mentionsCount → `entity.id` ASC для детерминизма; мёрженные исключены). `idea_block` → первичная `IdeaBlockEntity` (commitment-fallback на автора/получателя Person→entity); `goal` → `Goal.entityId`; `experiment` → `ownerEntityId`/personSubjectIds. `entityId` проставляется на create и добивается на существующих (`fillExisting`, только null→value, ручной перелинк не перетирается); fill-empty уважает человеческий провенанс (`appliedBy!='agent'`/`rolledBackAt`). `entityId` — атрибут-мост для чтения, НЕ ключ дедупа (дедуп остаётся по `sourceObjectType,sourceObjectId`). Зажигает `fetchByEntityBridge` (dialog уже резолвит `structuralFilters.entityIds`).
- **Ф3 — провенанс источника**: `sourceLink` deep-link на исходную встречу (`IdeaBlockEvidence→RawEvent{sourceType=meeting}`, `buildProvenanceDeepLink`) + метка «Встреча …»; для «Обещаний» — видимая ячейка «Источник» (`fieldMap 'Источник':'sourceLabel'`).
- **Схема БД не менялась** (`TableRow.entityId String?` уже есть+индексирован). Крутилки: `chat_v2.table_context_{enabled,max_rows=8,max_tables=2,max_rows_per_entity_table=3,structural_query_classes,generic_stopwords}` (реестр + сид `seed-admin-setting-chat-v2-tables.ts`). Backfill descriptions+entityId — `backfill-table-graphsync.ts` (config+descriptions+reconcile).

**Валидация на «Стреле» (dev, 2026-07-04):** проба выбора таблиц `_probe-table-context.ts` — **17/21 правильные таблицы** (было 4/21, почти все — не той таблицы); «Обещания» больше НЕ перехватывает риск/blocker-вопросы (blockers→«Реестр рисков», goals→«Цели и метрики»); вагые/фактовые вопросы → пусто (fail-closed). `entityId` покрытие: **risks 27/27, ideas 17/17, promises 43/43 = 100%** (hypotheses/okr null — у Goal/Experiment нет entity в графе, валидный null). typecheck+lint+build+168 тестов зелёные.

**Стенд-A/B recall доведён (2026-07-04, «Стрела») — лифт ПОДТВЕРЖДЁН, прод-гейт СНЯТ:** прогон `backend/scripts/_measure-tables-ab.ts` — gold-50, детерминированный mustMention-судья (не шумный одиночный LLM-судья старого замера), **3 повтора × 2 плеча = 252 ответа**, таблицы вкл/выкл через `archivedAt`, redis-кэш очищается между плечами. Итог: **всего OFF 82.5% → ON 85.7% (+3.2 п.п.)**; **структурный subset (risks+blockers+goals) 88.9% → 100% (+11.1 п.п.)**, risks 67→100%; **guard (expertise/fact/broad/decision/synonym) 77.4% → 78.6% — не просел**; галлюцинации honest_empty 0/0. Флипы: ↑ q088 «что горит» 0→100%, ↑ q057 встречи 67→100%, ↑ q029 CRM→Битрикс 0→33%, ↓ q014 «метрика квартала» 33→0% (единственный вниз, внутри ±шума LLM — флейкает и без таблиц; занесён в [[../04_не-сделано/README]]). **Старый вред (−9.5 п.п.: 23.8%→14.3% на сломанном коде) устранён, получен лифт ровно на целевом subset.** Рефлексия: [`05_история/2026-07-04-smart-tables-stand-ab`](../05_история/2026-07-04-smart-tables-stand-ab.md). **Остаток:** relation/rollup/views/мульти-person — отдельная волна (не соревнуемся в ширине конструктора; см. анализ §4); прод-выкат (backfill+seed) — действие владельца, код готов ВКЛ.
...
> **🔴 Живой прод-аудит 2026-06-20** ([plans/analysis/2026-06-20-smart-tables-live-audit/](../../plans/analysis/2026-06-20-smart-tables-live-audit/README.md)): на проде таблицы операционно мертвы — (P0) деталки крашится React #185 (фикс `26219233` НЕ выкачен, прод на старом билде без R1–R4) + нет пункта меню «Таблицы» в `nav-config.ts`; (P2) авто-наполнение охватывает 4/10 шаблонов и даже у них все прод-строки ручные (`entityId:null`, regulations=0); (P1) промпты 12b лучше живых (A/B 7/8 vs 5/8) но не выкачены, латентность агента 41–47с. Агенты при этом работают и качество хорошее. Дизайн моста граф→6 «мягких» таблиц с гейтом уверенности — в анализе.
...
- **Поток Privacy — research готов, но ⛔ ОТЛОЖЕН владельцем (2026-06-02).** На этом этапе конфиденциальность/комплаенс **не в приоритете**: отправка данных во внешний LLM (DeepSeek и т.п.) — ОК, не блокер. Доку держим как задел на будущий GTM; privacy-доработки сейчас НЕ делаем. См. [`plans/analysis/2026-06-02-smart-tables-privacy-positioning.md`](../../plans/analysis/2026-06-02-smart-tables-privacy-positioning.md) (там же находка R1 на будущее: table-задачи шлют `dataClass:'internal'` → данные идут во внешний LLM; механизм Ollama не задействован — править, когда вернёмся к privacy). все table-задачи передают `dataClass:'internal'` → транскрипты встреч (Фаза 3) и содержимое импортируемых файлов (Фаза 2) штатно уходят во внешний **DeepSeek (Китай)**, fallback — OpenAI/США; механизм локальной маршрутизации (Ollama, `localOnly`) для table-задач **не задействован**. Это стоп-фактор для крупного B2B/госсектора (ФСТЭК-117). Рекомендации в доке: признак «конфиденциальная таблица/встреча» + повышение `dataClass` → переиспользовать готовый фильтр роутера; RF-резидентный/in-perimeter inference; уведомления РКН; фиксация ToS провайдеров. Дифференциатор vs Notion/Coda/Airtable/Tana/MS Fabric: ни у кого нет on-prem AI-инференса и RF residency.
