...
**Встреча→трекер + модели (2026-06-22, ТЗ `meeting-to-tracker-and-models-unified-fix`):** 8 новых ключей в `admin-setting-schema-registry.ts`, засеиваются расширенными существующими сидерами (все в STEPS `phase:'seed-base'`): `seed-admin-setting-tracker.ts` (`tracker.meetingTasksAlwaysPromote` kill-switch ON — задача со встречи промоутится в Issue всегда), `seed-admin-setting-chatbox.ts` (`chatbox.taskExtraction.enabled` kill-switch ON), `seed-admin-setting-retention-logging.ts` (`ai.usageLog.previewMaxBytes` 32768), `seed-admin-setting-goals-pulse.ts` (`goals.minExtractConfidence` 0.4 / `goals.autoPromoteConfidence` 0.8 / `goals.maxActiveGoalsPerHorizon` 7), `seed-admin-setting-worker-knobs.ts` (`recording.trackWatchdogEnabled` kill-switch ON / `recording.trackWatchdogTimeoutMinutes` 20). Полные значения и «что делает / выкл→что» — единый реестр [[../../docs/operations/feature-flags]]. Модели — не AdminSetting (через `LlmTaskRoute`/сиды, см. [[ai-jobs]] §«Извлекающие taskType → Pro»).
