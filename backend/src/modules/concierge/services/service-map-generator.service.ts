  ...
  private buildStaticToolMap(): ToolSchema[] {
    return [
      ...
      },
      {
        name: 'assign_task',
        description:
          'Используй для постановки задачи ДРУГОМУ сотруднику (не себе). Когда просят «поставь задачу на <имя>», «поручи <имя>…». Передай assigneeName, как назвал пользователь. Если исполнитель неясен или это отдел/роль — всё равно вызови: Кора создаст задачу и сама уточнит исполнителя и срок. Переспрашивай ДО вызова только если неясна сама СУТЬ задачи.',
        method: 'POST',
