...
## Ф1 — Время по Москве + порядок ночи (O7+O10) `[x]`
...
## Ф2 — Вектор людей: отвязать от обещаний (O6) `[x]`
...
## Ф3 — Ручные цели ведутся как AI (O2) `[x]`
...
## Ф4 — Суточная пересборка иерархии (O3) `[x]`
...
## Ф5 — Каскад статуса по дереву (O4) `[x]`
...
## Ф6 — Ужесточить промпт извлечения целей (O1) `[x]`
...
## Ф7 — Чистка осиротевшего задачного UI + деталь карточки (O5) `[x]`
...
## Ф8 — Один вердикт статуса в UI (O8) `[x]`
...
## Ф9 — Крутилки целей в AdminSetting (O9) `[x]`
...
## Итог
Реализовано: ✅ целиком (Ф1–Ф9, коммиты a3105ad2/847182f6/0ca21279/7fb361e7/0ab99174/a15d73b8/acf94a56/12cf5b6f/35703d4f). Схема БД не менялась, миграций нет. Остаток/отложенное — в `04_не-сделано` (Issue↔Goal реальная связь, cron-расписания→AdminSetting, немедленный producer тем при создании цели, delete-vs-fail различение в каскаде).
