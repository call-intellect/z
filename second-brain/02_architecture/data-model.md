...
## TaskSolution — «Решение задачи» (2026-07-07)

**Модель `TaskSolution`** (таблица `task_solutions`, миграция `20260707120000_add_task_solution`, ТЗ [`task-solution-entity`](../../plans/tz/2026-07-07-task-solution-entity.md)) — материализует **опыт решения конкретной задачи** отдельной просматриваемой сущностью (раньше «как решали» растворялось в клоне). Единичный кейс, а не повторяемая норма — в отличие от `Regulation`/`Instruction`. **Двойное назначение (Q2):** те же блоки `reasoning`/`methodology_step` продолжают кормить клон (SkillProfile), `TaskSolution` — дополнительный потребитель тех же блоков, не замена. Версионирование / dataClass / embedding зеркалят `Regulation`/`Instruction`.

Поля: `id`, `tenantId`, `title @db.VarChar(300)` (название задачи), `taskDescription @db.Text`, `solutionMd @db.Text` (как решалась, собран компилятором), `ownerPersonId` (**обязателен** — исполнитель-владелец, ось A4: решавший, не упомянувший), `personSubjectIds[]` (обычно = [owner]), `sourceIssueId` (задача трекера), `sourceBlockIds[]` (блоки-источники: опрос + дневные), `skillTags[]`, `status ProcessStatus @default(active)`, `version`, `currentVersionId? → CardVersion`, `confidence?`, `promotedToInstructionId?` (если перерос в инструкцию), `repeatGroupKey? @db.VarChar(120)` (ключ группы похожих решений), `candidateInstruction Boolean` (повтор способа ×N → кандидат в Инструкцию — флаг, БЕЗ авто-создания), `previewQuote? @db.Text` + `previewSourceRef Json?` (денорм-снимок провенанса, как у `Regulation`/`Issue`), `dataClass` / `dataClassAudit Json?` / `ownerAudit Json?` (снимок расхождения assignee↔решатель при выводе владельца из текста — B2, миграция `20260708120000_add_task_solution_owner_audit`; заполняется ТОЛЬКО когда владелец ВЫВЕДЕН и не совпал с assignee), `embedding` (в schema декларирован `Unsupported("vector(768)")` — как у `Regulation`/`Instruction`, физически **1536**; из `title+solution`, для повтора/поиска), `lastConfirmedAt?`, `createdAt` / `updatedAt` / `deletedAt?` / `deletedById?`.

`@@unique([tenantId, sourceIssueId])` — **одна задача → одно решение** (суточная сборка апдейтит, не плодит на реплики). Индексы: `[tenantId, status]`, `[tenantId, ownerPersonId]`, `[tenantId, repeatGroupKey]`. FK: `Org` (`onDelete: Cascade`) / `Person` (`onDelete: Restrict`, relation `TaskSolutionOwner`) / `Issue` (`onDelete: Cascade`, relation `TaskSolutionIssue`) / `CardVersion` (`onDelete: SetNull`). HNSW-индекс `task_solutions_embedding_hnsw_cosine_idx` (cosine, `WHERE embedding IS NOT NULL`) — в `postgres-init.sql`, не в schema. **`CardVersion.resourceType` += `'task_solution'`** (история версий, как у регламентов).

Сборка — суточный крон `TaskSolutionBuildCron` + `TaskSolutionBuildService` (knowledge-core), см. [[module-map]], [[../01_projects/ai-jobs]], [[../01_projects/workers-queues]]. Витрина — модуль `task-solutions` + страница `/task-solutions` (см. [[../01_projects/api-layer]], [[../01_projects/frontend-pages]]). Авто-создание Инструкции из повтора решений НЕ делается (только `candidateInstruction` + `repeatGroupKey`, решение владельца Q3) — [[../04_не-сделано/README]].
...
> **`DailyCheckIn` += 4 сущности отчёта (дневной план/отчёт из графа, 2026-06-29).** Расширение под сборку плана/отчёта тонким сборщиком из разметки `block-ingest` (4 части: сделано/не сделано/помешало/идеи), а не отдельным LLM-свипом `day-signal-*` (снесён). Новые поля: `notDoneJson Json?` — `Array<{text, sourcePlanText?, verdictConfidence?}>` («не сделано»: либо незакрытый пункт плана через `ClosureVerifierService`, либо явный негатив «не дал» строкой-ожиданием); `ideasJson Json?` — `Array<{text, sourceBlockId?}>` (идеи дня); `reportCompleteness String? @db.VarChar(8)` — `draft`/`full`, **выводится в `toDto` на чтении** (колонка зарезервирована под будущих прямых SQL-читателей, сейчас не пишется — см. [[../04_не-сделано/README]]). Полнота НЕ влияет на «сдал/не сдал». Миграция `20260630010000_daily_checkin_report_4_entities` (аддитивная nullable). Источник — graph-сборщик `DayReportCollectorService.collectForDay` (атрибуция по `evidence.authorPersonId` + `sourceTimestamp`). ТЗ [`plans/tz/2026-06-29-checkin-day-report-from-graph.md`](../../plans/tz/2026-06-29-checkin-day-report-from-graph.md); cron/листенер — [[../01_projects/workers-queues]].
...
- **`ChatboxChat`** — чат (тред), зеркало 1:1 с ChatBox-«чатом»: денорм `channelType`/`customerExternalId`/`responsibleExternalId`, `status` (enum active/closed), `isGroup Boolean @default(false)` (производный флаг группового чата — `true`, если в чате >1 различного CLIENT-отправителя; проставляется синком, миграция `20260624160000_chatbox_chat_is_group`), `title String?` (имя диалога из `raw.client.name` — название группы / имя человека; миграция `20260624170000_chatbox_chat_title` + бэкофилл из raw; проставляется `upsertChat`), `externalCreatedAt/UpdatedAt`, `lastMessageAt`, `messageCount`. Relations `sessions`/`messages`. Индексы по `customerExternalId`/`lastMessageAt`/`status`. **Важно: ChatBox-«чат» ≈ наша сессия, не диалог** — ChatBox плодит отдельную chat-запись на каждую беседу с тем же `(customer+channel)`. Поэтому read-слой (`ChatboxChatsService.listChats`) **группирует** записи в один диалог по `(channelExternalId, COALESCE(customerExternalId, clientExternalId, externalId))` через `$queryRaw`; `getChat`/`listMessages` склеивают сессии/сообщения всех sibling-чатов. Группировка — только на чтении, данные/сессии не меняются.
