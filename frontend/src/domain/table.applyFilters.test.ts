...
const rows: Row[] = [
  {
    ...
    cells: {
      ...
      lastContact: "2026-05-30T10:00:00.000Z",
    },
  },
  {
    id: "r2",
    cells: {
      ...
      status: "Завершён",
      tags: ["малый"],
      lastContact: "2026-04-01T10:00:00.000Z",
    },
  },
  {
    id: "r3",
...
describe("applyFilters", () => {
  ...
  it("ФИКС 4: gt по ячейке-строке ru-RU («1 234,56») парсится в число", () => {
    const ruRows: Row[] = [
      { id: "a", cells: { amount: "1 234,56" } },
      { id: "b", cells: { amount: "999,00" } },
    ];
  ...
  it("ФИКС 7: eq/neq на пустой ячейке не проходят (ни true, ни false)", () => {
    expect(
      run({ propertyId: "status", op: "eq", value: "Активен" }),
    ).not.toContain("r3");
    expect(
