/**
 * Unit-тесты блокирующей валидации резолюции конфликта (SBA, Фаза C3).
 *
 * Проверяем чистую функцию `isResolveBlocked`:
 *   - `evolving` без обеих дат → blocked (подсказка);
 *   - `evolving` с обеими датами → ok;
 *   - `accept_new` / `keep_old` / `merge` → никогда не blocked.
 */
