...
> **⚠️ Статус: построен, но НЕ активен (dormant).** Гейт провенанса написан, но фактически спит: `resolveProbeProvenance(...)` в `backend/src/modules/probe/probe-reason-policy.ts:55` — заглушка, всегда `return 'unknown'`. Поэтому условие `provenance === 'auto_unconfirmed'` (`probe.service.ts:97`) **никогда** не выполняется: probe не подавляется, `dropped_policy_silent` в проде не эмитится, тихого статуса «Требует внимания» этот путь не порождает. Ветка проверена только тестами (`probe.service.spec.ts`, `probe-reason-policy.spec.ts`). Активируется реальной реализацией провенанса (различить авто-извлечённую-неподтверждённую запись от подтверждённой/ручной) — до этого гейт бездействует. См. [[../02_architecture/code-pitfalls]].
