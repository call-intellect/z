...
> **Зачем этот документ.** В Коре ~176 LLM-агентов (`union LlmTaskType`, факт по коду
> `llm-router.service.ts` — 176 членов на 2026-07). Чтобы в них не утонуть, группируем их
> в **функциональные модули** — связки агентов, которые ВМЕСТЕ дают одну способность,
> с понятной границей и входом/выходом.
> Это навигационная «лупа»: смотришь на модуль целиком, а не на ~176 имён.
