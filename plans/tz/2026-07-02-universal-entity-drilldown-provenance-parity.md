...
### Ф0 — A: задача из встречи проваливается + кружок отмечает выполнение `[ ]`
...
### Ф1 — Обобщение резолвера провенанса (goal/insight/friction + два уровня) `[ ]`
...
### Ф2 — B: цель показывает «откуда→момент» `[ ]`
...
### Ф3 — C: дашборд — блокер/риск/идея кликабельны по факту источника `[ ]`
...
### Ф4 — D: конфликт/трение — двухуровневый drill `[ ]`
...
### Ф5 — E: оживить deepLink задач/хода из встреч (сквозной корень) `[ ]`
