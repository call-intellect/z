/**
 * API-клиент для Knowledge-Core analytics (Z-Admin, Фаза 2 редизайна).
 *
 * Контракт: backend `/api/v1/admin/analytics/knowledge/*`. Защита — `SuperAdminGuard`.
 * На момент создания фронта бэкенд готовится параллельно — при отсутствии
 * эндпоинтов клиент получит 404 / 501, UI покажет `AdminEmpty`.
 */
