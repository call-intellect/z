...
8. **Ship-On — выкатываем включённым.** Готовая фича идёт в прод СРАЗУ включённой; «выкатить выключенным (дефолт OFF) и включить потом» — **запрещено**: не готова включиться — значит не готова к выкату. Никаких раскаточных «OFF → понаблюдаем → включим» — это плодит склад забытого выключенного функционала. Допустимы ровно два вида флагов: **(а) аварийный рубильник (kill-switch)** — фича работает (ON), рубильник нужен только чтобы экстренно выключить при инциденте, действий владельца не требует; **(б) решение владельца** — изменение, которое необратимо меняет что видят/могут люди или тратит/блокирует деньги (разграничение доступа, блокировка бюджета), выкатывается только вместе с заданным владельцем параметром (матрица доступа, сумма лимита) и сразу включается — нет параметра, не выкатываем. Единый реестр всех флагов (тип · состояние · что ждёт) — [docs/operations/feature-flags.md](docs/operations/feature-flags.md); любой новый флаг = строка туда.
9. **Крутилки — в AdminSetting, не в ENV и не в коде.** В `.env`/`env.schema.ts` допустимы ТОЛЬКО: секреты (ключи/пароли/токены/секреты подписи/ключи шифрования/соли), строки подключения (`DATABASE_URL`/`REDIS_URL`), bootstrap рантайма, нужный ДО доступности БД (`NODE_ENV`/`PORT`/`LOG_LEVEL`/домены cookie/публичный URL), и креды+endpoint внешней инфры (S3/LiveKit/TURN/SMTP/LLM-провайдеры/платёжка — переключаемые endpoint'ы остаются в ENV). **Всё остальное — порог, лимит, флаг, час доставки, debounce/TTL, retention, rate-limit, вес, выбор модели — это крутилка** и идёт в `AdminSetting` через `resolveSync`/`getDynamic` (admin→ENV→code-fallback) + строка в реестре [admin-setting-schema-registry.ts](backend/src/modules/admin/settings/admin-setting-schema-registry.ts) + сид + UI-поле. Магическая константа того же рода прямо в коде/SQL — то же нарушение (выносить через `getDynamic` с code-fallback). Прямой `process.env.*` мимо `env.schema.ts` — **запрещён**. Критерий «секрет vs крутилка», 6 GRAY-решений и реестр переноса — [plans/analysis/2026-06-20-config-env-vs-admin-settings-audit.md](plans/analysis/2026-06-20-config-env-vs-admin-settings-audit.md); ТЗ (гейт + перенос, по шагам) — [plans/tz/2026-06-20-config-knobs-to-admin-settings.md](plans/tz/2026-06-20-config-knobs-to-admin-settings.md).
