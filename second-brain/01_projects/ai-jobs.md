...
| **specialist 3-15 (Tasks, unified-task-extraction Ф1, 2026-06-23)** | `task-extract` | ~~DeepSeek-flash → OpenAI-mini → Ollama. Спайн-извлечение задач из НЕ-meeting-каналов через `Specialist315TasksWorker`.~~ **Извлекающий спайн снесён (заход B, 2026-06-30):** `Specialist315TasksWorker` и роут `action_item→TASKS` удалены — задачи извлекает combo (см. строку combined ниже). **`Specialist315TasksService` ЖИВ** — держит `runClarifySweep` для `TaskClarifySweepCron`. `task-extract` taskType оставлен пассивным. См. [[tracker]] §«Combo — единый источник задач». |
...
| **operations (β-8.1, 2026-05-25; расширен «Неделя компании» 2026-06-29; v2 2026-07-01)** | `checkin-sentiment`, `operations-weekly-digest` | DeepSeek-chat → OpenAI-mini → Ollama qwen3.5:9b. **С «Недели компании» (2026-06-29):** `operations-weekly-digest` — capable-вызов `deepseek-v4-pro` (`json_schema` strict, `reasoningEffort high`, `maxTokens 8000`), ОДИН проход сводит **5 дневных «Дней компании» (пн–пт)** + команда план/факт + повторяющиеся блокеры/риски + компас + прошлая неделя → строгий JSON: вердикт недели (4 оси) + письмо-проза + недельный компас (`WeeklyDigestService.generate` + `weekly-digest.prompt.ts`). Парс устойчивый (`extractWeekCompanyResponse`: toolCalls + обёртка `{result}` + lenient Zod). Post-LLM `clampWeekVerdict` (клиентский risk в дневных ⇒ `clients≠ok`; план/факт<50% ⇒ `execution≠ok`). `dayTrend` — детерминирован из 5 дневных вердиктов (без LLM). При провале — «сухой» fallback (NULL новых полей). Новый извлекающий taskType/агент НЕ вводился. Поля persist в `WeeklyOperationsDigest.{verdictJson,letterJson,goalAlignmentWeekJson,dayTrendJson}` (см. [[../02_architecture/data-model]], [[director-dashboard]] §«Неделя компании»). **С «Недели компании v2» (2026-07-01):** промпт `week-company-v2` — письмо COO из 12 секций (`intro..reflection`), имена людей/клиентов прямо, петля с прошлой неделей, «взгляд операционного директора» (`reflection`); строгая zod (`z.enum` ключей письма). Вход недельного промпта = **полные дневные письма** (`letterJson` дневных снапшотов, а не только вердикты; бюджет символов — крутилка `operations.weekly_digest.raw_char_budget`=60000). Сигналы за окно недели снапшочены в `metricsJson` (`risksByCause`/`ideaClusters`/`teamFrictions`/`blockers`, helper `collectWindowSignals`); ось «Команда» краснеет по повторяющимся трениям (`computeTeamFrictionClamp`, пороги-крутилки `operations.digest.team_friction_min_confidence`/`_repeat_count`). Миграций нет. См. [[director-dashboard]] §«Неделя/Месяц компании v2». |
| **operations (β-8.2, 2026-05-25)** | `commitment-extract-dates` | DeepSeek-chat → OpenAI-mini → Ollama qwen3.5:9b. _(`commitment-extract-status` — **снят** ТЗ commitment-social-layer-cleanup, 2026-07-01, Ф1, вместе с маршрутом из `seed-llm-task-routes-beta-8-2.ts`; извлечение срока обещания остаётся — граница факта.)_ |
| **operations (β-8.3, 2026-05-25; расширен «День компании» 2026-06-29; v2 2026-07-01)** | `operations-daily-digest` | Seed — `backend/scripts/seed-llm-task-routes-beta-8-3.ts`. **С «Дня компании» (2026-06-29):** capable-вызов вместо прежней markdown-сборки — ОДИН вызов выдаёт строгий JSON: вердикт дня (4 оси) + письмо-проза + дневной компас (`DailyDigestService.generate` + `daily-digest.prompt.ts`). Парс устойчивый (`extractDayCompanyResponse`: `toolCalls[0].input` И text, снятие обёртки `{result:{...}}`; Zod снисходительный с `.catch`). Post-LLM `clampVerdict` (критич. клиентский сигнал ⇒ ось «Клиенты» ≠ ok). При провале LLM/JSON — «сухой» fallback (NULL новых полей). Поля persist в `DailyOperationsDigest.{verdictJson,letterJson,goalAlignmentDayJson}`. **С «Дня компании v2» (2026-07-01, ТЗ `day-company-report-v2` Ф4/Ф5/Ф6):** промпт `day-company-v2` (`DAY_COMPANY_SYSTEM_PROMPT`) — письмо COO из 12 секций (`intro..reflection`), имена людей/клиентов прямо, петля со вчера, «взгляд COO» (`reflection`), сущность **«решения» (`decisions`) удалена** из промпта/схемы/`letterJson`; вход промпта наполнен 6 слоями с атрибуцией (голос/переписка/сигналы/конфликты/план-факт/вчера, см. [[director-dashboard]] §«День компании v2»); числа считает СИСТЕМА. **Маршрут:** DeepSeek Pro (`deepseek-v4-pro`) → GPT (`openai-via-proxy/gpt-5.4-mini`) → KIE (`kie/gemini-3.1-pro`), жёсткий `maxTokens` **снят** (патч `patch-daily-digest-route-deepseek-pro-gpt-kie.ts`, в `apply-prod-deploy.ts STEPS`). Крон `OperationsDailyDigestCron` `0 22 * * *` → `0 3 * * *` → **`@Cron('0 7 * * *', Europe/Moscow)` = 07:00 МСК** (после сбора чек-инов 05:00). См. [[../02_architecture/data-model]], [[director-dashboard]] §«День компании v2». |
| **operations («Месяц компании», 2026-06-30; v2 2026-07-01)** | `operations-monthly-digest` | Capable-вызов `deepseek-v4-pro` (`json_schema` strict + lenient Zod, `maxTokens 8000`), ОДИН проход сводит **4 недельных `WeeklyOperationsDigest`** месяца (компресс-вход: select без `letterJson`; пропущенные недели — graceful) → строгий JSON: вердикт месяца (4 оси) + письмо-проза + месячный компас (`MonthlyDigestService.generate` + `monthly-digest.prompt.ts`; `MONTH_COMPANY_SYSTEM_PROMPT` стабилен — prompt-caching). Post-LLM `clampMonthVerdict` (нельзя зелёный при красном клиенте). `weekTrend` детерминирован из недельных вердиктов (без LLM); `pace` (факт/план/ETA) считает **КОД** — LLM отдаёт только текст `leadingSignal`. При провале — «сухой» fallback. Новый извлекающий agent НЕ вводился. Route-сид `backend/scripts/seed-llm-task-routes-month-company.ts`. Запуск — 1/мес/Org (крон `OperationsMonthlyDigestCron`, см. [[workers-queues]]). Поля persist в `MonthlyOperationsDigest.{verdictJson,letterJson,goalAlignmentMonthJson,weekTrendJson}` (см. [[../02_architecture/data-model]], [[director-dashboard]] §«Месяц компании»). **С «Месяца компании v2» (2026-07-01):** промпт `month-company-v2` — письмо COO из 12 секций (`intro..reflection`), имена людей/клиентов прямо, петля с прошлым месяцем, «взгляд операционного директора» (`reflection`); ключ письма `decisions` переименован в **`ownerForks`** (блок «Что решить собственнику» → «Развилки месяца»). Сигналы за окно месяца снапшочены в `metricsJson` (`risksByCause`/`ideaClusters`/`teamFrictions`/`blockers`, helper `collectWindowSignals`); ось «Команда» краснеет по повторяющимся трениям (`computeTeamFrictionClamp`, пороги-крутилки `operations.digest.team_friction_min_confidence`/`_repeat_count`). Миграций нет. См. [[director-dashboard]] §«Неделя/Месяц компании v2». |
| **operations (Стенд сотрудника, письмо «Твой день», 2026-07-06; утро/вчера 2026-07-08)** | `personal-day-narrative` | **`deepseek-v4-pro`** (flash нестабильно держит строгую json_schema письма — эксперимент на «Стреле» 3/6 сломанных → PRO 6/6). Seed `backend/scripts/seed-llm-task-routes-personal-day-narrative.ts`. Веером **по одному сотруднику** (изоляция приватности + токены + устойчивость; общий статичный SYSTEM-префикс `personal-day-v2` кэшируется). Вход = разобранный слой графа + AI-резюме встреч person (commitments/blockers/voice/tasks/checkins/meetings/load/contribution). Настроение/конфликты физически не кладутся в пакет + запрет в промпте. Крон `PersonalDayNarrativeCron` (см. [[workers-queues]]) — **утро про вчера** (`packageRef=now−24ч`, час `operations.personal_day_narrative.morning_hour` дефолт 7). Persist `PersonalDayNarrative`, отдаётся `GET /me/day-letter`. Kill-switch `operations.personal_day_narrative.enabled` (ON). См. [[employee-stand]]. |
...
| **~~operations (Универсальный фиксатор, 2026-06-21)~~ → СНЕСЁН 2026-06-29** | ~~`day-signal-detect`~~ | **Удалён** (день план/отчёт из графа, ветка `work/2026-06-29`): второй LLM-проход больше не нужен — план/отчёт дерайвятся из разметки `block-ingest` тонким НЕ-LLM сборщиком, «не сделано» — через `ClosureVerifierService` (taskType `task-closure-verify`). Снесены: taskType `day-signal-detect`, маршрут `seed-llm-task-routes-day-signal.ts`, промпт `day-signal-detect.prompt.ts`, метрики `day_signal_*`. См. §«Универсальный фиксатор чек-инов» ниже. |
| **specialists combined (Фаза 6 §3, 2026-05-26; основной путь + канало-агностичен с захода A 2026-06-30)** | `knowledge-specialists-combined` | **DeepSeek V4 Pro** → OpenAI via proxy `gpt-5.4` → Ollama `qwen3.5:9b`. Б+ объединённый вызов: один LLM-запрос извлекает сущности вместо раздельных специалистов. ~3.7× дешевле при сопоставимом качестве (eval `backend/scripts/eval/judge-specialists-bplus-vs-g.ts`). Воркер — `SpecialistsCombinedWorker`, очередь `core.specialists-combined`. **С захода A (2026-06-30) — ВКЛ по дефолту, основной путь** (рубильник AdminSetting `knowledge.specialists_combined_enabled`, kill-switch ON; не A/B): **канало-агностичен** (chat/chatbox/Bitrix, не только встречи — контекст из `RawEvent`), воспроизводит 4 побочки (rebuild профиля/навыков, гигиена решений, ProcessTemplate через раздельный process-detector), питается few-shot реестром всех 57 типов + скелет-картой + нахлёстом окон/gleaning. **С захода B (2026-06-30) combo эмитит ещё и задачи** — tool переименован `submit_all_8_entities`→`submit_all_entities`, 9-й выход `tasks[]` (decisions / ideas / insights / experiments / regulations / knowledge_categories / skill_traits / helpfulness_traits + **tasks**); правило «обещание=задача», группировка соседних поручений автора в `subtasks[]`; задачи материализует `TaskDraftMaterializerService` (tracker) → `IntakeIssue`. См. §«Переписывание извлекающего слоя — заход A/B» ниже, [[tracker]] §«Combo — единый источник задач», [[../02_architecture/knowledge-core]]. |
...
### Капсула «## О компании» в извлекающих агентах (`CompanyCapsuleService`, 2026-07-07, коммит `25e92279`)

Общий загрузчик `CompanyCapsuleService.load(tenantId, surface)` (ai-модуль, exported) собирает короткий блок «## О компании» (название / чем занимается / стадия / миссия из `CompanyProfile`, fail-open, метрика `company_capsule_injected_total{surface}`). Капсула инъектится в **USER-часть** (НЕ system — межтенантный prefix-cache цел) двух извлекающих путей: **block-ingest** (`BlockExtractionService.extractFull` грузит один раз на извлечение, surface `block_ingest`, прокидывает во все окна) и **tasks** (`TaskExtractionService.extractTasks`, surface `tasks`). Попутно структурный путь задач начал получать `orgContext` (проекты/цели/люди из `OrgContextService`), который до `buildTasksStructuredPrompt` раньше не доезжал. Прежние потребители капсулы (chat-v2/concierge — surfaces `chat_v2`/`concierge`) не тронуты. Источник короткого паспорта — [[company-framework-slots]] (`company-summary-compiler`).
...
## Универсальный фиксатор чек-инов (2026-06-21 → перестроен на граф 2026-06-29)

**Источник:** анализ [`plans/analysis/2026-06-29-checkin-ingest-rebuild.md`](../../plans/analysis/2026-06-29-checkin-ingest-rebuild.md) · ТЗ [`plans/tz/2026-06-29-checkin-day-report-from-graph.md`](../../plans/tz/2026-06-29-checkin-day-report-from-graph.md) (8 фаз, ветка `work/2026-06-29`, коммиты `114fc0f4..0ab7090b`). Модель/поля — [[../02_architecture/data-model]] §«`DailyCheckIn` += 4 сущности отчёта»; cron/листенер — [[workers-queues]].

Раньше «план/отчёт» распознавался только в личке Telegram-бота → переписки (Bitrix/chatbox/email) и реплики на встречах в `DailyCheckIn` не попадали (сотрудник на дашборде «не сдал»). ТЗ 2026-06-21 ввело выделенный слой-детектор `day-signal-*` (второй LLM-проход `day-signal-detect` + дневной свип в 21:00), но из переписок он структурно не срабатывал (противофаза таймингов забора Bitrix/chatbox и свипа) и **дублировал разметку, которую уже делает `block-ingest`** (`plan_item`/`done_item` уже в enum графа).

**С 2026-06-29 слой `day-signal-*` СНЕСЁН целиком** (детектор/экстрактор/агрегатор/свип/spec/prompt + taskType `day-signal-detect` + метрики `day_signal_*`). План/отчёт теперь **дерайвятся из графа** тонким НЕ-LLM сборщиком — без второго LLM-прохода:

- **`DayReportCollectorService`** — graph-read: `collectForDay` тянет размеченные `block-ingest` блоки (атрибуция по `evidence.authorPersonId` + `sourceTimestamp` в TZ `Person`), `assembleAndUpsert` собирает **4 сущности** дня (сделано / не сделано / помешало / идеи) и пишет через `DailyCheckInService.upsertFromDaySignal`/`upsertInternal` (патч-стиль `notDone`/`ideas`, не затирает личный ответ; source = преобладающий из `evidence.sourceType`). Метрики `day_report_{collected,block_dropped_no_person,not_done_verify_calls}_total`.
- **«Не сделано» — семантически** через переиспользуемый **`ClosureVerifierService`** (taskType `task-closure-verify`, вынесен из `task-completion.handler` — поведение `verify`/`parse`/guard идентично): `computeNotDone` сначала предфильтрует точные done-совпадения, иначе verify незакрытых пунктов плана. Явный негатив «не дал» — строкой-ожиданием (`CheckinExpectationService`).
- **Два входа** (см. [[workers-queues]]): `DayReportCollectorCron` (`@Cron('0 5 * * *', Europe/Moscow)` за вчера по всем Org, Redis NX) + `MeetingCheckinListener` (`@OnEvent('meeting.ai_ready')`, graph-derive same-day по участникам встречи).
- **Полнота отчёта** `reportCompleteness` (draft/full) выводится в `toDto` на чтении (qualityScore ≥ `dayReport.completenessQualityThreshold` И ≥3/4 частей); НЕ влияет на «сдал».
- **Крутилки** (AdminSetting, без ENV): `dayReport.enabled` (kill-switch ON), `dayReport.completenessQualityThreshold` (0.5).
...
- **МИНУС cron-проходы:** удалён 05:00 UTC-проход решений изнутри `specialist-3-3-probe.service.ts` (Ф1, поводы `decision.*` сняты — `decision-extract`/извлечение решений **остаётся**) + cron `commitment-followup` (Ф2, probe-инспектор обещаний; `commitment-extract-dates` для извлечения срока **остаётся**, а `commitment-extract-status` снят целиком ТЗ commitment-social-layer-cleanup, 2026-07-01).
...
**Источник:** ТЗ [`plans/tz/2026-06-30-extraction-layer-rewrite.md`](../../plans/tz/2026-06-30-extraction-layer-rewrite.md) (Ф1–Ф10 + Ф12a, ветка `work/2026-06-29`, коммиты `41736929`..`c8fef016`). Полная карта пайплайна — [[../02_architecture/knowledge-core]] §«Переписывание извлекающего слоя — заход A»; воркеры/cron — [[workers-queues]].

- **Новый taskType `meeting-skeleton` (Ф6)** — **дешёвая** модель по стандарту (deepseek-flash → gpt-5.4-nano → kie/gemini-3.1-pro, **НЕ anthropic**), 1 проход на сжатом входе разговора → «Карта встречи» (`{agenda, milestones[], keyNames[]}`, maxTokens ~800, fail-open). Зарегистрирован в `llm-router` (union `LlmTaskType` + `ALL_LLM_TASK_TYPES`); сид-маршрут `seed-llm-task-routes-knowledge-core.ts` (доезжает агрегатором через `seed-llm-task-routes-${sub}.ts`, `sub='knowledge-core'`, phase `seed-llm-routes`). Карта держится **in-memory** на время одного `extractFull` и подмешивается в шапку каждого окна `block-ingest` для разрешения кореференций. Сервис `MeetingSkeletonService` (knowledge-core/services). Kill-switch `knowledge.skeleton_pass_enabled` / порог коротких `knowledge.skeletonMinSegments`. Метрика `kc_meeting_skeleton_total{outcome}`.
- **Движок `block-ingest` НЕ менялся** — остаётся `deepseek-v4-pro` primary (**Вариант A**: постановка Claude Opus отменена владельцем — стандарт «DeepSeek primary везде, НЕ anthropic», + `anthropic.maxDataClass='sensitive'`<`private` молча отфильтровывает Opus на private-данных, + block-ingest — самый частый LLM-вызов). Качество извлечения дают модель-агностичные Ф4 (few-shot реестр 57 типов) + Ф5 (нахлёст окон/gleaning) + Ф6 (скелет-карта).
- **`process-detector` снова в РАЗДЕЛЬНОМ dispatch (Ф3).** В combo-режиме он был подавлен (`PROCESS_DETECTOR ∈ COMBINED_COVERED`); теперь убран из `COMBINED_COVERED` (`router.service.ts`) и диспатчится отдельным путём — строит `ProcessTemplate.steps[]`. Вариант A; авто-`handoffs`/`decisionPoints` сознательно НЕ строим. Combo при этом восстанавливает остальные 3 побочки (rebuild профиля клона + навыков, гигиена решений).
- **Комбо канало-агностичен (Ф2)** — `knowledge-specialists-combined` теперь разбирает chat/chatbox/Bitrix так же связно, как встречу (`SpecialistsCombinedJobData{tenantId, sourceType, externalId}`, контекст из `RawEvent`, `channelKind` в промпт). Раньше combo был meeting-only, чат обслуживали слабые поблочные пути.
[[../index|← index]]

## Переписывание извлекающего слоя — заход B: задачи через combo (2026-06-30)

**Источник:** ТЗ [`plans/tz/2026-06-30-extraction-layer-rewrite.md`](../../plans/tz/2026-06-30-extraction-layer-rewrite.md) (заход B, ветка `work/2026-06-29`, коммиты `6816cacd`..`fa55ddf2`). Combo стал единым движком задач: общий разборщик `knowledge-specialists-combined` достаёт задачи/обещания тем же проходом, что граф, из canonical-блоков **встречи И чата**. Прод-операции — [`docs/operations/prod-deploy-log.md`](../../docs/operations/prod-deploy-log.md).

- **combo эмитит `tasks[]` (9-й выход).** Tool combo переименован `submit_all_8_entities`→`submit_all_entities`. Промпт: правило «обещание=задача» (само-назначение автору реплики) + группировка соседних поручений одного автора в `subtasks[]` (шаги одного дела → одна задача + чек-лист, не N карточек). `persistTasks` → `TaskDraftMaterializerService`.
- **`TaskDraftMaterializerService`** (`tracker/services/task-draft-materializer.service.ts`, инжектится в combo) — канало-агностичный материализатор task-черновиков → `IntakeIssue`: идемпотентность `externalId=mat_<sha1(channel:sourceId:quote|title)>`, резолв исполнителя org-wide `AssigneeResolverService`→skill-routing, дедуп-suggest `TaskDedupService.evaluate`, подзадачи → `IntakeIssue.checklistJson`, enqueue auto-triage. Метрика `task_draft_materialized_total{channel,status}`. Provider+export в `TrackerModule` (knowledge-core→tracker, оба `@Global`, без цикла).
