...
export const GOAL_EXTRACT_SYSTEM_PROMPT = withAsrNote(
  ...
  [
    ...
    '- Тест на цель: есть ли НАБЛЮДАЕМОЕ изменение состояния/метрики («было → стало»)? Если в блоке описана только выполненная работа (что-то сделали/запустили/провели), а изменение состояния не названо — это НЕ цель.',
    ...
    '',
    'Блок НЕ выражает цель (болтовня, вопрос, частная задача-output, рутинная операционная задача без изменения метрики/состояния, разовое поручение, благодарность, абстрактное пожелание) → isGoal=false, низкий confidence, statement «недостаточно сигнала», description=null, measurable=null.',
    '',
    ...
    '',
    'Пример 3 (good — маленькая цель-ступенька с числом):',
    'Блок «Планы месяца» (пункт плана). Цитаты: «Сергей: в этом месяце надо провести 50 кастдев-встреч, сейчас сделали около 10».',
    'Вывод: {"isGoal": true, "statement": "Провести 50 кастдев-встреч за месяц", "description": "Рост каждевов: с ~10 до 50 за месяц.", "horizon": "monthly", "measurable": {"name": "Кастдев-встречи", "unit": "встреч", "startValue": 10, "targetValue": 50}, "confidence": 0.6}. Маленькая, но настоящая цель-ступенька: есть число и изменение состояния.',
    '',
    'Пример 4 (bad — output под видом цели):',
    'Блок «Идеи» (пункт плана). Цитаты: «Аня: давайте запустим рекламную кампанию».',
    'Вывод: {"isGoal": false, "statement": "недостаточно сигнала для извлечения цели", "description": null, "horizon": "quarterly", "measurable": null, "confidence": 0.2}. Это output (работа) — наблюдаемое изменение состояния/метрики не названо.',
    '',
    'Пример 5 (bad — не цель, обычная задача-output):',
    ...
    '1. isGoal=true только при НАБЛЮДАЕМОМ изменении состояния/метрики (outcome), а не при просто выполненной работе (output)/задаче/вопросе?',
...
export const GOAL_EXTRACT_JSON_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    isGoal: {
      type: 'boolean',
      description: 'Выражает ли блок цель компании (наблюдаемое изменение состояния/метрики). false — болтовня/вопрос/рутинная задача-output/разовое поручение.',
    },
    statement: {
      type: 'string',
      minLength: 3,
