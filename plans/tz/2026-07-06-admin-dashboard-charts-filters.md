...
## Фаза 3 (по фидбеку после второй живой QA)

Владелец увидел графики и попросил доработать ещё раз:
- [x] Тултип на графиках показывал сырое `costUsd: 0.0299` вместо человеческого `$`-формата.
- [x] Клик по дате на `/admin` должен сразу открывать календарь, а не заставлять переключать период на «Свой период» и вручную вписывать 2 даты.
- [x] Аналогичная возможность выбрать конкретный период нужна и на `/admin/analytics/llm-cost` (там раньше был только `7д/30д/90д`, вообще без произвольного диапазона на overview).

**Тултип** — `ChartTip`/`BarTrend`/`AreaTrend` (`src/ui/components/dashboard/modern/`) получили опциональные `valueFormatter`/`seriesLabel`(`name`) — recharts берёт `name` с `<Bar>`/`<Area>` вместо сырого `dataKey`, значение форматируется через переданный `formatUsd`. Обратно совместимо (опциональные пропсы, остальные потребители — `WeeklyDigestClient`/`MaturityWidget`/etc — не тронуты). Применено во всех cost-usages: 4×`BarTrend` на `/admin`, 4×`AreaTrend` на `/admin/analytics/llm-cost`.

**Календарь** — добавлена новая зависимость `react-day-picker@^10` (все API-примеры сверены через context7 `/gpbl/react-day-picker`, версия 9.14/10 docs) + `frontend/src/ui/shadcn/calendar.tsx` (Tailwind-стилизованный `DayPicker`, `mode="range"`) + переиспользуемый `frontend/src/ui/components/admin/DateRangeCalendarPopover.tsx` (Popover+Calendar, кнопка «Применить»/«Сбросить»). Важная деталь, найденная живым QA: `react-day-picker` в range-режиме уже ПОСЛЕ ПЕРВОГО клика возвращает валидный `{from, to}` (`from===to`, однодневный диапазон) — наивный «авто-закрыть popover, как только range валиден» обрезал бы выбор до одного дня после первого же клика. Решение — explicit «Применить»/«Сбросить», без авто-коммита на `onSelect`.
- `/admin` — дата-бейдж стал самим триггером календаря (кнопка вместо текста); из выпадающего периода убран пункт «Свой период» (он теперь только следствие выбора в календаре — синхронизируется автоматически).
- `/admin/analytics/llm-cost` overview — `LlmCostOverviewQuerySchema`/`LlmCostOverviewView` += `dateFrom`/`dateTo` (запрос) и `dateFrom`/`dateTo` (факт. резолв в ответе, для бейджа); `resolveDateRange()` (был только в `companyDetail`) обобщён — теперь всегда возвращает и `prevGte` (для `changePct` при произвольном диапазоне — предыдущий период той же длины), используется в `overview()` тоже. Календарь встал рядом с `PeriodSwitcher` (7д/30д/90д), выбор пресета сбрасывает custom-диапазон и наоборот.

Тесты: 2 новых в `llm-cost-dashboard.service.spec.ts` (custom dateFrom/dateTo сужает overview; без дат — резолвит валидный диапазон из period). Полный прогон backend `src/modules/admin/` — 527/527 (16 новых/изменённых assert в связанных файлах: 11 llm-cost-dashboard + 5 admin-usage). Frontend typecheck/lint чисты, `test:unit` 595/599 (те же 4 пре-существующих сбоя в `tracker/*`).

Живая QA (Playwright): tooltip на обеих страницах показывает `Расход, $: $0.113`/`$0.021` вместо `costUsd: N`; календарь на `/admin` — клик по бейджу → 2-месячный календарь → клик 1 июля + клик 6 июля → видимая подсветка диапазона → «Применить» → бейдж/period/данные обновились согласованно ($0.153/16 вызовов), «Сбросить» внутри попапа корректно вернул на пресет «Неделя»; тот же цикл (выбор диапазона → Применить → сброс через пресет-кнопку «7д») подтверждён на `/admin/analytics/llm-cost`. 0 console-ошибок на всех состояниях.
