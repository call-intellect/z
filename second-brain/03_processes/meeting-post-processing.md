...
Сейчас основная цепочка **полностью работает в проде** (запись, расшифровка, отчёт хосту, попадание фактов в граф, полная маршрутизация в специалистов Слоя 3 по `signalType`). Остаётся половинчатым немногое: клон должности пересобирается не по событию «встреча кончилась», а по своему расписанию; авто-доставка отчёта в Telegram/email — не во всех контурах. Этот документ честно фиксирует — где как именно.
...
| 7б | Специалисты Слоя 3 | `RouterService.dispatch` по `signalType` кладёт блок в `core.specialist-routing` с нужным jobName — маршрутизация во **все** специалисты: `decision\|rationale\|decision_basis`→`3-3-decisions`, `regulation\|process_step`→`3-1-regulations`, `pain\|risk\|churn_risk\|objection`→`3-5-insights`, `idea\|feature_request`→`3-6-ideas`, `reasoning` (subject=employee)→`3-7-skill`, `fact` (есть Customer/Vendor/Project entity)→`3-4-project-customer` (+ `commitment\|plan_item`→`3-14-goals`) | `backend/src/modules/knowledge-core/services/router.service.ts:27-32,53-56`, `workers/<specialist>-*.worker.ts` | `core.specialist-routing` | `Decision`, `Regulation`, `Insight`, `Idea`, `Card` (как client/project), `SkillProfile`, `Goal` | ✅ полная маршрутизация |
...
| 9 | Уведомление хосту + пуш в связанные чаты | `MeetingEvent.ai_notified` + (в идеале) `sendNotification(eventType='meeting.report_ready')` → in-app + Telegram + email по предпочтению канала. **Плюс реальная проактивная доставка при `ai_ready`:** `MeetingsService.postMeetingClosedToLinkedChats(meetingId)` сам постит запись + резюме в связанный huddle-разговор и в work-chat связанных задач (best-effort, `.catch`) | `backend/src/modules/ai/workers/notify.worker.ts`, `backend/src/modules/meetings/meetings.service.ts:798,808`, `backend/src/modules/conversational/conversational.service.ts` | `ai.notify`, `conversational.send` | `MeetingEvent`, `Notification`, сообщения в huddle/work-chat | ⚠️ уведомление хосту в MVP-режиме (в Telegram/email не во всех конфигурациях); пуш в связанные чаты работает |
...
**Реализовано полностью (обновление аудита):**
- **Полная маршрутизация в специалистов Слоя 3 через `core.specialist-routing`** — `RouterService.dispatch` диспетчит по `signalType` во все специалисты (3-1 / 3-3 / 3-4 / 3-5 / 3-6 / 3-7-skill + 3-14-goals), каждый со своим jobName. Фаза 4 умбреллы `2026-05-22-final-roadmap.md` закрыта.
- **Проактивный пуш при `ai_ready`** — `postMeetingClosedToLinkedChats` постит запись+резюме в связанный huddle-разговор и в work-chat связанных задач (Шаг 9).

**Заложено в ТЗ, но реализовано частично:**
- **Авто-доставка отчёта хосту в Telegram/email** — `MeetingEvent.ai_notified` ставится всегда, но отправка через `NotificationDispatcher` не во всех контурах. См. `notify.worker.ts`.
