...
export class RetrievalCacheService {
  ...
  buildKey(args: RetrievalCacheKeyArgs): string {
    const payload = [
    ...
    return `${KEY_PREFIX}:${args.tenantId}:${hash}`;
  }

  async get(args: RetrievalCacheKeyArgs): Promise<RetrievalCacheEntry | null> {
    const key = this.buildKey(args);
    try {
      const raw = await this.redis.client.get(key);
      if (!raw) return null;
      const parsed = JSON.parse(raw) as RetrievalCacheEntry;
      this.metrics.incRetrievalCacheHit({
      ...
      this.logger.warn(
        ...
        'RetrievalCache.get: redis error — treat as miss',
      );
      return null;
    }
  }
  ...
  async set(args: RetrievalCacheKeyArgs, entry: RetrievalCacheEntry): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        { key, err: err instanceof Error ? err.message : String(err) },
        'RetrievalCache.set: redis error — игнор',
      );
    }
  }
  ...
  private async scanAndDelete(pattern: string): Promise<number> {
    ...
    try {
      ...
      this.logger.warn(
        ...
        'RetrievalCache.scanAndDelete: redis error',
      );
    }
    return deleted;
  }
