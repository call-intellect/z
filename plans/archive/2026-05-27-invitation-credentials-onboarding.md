...
> Все 8 фаз реализованы и подтверждены в production-коде (schema, сервисы, mail-шаблон, контроллер/эндпоинт, обе frontend-страницы), есть выделенный коммит 402ee517 и аудит-фикс b10ab547. Остаточные пункты косметические: о
> ⚠️ Хвосты (см. реестр приоритетов): Тест setInitialPassword в accounts.service.spec.ts отсутствует
