...
export class SprintsService {
  ...
  async list(args: { tenantId: string; query: ListSprintsQuery }): Promise<ListSprintsResponse> {
    ...
    const orderBy: Prisma.CycleOrderByWithRelationInput[] =
      query.sortBy === 'startDate' ? [{ startDate: query.sortDir }] : [{ startDate: 'desc' }];
    ...
    const [issueGroupTotal, issueGroupDone, hintGroupActive, hintGroupCritical, meetingGroup] =
      await Promise.all([
  ...
  async quickCreate(args: {
    ...
    if (dto.scope === 'project') {
      ...
      const cycleResponse = await this.cycles.create(project.id, cycleDto, tenantId, userId);
    ...
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        const identifier = await generateProjectIdentifier(projectName, tenantId, tx);
        ...
        const projectUnchecked: Prisma.ProjectUncheckedCreateInput = {
          ...
          subjectPersonId: dto.scope === 'person' ? (dto.refId ?? null) : null,
          departmentId: dto.scope === 'department' ? (dto.refId ?? null) : null,
  ...
  private buildProjectName(scope: QuickCreateSprintDto['scope'], refLabel: string): string {
