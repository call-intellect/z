...
- signalType: ровно одно значение из enum:
  ...
  // SBA α-2 wave 3 (2026-05-24) — типы для tracker ingest (Phase 1 трекера).
  // Обычно эти типы НЕ извлекаются LLM из текста — они генерируются адаптером tracker.adapter.ts напрямую
  // из событий трекера. Описание здесь — для случая, когда такие фразы встретятся в тексте/комментариях.
  ...
  - emotional_support: эмоциональная поддержка коллеги. Маркеры: «ты молодец», «нормально что устал», «бывает», «держись». Источник для Specialist 3.8 и β-7.
