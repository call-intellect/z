...
> Реализовано полностью (verified по коду ~98%): новая route-группа /company-admin со своим сайдбаром и тремя вкладками создана, старые settings/admin/* → redirect, баг двойного сайдбара устранён (settings/admin/layout.tsx
> ⚠️ Хвосты (см. реестр приоритетов): Backend OrgAdminKnowledgeController осознанно оставлен живым (не orphan, costUsd вырезан) — потенциальная техдолг-чистка
