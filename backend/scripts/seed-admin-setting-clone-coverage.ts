...
const SEEDS: SettingSeed[] = [
  {
    key: 'knowledgeClone.profileMinConfidence',
    value: 0.55,
    category: 'knowledge',
    section: 'clone',
    severity: 'low',
    description:
      'Мягкий порог уверенности, при котором профиль знаний (клон сотрудника) материализуется даже без auto-триажа (provisional/light). Ниже — в ручную курацию (deep). Диапазон 0–1.',
  },
];
...
async function main(): Promise<void> {
  console.log('=== seed-admin-setting-clone-coverage START ===');
  ...
  console.log('=== seed-admin-setting-clone-coverage DONE ===');
}

main()
  .catch((err) => {
    console.error('seed-admin-setting-clone-coverage FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
