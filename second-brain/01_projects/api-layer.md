...
| POST | `/api/v1/concierge/messages` `/messages/once` | SSE-стрим / polling-fallback Мастера. **С 2026-06-28 (chat-surface convergence)** body `PostConciergeMessageBodySchema` += `scope?`/`scopeRefId?`/`asOf?` — протаскиваются в `chatV2.askEphemeral` (питают scoped-чат issue/org + снимок «на дату»). SSE-эвент `message` несёт `citations`+`needsClarification`, добавлен эвент `confirm_required`. ТЗ [`2026-06-25-edinyy-pomoshnik-chat-surface-convergence`](../../plans/tz/2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md). | γ-2 |
| GET | `/api/v1/concierge/conversations` | История диалогов пользователя. **С 2026-06-28** каждая запись += `titlePreview` (первое user-сообщение, ≤80 симв., хелпер `concierge-title-preview.ts`) — фронт берёт его для названия диалога. | γ-2 |
...
| POST | `/api/v1/me/tasks/complete` | **Детерминированное «выполнил задачу» (task-loop Ф2c, 2026-06-22):** `{taskName, note?}` → `MeTasksService.resolveOpenTaskByName` (assignee + state.category∉completed/cancelled + `deletedAt:null`, каскад exact→includes; `404 task_not_found` / `409 task_ambiguous`) → **идемпотентно** создаёт `TaskClosureCandidate(pending)` (sentinel `sourceBlockId='concierge-complete:<userId>'`, `@@unique`). Задачу НЕ закрывает — подтверждает человек в `/actions` (R3). Инструмент помощника `complete_task`. RBAC `issue`/`write`. | — |
| POST | `/api/v1/me/tasks/progress` | **Отчёт о прогрессе по имени задачи (task-loop Ф2c, 2026-06-22):** `{taskName, progress}` → резолв задачи (как выше) → `ProgressUpdatesService.create` (human, `health='on_track'`, `body=progress`). Инструмент помощника `report_task_progress`. RBAC `issue`/`write`. | — |
| POST | `/api/v1/me/tasks/suggest-assignee` | **Подсказка исполнителя по скиллам (Слой 2, 2026-06-22):** по задаче (текст/`issueId`) `SkillRoutingService.suggestAssignee` предлагает кандидатов: hard-gate `departmentId` → semantic pgvector (`skill_traits`) → role-prior → LLM-арбитр `task-assignee-arbiter`. Возвращает `{userId, confidence, rationale, matchPath}[]` (top `taskRouting.topK`, ≥ `taskRouting.suggestMinConfidence`). **НЕ присваивает** (Р1, 152-ФЗ) — только подсказка; присвоение существующим путём `addAssignee` (`POST /issues/:id/assignees`, `viaRouting:true` → `incRoutingSuggestionAccepted`). Эндпоинт инструмента помощника `suggest_assignee`. Kill-switch `taskRouting.enabled`. ТЗ [`2026-06-21-skill-based-task-routing`](../../plans/tz/2026-06-21-skill-based-task-routing.md). | — |
...
| Метод | Путь | Назначение |
|---|---|---|
| GET | `/api/v1/provenance/:entityType/:entityId` | Цепочка первоисточника сущности (decision/issue/regulation/instruction/block/idea/entity + `goal`/`insight`/`friction` — 2026-07-03), отфильтрованная по правам зрителя (viewer из guard): `{ nodes[]{blockId, source{type,refId,label,deepLink}, quote, attribution, startMs, accessFiltered, needsReview}, coverage }`. `entityType` вне enum → 400 `invalid_entity_type`. Закрытый блок → `accessFiltered:true`, quote+label+deepLink скрыты. **`friction`** (конфликт person↔person, id=`EntityLink.id`) отдаётся двухуровнево по роли зрителя (из guard): роль вне `provenance.frictionVerbatimRoles` → агрегат (лейбл встречи, deepLink без `?t=`, quote=тема), owner/admin → дословная цитата + `?t=`. |
...
**Удаление/восстановление норм и решений (2026-06-22, qa-fixes Ф6–Ф7):** owner/admin (`requireWrite`):
- `DELETE /api/v1/regulations/:id?kind=` — soft-delete (`deletedAt=now`, `deletedById`); `POST /api/v1/regulations/:id/restore` (body `{kind}`) — снять. Для всех 4 типов норм (regulation/standard/process/policy/instruction по `kind`).
...
| **POST** | `/api/v1/dashboard/operations/weekly-digest/generate?weekStart=…` | Принудительная перегенерация. С «Недели компании» (2026-06-29) — ОДИН capable LLM-вызов json_schema strict: вердикт недели (`verdictJson`) + письмо (`letterJson`) + недельный компас (`goalAlignmentWeekJson`) + тренд по дням (`dayTrendJson`, детерминирован), persist в `WeeklyOperationsDigest`. | `admin` `super_admin` | **β-8.1** |
| **GET** | `/api/v1/dashboard/operations/weekly-digest/latest` | **Новый (2026-06-29, «Неделя компании»)** — последний сгенерированный недельный дайджест для героя `WeekCompanyHero` на `/dashboard`. | `coo` `owner` `admin` `super_admin` | **Неделя компании** |
...
| **POST** | `/api/v1/dashboard/operations/daily-digest/generate?date=YYYY-MM-DD` | Принудительная перегенерация. С «Дня компании» (2026-06-29) — ОДИН capable LLM-вызов с json_schema strict: вердикт дня (`verdictJson`) + письмо-проза (`letterJson`) + дневной компас (`goalAlignmentDayJson`), persist в `DailyOperationsDigest`. Новых эндпоинтов нет. | `admin` `super_admin` | **β-8.3 / День компании** |
| **GET** | `/api/v1/dashboard/operations/monthly-digest?period=YYYY-MM` | **Новый (2026-06-30, «Месяц компании»)** — сохранённый `MonthlyOperationsDigest` за месяц или null (`getStored`). | `coo` `owner` `admin` `super_admin` | **Месяц компании** |
| **GET** | `/api/v1/dashboard/operations/monthly-digest/latest` | **Новый (2026-06-30)** — последний сгенерированный месячный дайджест для героя `MonthCompanyHero` на `/month`. | `coo` `owner` `admin` `super_admin` | **Месяц компании** |
| **POST** | `/api/v1/dashboard/operations/monthly-digest/generate?period=YYYY-MM` | **Новый (2026-06-30)** — принудительная генерация: ОДИН capable LLM-вызов (`operations-monthly-digest`) сводит 4 недельных дайджеста → вердикт месяца + письмо + месячный компас + тренд по неделям, persist в `MonthlyOperationsDigest`. | `owner` `admin` `super_admin` | **Месяц компании** |
| **GET** | `/api/v1/dashboard/operations/monthly-digest/available-periods?limit=` | **Новый (2026-06-30)** — `{rhythm, periods:[{period,stateHint,title}], latest}` для навигатора/архива (`PeriodNavigator`). | `coo` `owner` `admin` `super_admin` | **Навигация по датам** |
| **GET** | `/api/v1/dashboard/operations/daily-digest/available-periods?limit=` | **Новый (2026-06-30, «Навигация по датам»)** — список доступных дней с `stateHint` для навигатора/архива. | `coo` `owner` `admin` `super_admin` | **Навигация по датам** |
| **GET** | `/api/v1/dashboard/operations/weekly-digest/available-periods?limit=` | **Новый (2026-06-30, «Навигация по датам»)** — список доступных недель с `stateHint` для навигатора/архива. | `coo` `owner` `admin` `super_admin` | **Навигация по датам** |
| ~~**GET**~~ | ~~`/api/v1/dashboard/operations/open-commitments?days=14`~~ **СНЯТ 2026-07-01** | Висящие обещания в команде с именами — эндпоинт удалён (ТЗ commitment-social-layer-cleanup, Ф5). | — | **β-8.2 (снято)** |
| **GET** | `/api/v1/dashboard/operations/weekly-per-person` | Недельный план-факт по людям: обещано/закрыто/просрочено per Person (по `IdeaBlock.commitmentAuthorPersonId`). `WeeklyPerPersonService`. Виджет в «Недельной сводке». **С «Недели компании» (2026-06-29):** опц. query `weekEnd=` (окно пн–пт для героя; по умолчанию пн–вс); строка `rows[]` += `goalContributionNet` (net-вклад в главную цель из `PersonGoalContribution`). | `coo` `owner` `admin` | **дашборды 2026-06-05 (D)** |
...
| ~~**GET**~~ | ~~`/api/v1/me/promises?status=…`~~ **СНЯТ 2026-07-01** | Свои обещания — эндпоинт + `MyPromisesController` удалены (ТЗ commitment-social-layer-cleanup, Ф5). | — | **β-8.2 (снято)** |
| ~~**POST**~~ | ~~`/api/v1/me/promises/:blockId/mark`~~ **СНЯТ 2026-07-01** | Ручное закрытие обещания — удалён. | — | **β-8.2 (снято)** |
| ~~**PATCH**~~ | ~~`/api/v1/me/promises/:blockId/reschedule`~~ **СНЯТ 2026-07-01** | Перенос срока своего обещания — удалён вместе с разделом `/me/promises`. | — | **дашборды 2026-06-05 (E) (снято)** |
| GET | `/api/v1/personal-relations?personId=&relationType=` | EntityLink-связи человека | `admin` `coo` | β-8 |
| ~~**GET**~~ | ~~`/api/v1/personal-relations/commitments?personId=`/`?entityId=`~~ **СНЯТ 2026-07-01** | Исходящие + входящие обещания человека — эндпоинт удалён (ТЗ commitment-social-layer-cleanup, Ф5; секция «Обещания» на `/persons/[id]` тоже снята). | — | **β-8.2 (снято)** |
...
> **`ensurePersonForUser` — у владельца Org есть своя `Person` (МТЗ №1 Фаза 9, 2026-06-04, коммит `7cffb1e3`).** _(Триггером было падение `/me/promises`; раздел `/me/promises` снят 2026-07-01 ТЗ commitment-social-layer-cleanup, но сам fix self-Person остаётся — он нужен всем self-сценариям. Резолв своей `Person` вынесен в `SelfPersonResolverService`.)_ Механика:
...
| GET | `/api/v1/orgs/:orgId/welcome` | сохранённые ответы Блока A `{teamSize,industry,painPoints,currentStack,plannedFeatures}` — для регидрации мультивыбора шагов 4/5/6 при «Назад» (2026-06-25). Парный к `PATCH /orgs/:orgId/welcome`. |
...
- **2026-06-23 (наблюдаемость самообучения SubjectMemory, автономизация Блок D):** новый эндпоинт `GET /api/v1/subject-memory` (Swagger-тег `subject-memory`, доступ owner/admin/coo, `CookieAuthGuard+TenantGuard`) — правила самообучения probe по `tenantId`, фильтры `status`/`kind`, агрегат `countsByStatus`. Контроллер `probe/subject-memory/subject-memory.controller.ts`, DTO `dto/subject-memory.dto.ts`, зарегистрирован в `probe.module.ts`. Фронт — страница «Что Кора выучила» `/company-admin/subject-memory` ([[frontend-pages]] §«Что Кора выучила»); диаг-команда `diag.ts subject-memory`. ТЗ [`2026-06-23-remove-manual-confirmations-master-tz`](../../plans/tz/2026-06-23-remove-manual-confirmations-master-tz.md) Блок D. См. [[probe-agent]] §«Наблюдаемость самообучения SubjectMemory».
- **2026-06-29 (Неделя компании — герой `/dashboard`):** новый `GET /api/v1/dashboard/operations/weekly-digest/latest` (последний недельный дайджест, owner/admin/coo/super). `weekly-per-person` += опц. query `weekEnd=` (окно пн–пт) и строка `rows[]` += `goalContributionNet` (net-вклад в главную цель из `PersonGoalContribution`). `weekly-digest/generate` — JSON-синтез (вердикт/письмо/недельный компас + детерминированный тренд по дням, persist в `WeeklyOperationsDigest`). Новый извлекающий agent/taskType НЕ вводился. См. [[director-dashboard]] §«Неделя компании», [[ai-jobs]] §«operations-weekly-digest».
- **2026-06-29 (День компании — герой `/dashboard`):** **новых эндпоинтов нет.** `POST /api/v1/dashboard/operations/daily-digest/generate` теперь делает JSON-синтез (вердикт дня `verdictJson` + письмо-проза `letterJson` + дневной компас `goalAlignmentDayJson`, json_schema strict, persist в `DailyOperationsDigest`). Расширены DTO-формы (без новых эндпоинтов): `getStuckCrossProject` (execution-dashboard) += `assigneeUserId`/`assigneeName`/`dueDate`; value-strip директора (director-dashboard) += `tasksResolved`/`ideasCollected`. Герой переиспользует daily-digest (3) + stuck/cross-project + director + insights/top + ideas/top. См. [[director-dashboard]] §«День компании», [[ai-jobs]] §«operations-daily-digest».
