...
> Корпус+эталон (493 сценария) УЖЕ ГОТОВЫ. Твоя работа — **написать скрипты стенда и прогнать его батчами
...
**Корпус+эталон УЖЕ ГОТОВЫ** (493 сценария, 1:1, покрытие карты 360°) — их НЕ надо генерить, только использовать
...
docs/testing/regulation-stand-corpus.json    # ГОТОВ (493 сценария)
docs/testing/regulation-stand-ruler.json     # ГОТОВ (493 эталона 1:1)
...
Фазы: `prepare` (тенант + Person/роли + конфиг) → `build` (посев блоков ПО ПОРЯДКУ → триггер специалиста →
`consolidateTenant`) → `annotate` (слепая разметка, кросс-проверка эталона) → `run` (read-only сверялка БД↔эталон) →
`judge` (панель 3 судей) → `report` (scorecard). Через `bun run scripts/regulation-stand/stand.ts <phase>`.

**НЕ гони все 493 разом. Идём батчами по ~50 со стоп-гейтом:**
