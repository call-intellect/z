        ...
        <div className="rounded-lg border border-border-subtle bg-bg-card p-5">
          {!selectedId ? (
            ...
            <article className="space-y-3">
              ...
              {activeTab === "decisions" && <DecisionsTab detail={detail} />}
              {activeTab === "handoffs" && <HandoffsTab detail={detail} />}
              {activeTab === "versions" && (
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-fg-secondary">
        ...
        `crossFunctionalScore ≥ порога` (детектор пересчитывается при изменении
        шаблона). Цифра справа — активные friction-отчёты.
      </p>
      <div className="mb-4">
        <Input
          value={q}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-[1fr_2fr]">
        <div>
          {items.length === 0 ? (
            <p className="text-sm text-fg-tertiary">
              Сквозных шаблонов пока нет. Они появятся автоматически, как только
              в шагах будут owner'ы из разных отделов.
            </p>
          ) : (
            <ul className="divide-y divide-border-subtle rounded-lg border border-border-subtle bg-bg-card">
              {items.map((t) => {
        ...
        <div className="rounded-lg border border-border-subtle bg-bg-card p-5">
          {!selectedId ? (
            <p className="text-sm text-fg-tertiary">
              Выберите сквозной шаблон слева, чтобы увидеть отчёты по слабым
              местам между командами.
            </p>
          ) : (
        ...
        >
          ...
          {dp.condition ? (
            <div className="mt-1 text-fg-secondary">
              Условие: {dp.condition}
            </div>
          ) : null}
          {dp.branches.length > 0 ? (
            <div className="mt-1 text-fg-tertiary">
              Ветви: {dp.branches.map((b) => b.name).join(" · ")}
            </div>
          ) : null}
        </li>
      ))}
  ...
  return (
    <div className="space-y-2 text-xs">
      ...
      <ul className="mt-2 list-disc space-y-1 pl-5 text-fg-secondary">
        ...
        <li>Источников (Idea-блоков): {detail.sourceBlockIds.length}</li>
        <li>
          Подтверждено:{" "}
      ...
      <p className="mt-2 text-fg-tertiary">
        Полнота пересчитывается ежедневно (cron) и при изменениях шаблона. Когда
        у шагов нет владельцев или артефактов — приходят probe-уведомления.
      </p>
    </div>
  );
}
