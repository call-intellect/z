...
export function PromptsListClient() {
  const [scopeFilter, setScopeFilter] = useState<"all" | PromptTemplateScope>(
    "all",
  );
  const [statusFilter, setStatusFilter] = useState<
    "all" | PromptTemplateStatus
  >("all");
  const [taskTypeFilter, setTaskTypeFilter] = useState<"all" | PromptTaskType>(
    "all",
  );
    ...
    <div className="mx-auto max-w-6xl">
      <header className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-fg-secondary">
            Конструктор промптов для AI-отчётов. Системные поставляются с Z,
            свои создаются под Org.
          </p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <Select
          value={scopeFilter}
          onValueChange={(v) =>
            setScopeFilter(v as "all" | PromptTemplateScope)
          }
        >
        ...
        <Select
          value={statusFilter}
          onValueChange={(v) =>
            setStatusFilter(v as "all" | PromptTemplateStatus)
          }
        >
      ...
      {!loading && !q.isForbidden && !error && items.length === 0 && (
        <div className="rounded-md border border-dashed border-border-subtle p-8 text-center text-sm text-fg-secondary">
          У вас нет шаблонов под выбранные фильтры. Скопируйте системный или
          создайте с нуля.
        </div>
      )}
