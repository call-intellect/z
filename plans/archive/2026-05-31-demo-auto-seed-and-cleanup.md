...
> ТЗ полностью заменено более новым 2026-06-01-demo-shared-org-model.md (который явно объявляет supersedes). Реализация этого ТЗ (per-user seed-копия + cleanup 35 таблиц при оплате) существовала меньше суток и была откачен
> ⚠️ Хвосты (см. реестр приоритетов): §11: Фаза 6 E2E на живом стенде — не выполнена локально, перенесена «на прод по smoke-чек-листу» · §11: Prometheus-метрики (§4.7) — не реализованы, помечены «опционально, отдельной задачей при необходимости»
