...
- **Шаг 12 — Ручная админ-операция (решение владельца, скрипта нет):** Z-Admin → LLM-провайдеры: у `minimax` сменить тип протокола на `anthropic-messages`, baseUrl на `https://api.minimax.io/anthropic` (API-ключ тот же). Маршруты: `block-ingest` primary → `minimax`/`MiniMax-M3`, secondary → `llm-kora-team`/`gemma4:e4b`. Причина: OpenAI-эндпоинт MiniMax не принуждает вывод к `json_schema` (валидирует схему, но модель отвечает своей структурой); Anthropic-путь идёт форс-tool'ом и отдаёт строгий JSON. На dev применено `scripts/_tmp_switch_block_ingest_route.ts` — можно переиспользовать как образец.
