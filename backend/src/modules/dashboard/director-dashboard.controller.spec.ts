...
describe('DirectorDashboardController (IDOR fence)', () => {
  ...
  it('cross-tenant: user из Org-A с X-Org-Id=Org-B и canView=false → 403', async () => {
    ...
    const q = DirectorDashboardQuerySchema.parse({});
    await expect(ctrl.director('org-B', buildReq('u-1') as never, q)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  ...
  it('BadRequest если X-Org-Id не передан', async () => {
    const { ctrl } = build();
    const q = DirectorDashboardQuerySchema.parse({});
    await expect(ctrl.director(undefined, buildReq('u-1') as never, q)).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it('403 no_user если req.user отсутствует (CookieAuthGuard сбоит)', async () => {
    const { ctrl } = build();
    const q = DirectorDashboardQuerySchema.parse({});
    await expect(ctrl.director('org-A', buildReq(undefined) as never, q)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
});
