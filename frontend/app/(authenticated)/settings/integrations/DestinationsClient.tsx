...
> = {
  ...
  slack_webhook: {
    label: "Slack",
    icon: MessagesSquare,
    description: "Входящий вебхук Slack",
  },
  telegram_bot: {
    label: "Telegram",
    icon: MessagesSquare,
  ...
  generic_webhook: {
    label: "Произвольный вебхук",
    icon: Globe,
    description: "Любой HTTPS-адрес",
  },
};

const TYPE_ORDER: DestinationType[] = [
  "email",
  "slack_webhook",
  "telegram_bot",
  "generic_webhook",
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        ...
        if (state.dest.type === "email" && email)
          cfg.recipient_email = email.trim();
        ...
        const body = buildCreatePayload(type, name.trim(), {
          email,
          url,
          botToken,
          chatId,
        ...
        <div className="space-y-4">
          {!isEdit && (
            <div className="space-y-1.5">
              ...
              <Select
                value={type}
                onValueChange={(v) => setType(v as DestinationType)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          {isEdit && (
            <div className="text-xs text-fg-tertiary">
              ...
              <Badge variant="secondary">
                {TYPE_META[state.dest.type].label}
              </Badge>
            </div>
          )}
          ...
          {(type === "slack_webhook" || type === "generic_webhook") && (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={
                  isEdit ? "Оставьте пустым, чтобы не менять" : "https://..."
                }
              />
              {isEdit && (
                <p className="text-xs text-fg-tertiary">
                  Текущий URL зашифрован и не показывается. Заполните для
                  замены.
                </p>
              )}
            </div>
...
function buildCreatePayload(
  ...
  if (type === "email") {
    ...
    return {
      type: "email",
      name,
      config: { recipient_email: values.email.trim() },
    };
  }
