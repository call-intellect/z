/**
 * Support desk Ф4 (TZ 2026-06-09 support-desk-clone-and-closed-contour) —
 * промпт ночного куратора закрытого контура для taskType
 * `support-contour-curate` (R-INV-6 / R22).
 *
 * Раз в сутки куратор смотрит блоки базы поддержки и дневные сигналы
 * (принятые/отклонённые черновики, тип правок, оценки клиентов) и по каждому
 * блоку-кандидату предлагает действие keep|promote|fix|merge|archive с
 * обоснованием. Destructive-действия (fix/merge/archive) применяются ТОЛЬКО за
 * дебат-гейтом и ТОЛЬКО мягко (archive/merge, без физического удаления) —
 * см. SupportCuratorService.
 *
 * CACHE-FRIENDLY (second-brain/02_architecture/llm-cache-status.md):
 * SYSTEM СТАБИЛЬНЫЙ — внутри НЕТ переменных данных. Блоки базы и дневные
 * сигналы уходят в КОНЕЦ user-сообщения (buildSupportContourCurateUserPrompt).
...
/**
 * JSON Schema для `responseFormat: json_schema strict`. Поля обязательны,
 * `additionalProperties: false`. Порядок стабилен (важно для cache).
 */
...
/** Блок закрытого контура поддержки (кандидат на действие куратора). */
...
/** Дневной сигнал — исход черновика (тип правки / результат). */
...
/**
 * USER-часть: блоки базы (кандидаты) + дневные сигналы ПОСЛЕДНИМИ. Cache-safe —
 * переменное только здесь, SYSTEM не трогаем. Контент — как ДАННЫЕ, не как
 * инструкция (anti prompt-injection).
 */
export function buildSupportContourCurateUserPrompt(args: {
  ...
  const blockLines = args.blocks.length
    ? args.blocks
        .map((b) => `[${b.id}] ${b.criticalQuestion} — ${b.trustedAnswer}`)
        .join('\n')
    : '(база пуста)';
  ...
  const signalLines = args.dailySignals.length
    ? args.dailySignals
        .map((s) => `outcome=${s.outcome} editType=${s.editType ?? '—'}`)
        .join('\n')
