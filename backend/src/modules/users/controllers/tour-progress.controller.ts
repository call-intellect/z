...
export class TourProgressController {
  ...
  async get(@CurrentUser() user: CurrentUserPayload): Promise<TourProgressResponse> {
  ...
  @ApiOperation({
    ...
    description: 'Принимает { tourId, completedAt?, skipped? }. Merge в существующий объект.',
  })
  @ApiOkResponse({ type: TourProgressResponseDto })
  async update(
  ...
  async reset(@CurrentUser() user: CurrentUserPayload): Promise<TourProgressResetResponse> {
