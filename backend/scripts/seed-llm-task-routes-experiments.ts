...
const SEEDS: TaskRouteSeed[] = [
  {
    ...
    playbookSection: '§2.1 block-distill (similar complexity, structured JSON) + β-6 sub-TZ §9',
    chain: [
  ...
  },
  {
    ...
    playbookSection: '§2.1 digest-task (multi-document summarization) + β-6 sub-TZ §9',
    chain: [
      { tier: 'primary', providerName: 'deepseek', model: 'deepseek-v4-flash' },
      {
        tier: 'secondary',
...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log(`=== seed-llm-task-routes-experiments START (updateExisting=${updateExisting}) ===`);
  // eslint-disable-next-line no-console
