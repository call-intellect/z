...
| Фаза 0а | наполнение/пере-сев + манифест | ✅ done | `docs/testing/clone-feed-manifest.json` | 2026-07-03/05 |
| Фаза 0б | Канал Б: 45 задач → probe → воронка (93% dispatch) | ✅ done | воронка в `channelB` | 2026-07-05 |
| Фаза 0в | Слой 0 фиделити построения | ✅ done | `clone-layer0-report.md` | 2026-07-05 |
| Фаза 1 | раннер run + трасса llmMeta (ось L) | ✅ done | `clone-stand-results.json` | 2026-07-05 |
| Фаза 2 | судьи (3 линзы) + report | ✅ done | `judge.ts`, `report.ts` | 2026-07-05 |
| Фаза 3 | BASELINE-прогон + отчёт владельцу | ✅ done | `docs/testing/clone-stand-report.md` | 2026-07-05 |
...
## Этап (2026-07-05) — весь недостающий стенд построен + BASELINE снят

### Постановка
Продолжить с места handoff (построение готово, 20 черт/5 персон): Фаза 0б (Канал Б) → Слой 0 → раннер →
судьи → **BASELINE**. Автономно до baseline; стоп на «решениях владельца» и перед push.

### Как делал (фаза за фазой)
- **Картография** — 2 Explore-агента: точные якоря `clones.service.ts` (4 ask-ветки + точки llmMeta + коды
  отказов) и цепочки Канала Б (respondToProbe / IssuesService / probe-гейты / Redis-ключи).
- **Фаза 0б Канал Б** (`seed-clone-feed.ts` + режимы `configure`/`channelB`): 45 задач (3 ответа × 15 методов) →
  probe `task.method_capture` → `respondToProbe`. Knobs (methodCaptureMinComplexity=0, semanticDedup/coldStart/
  fatigue off, rateLimit 1000) выставляются пред-boot (гейтинг идёт в контексте seed-скрипта).
- **Фаза 1 раннер** `stand.ts` (run|judge|report|all) + прод-правка: аддитивная трасса `llmMeta`
  (`usedBlockIds/usedSkillIds/usedRegulationNames/topicMatchedBlocks`, тип `string[]`) в 4 success-ветках.
- **Фаза 2 линейка** `types.ts`+`judge.ts`+`report.ts`: панель 3 линз (E+P/M/G, deepseek-v4-pro) + boundary,
  детерминированный вердикт §4.2, атрибуция по слою.
- **Фаза 0в Слой 0** `clone-seed-data.ts`+`layer0-annotate.ts`+`layer0-match.ts` (слепая разметка → сверялка).
- **Наполнение → build**: дренаж Канала Б (~104 клон-блока) → rebuild → **43 черты, 9 персон** (все 5 person +
  все 4 role; было 20/5).

### Что вышло — воронка Канала Б
`задач 45 → probe 42/45 задиспатчено (93%) → 0 drop-статусов → 42 ответа → клон-блоки (Сергей 29, Дарья 25,
Михаил 24, Игорь 17, Елена 9)`. Гейты не резали (knobs). Узкое место — ингест (LLM-насыщение), не dispatch.

### 🎯 BASELINE (чистый прогон, `docs/testing/clone-stand-report.md`)
| Метрика | Значение |
|---|---|
| EXPERT_PASS | **0/96 (0%)** |
| REFUSED | **96/96** отвечаемых (все `topic_starved`) |
| FABRICATED | **0** ✓ (инвариант держится) |
| BOUNDARY_OK | 5/14 (9 boundary-rude; 5/9 split — шум судьи на идентичном отказе) |
| Оси (на отвеченных) | E 0.00 · M 0.00 · G 1.00 · L 0.00 · P 0.00 |
| Атрибуция провалов (105) | Слой 2/3 = **103** (gate-refusal 94 + boundary-rude 9) · Слой 0 = 2 (versioning) |

**Единственный ответ клона на 100% вопросов:** «У оригинала недостаточно высказываний по этой теме, чтобы я мог
отвечать в его стиле без выдумывания. Спроси напрямую.»

### Диагноз (доказан кодом + данными, НЕ ruler-defect)
Клон **построен**, но ответный путь refusal-first:
1. `loadRoleSubgraph` = **последние-20-блоков-по-дате** (`orderBy createdAt desc take 20`), НЕ поиск по вопросу.
2. Анти-deepfake `assertTopicDensity` требует ≥2 из 20 с `cosineSim(вопрос,блок) ≥ 0.7`. Проверено: subgraph
   полон (19-20 блоков) → отказ из-за **cosine<0.7**, не пустоты. Компаундится header-less пространством (тот же
   сдвиг, что уронил порог склейки 0.72→0.60).

Прогноз был REFUSED ≈50, факт 96 → расхождение = компаундинг гейта + header-less, а не ошибка стенда (раннер
честно фиксирует реальные отказы клона; subgraph полон).

### Слой 0 (фиделити построения)
- Фиделити **ВЫСОКАЯ** (ручной спот-чер: у Дарьи 14 черт покрывают все 4 её метода; derivation 100%, persona
  100% на сматченных). Клоны верно ВЫВОДЯТ методы в черты (правило №1 — annotate agreement=full).
- **Авто-recall LLM-судьи 3/18 (17%) — НЕДОСЧИТ, не реальный промах.** Судья теряется при многих кандидатах
  (Дарья 14 черт → 0/4, хотя черты покрывают все методы). Это известная мягкость матчера (метод-док: «для строгой
  метрики нужна калибровка cosine + 2-из-3»). Первый прогон крашнул на пустом LLM-ответе; фикс — try/catch + ретрай
  ×5 с бэкоффом; но число всё равно недостоверно как строгая метрика.
- **no_false_merge FAIL (реален):** frozen-снимок Елены отсутствует — versioning отдал обе версии support Игорю
  (v2/active + v1/superseded, оба Игорь). Отсюда 2 Слой-0 провала в baseline (former_bearer frozen_v1 → 404).

### Косяки / находки
- **Квота (загрязнение первого baseline):** `aiChatQuota.dailyLimitAdmin` — крутилка через `getDynamic`
  (admin→ENV). Shell-override `AI_CHAT_DAILY_LIMIT_ADMIN=100000` и правка `.env` НЕ применились — источник
  AdminSetting=50. Первый прогон исчерпал квоту на c047 (c047-110 = `quota_exceeded`, невалидны). Фикс:
  `UPDATE AdminSetting=100000` + сброс Redis `quota:*:ai_chat_messages_per_day:*` → чистый перепрогон.
- **LLM-судья хрупок под нагрузкой** (Слой-0 match крашнул сразу после baseline+annotate — пустые ответы LLM).
- Канал Б: dispatch 93%, но ингест ~3-4 блока/мин (LLM-насыщение) — большие наполнения закладывать по времени.

### Артефакты
- Код: `clone-seed-data.ts`, `seed-clone-feed.ts`(+channelB), `stand.ts`, `types.ts`, `judge.ts`, `report.ts`,
  `layer0-annotate.ts`, `layer0-match.ts`; прод-правка `clones.service.ts` (llmMeta ×4). README `clone-stand.md`.
- Отчёты: `clone-stand-report.md`(+json), `clone-layer0-report.md`(+json), `clone-feed-manifest.json`,
  `clone-layer0-manifest.json`, `clone-stand-results.json`, `clone-stand-judged.json`, архив `clone-stand-runs/`.
- Верификация: typecheck 0 (clone-stand + прод), clones-тесты 49/49, lint 0.
- **Prod-операций НЕТ** (стендовые скрипты — не прод; llmMeta-правка аддитивна, миграция не нужна).

### Вывод для владельца (аналитик-разбор)
Baseline **доказал числом главное противоречие** мандата: клон построен (43 черты, 9 персон, фиделити высокая),
но политика ответа спроектирована ПРОТИВОПОЛОЖНО замыслу — **0% экспертных ответов, 100% отказ**, вся своя
фактура задушена анти-deepfake гейтом. **FABRICATED=0** держится тривиально (не отвечает → не выдумывает).
Рычаги ТЗ-1 «Разворот» (по диагнозам): **H1** — снять/переприцелить topic-density гейт (главный убийца:
question-search ретрив вместо последних-20 + калибровка порога 0.7 под пространство); **H3** — судьба
`CLONE_V2_ENABLED=false` (экспертный judgmental-режим заперт за флагом); **H5** — пороги `CLONE_TOPIC_*` →
AdminSetting. Плюс Слой-0 ветка: versioning Елены (frozen), motivation-слой=0, недопроизводство части методов
(Михаил logs/monitor/stage).

### Статус
✅ ТЗ-0 выполнено целиком (Фазы 0а–3). Baseline снят, диагнозы разложены по слою. Дальше — **решение владельца
по ТЗ-1** (разворот на лестницу опоры) + петля до планки.
