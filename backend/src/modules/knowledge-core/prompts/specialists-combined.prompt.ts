...
export function buildSpecialistsCombinedSystemPrompt(
  ...
  const body = [
    ...
    '- эксперимент с конкретным действием к исполнению («давайте проведём эксперимент: сделаем/настроим/запустим/замерим Y») → И experiments[] (гипотеза/результат/урок), И tasks[] (само действие Y). Абстрактный призыв «надо больше экспериментировать / давайте поэкспериментировать с ценами» без конкретного действия → только experiments[] низкой уверенности (или пропуск), tasks[] НЕ создавать',
    ...
    '- Действие внутри эксперимента = задача: «проведём эксперимент — сделаем/замерим Y» → task с title действия Y. Но абстрактный призыв экспериментировать без конкретного действия — НЕ задача (эксперимент как гипотеза при этом может остаться в experiments[]). Исполнитель не назван → assignee=null.',
    ...
    '- description — короткая структурная суть задачи своими словами: что нужно сделать, зачем/контекст, ключевые факты (срок, имена, суммы). 1–3 предложения, деловой русский, без слов-паразитов, приветствий, эмодзи, латиницы и кодов. НЕ копировать речь дословно и НЕ выдумывать факты, которых нет в реплике. sourceQuote остаётся дословной цитатой-опорой.',
    ...
    '',
    'ПРИМЕР 6 (эксперимент с действием → И experiments[], И tasks[]). Блоки: [гипотеза] «Проверим гипотезу: новый заголовок лендинга поднимет конверсию», [достигнутый результат/поручение] «Пётр настроит A/B-тест на неделю».',
    'ПЛОХО: только experiments[], действие «настроить A/B-тест» потеряно; ИЛИ только tasks[] без записи эксперимента.',
    'ХОРОШО: experiments — одна запись «A/B-тест заголовка лендинга» с гипотезой; tasks — одна запись title «Настроить A/B-тест заголовка на лендинге», assignee «Пётр», dueDate «неделя».',
    'ПРИМЕР 7 (абстрактный эксперимент → задачи НЕТ). Блок: «Нам бы вообще побольше экспериментировать с воронкой».',
    'ПЛОХО: завести task «экспериментировать с воронкой».',
    'ХОРОШО: tasks — пусто (конкретного действия нет); максимум experiments низкой уверенности.',
    '',
