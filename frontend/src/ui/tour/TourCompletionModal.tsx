...
/**
 * TourCompletionModal — финальный pop-up после завершения тура.
 *
 * После 'welcome': «Настройка готова!» → «Перейти на главную» → старт 'overview' через 1 сек.
 * После 'overview': «Готово, вы знаете кабинет» → «Перейти к работе».
 *
 * ТЗ 2026-05-29 onboarding-v2 §5.6 / §5.8.
 */
...
export function TourCompletionModal({ tourId, onDismiss }: Props) {
  ...
  const handleWelcomeComplete = useCallback(async () => {
    ...
    // POST setup/complete — отмечаем все 6 шагов пройденными.
    if (currentOrgId) {
      try {
        await onboardingApi.completeSetup(currentOrgId);
      } catch {
        // Не блокируем UX — setupCompletedAt проставится при следующем визите.
      }
    }
    // Через 1 сек стартуем overview-тур.
    setTimeout(() => {
    ...
    <Dialog open onOpenChange={(open) => { if (!open) onDismiss(); }}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="mt-4 flex justify-end">
          <Button onClick={isWelcome ? handleWelcomeComplete : handleOverviewComplete}>
