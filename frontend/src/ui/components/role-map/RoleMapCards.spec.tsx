/**
 * Render-тесты общего презентационного компонента карты должности
 * (`RoleMapGrid` + `isRoleMapEmpty`), который рендерят `/roles/[id]/map`,
 * `/roles/[id]` (overview) и `/me` (RoleProfileBlock).
 *
 *   - RoleMapGrid рендерит 6 категорий с элементами;
 *   - пустые категории показывают подсказку, но карточка присутствует;
 *   - isRoleMapEmpty: true когда все 6 counts=0, false когда есть хоть что-то.
 */
...
describe('RoleMapGrid', () => {
  it('рендерит элементы 6 категорий', () => {
    const map = baseMap({
      ...
      metrics: [
        { id: 'm1', name: 'CAC', unit: '₽', targetValue: 1000, currentValue: 1200 },
      ],
    });

    render(<RoleMapGrid map={map} />);

    // Заголовки всех 6 категорий присутствуют.
  ...
  it('пустые категории показывают подсказку, но карточки остаются', () => {
    ...
    expect(screen.getByText('Обязанности')).toBeInTheDocument();
    expect(
      screen.getByText(/Обязанности появятся/),
    ).toBeInTheDocument();
  });
});
