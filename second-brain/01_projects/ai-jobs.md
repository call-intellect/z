...
| **specialists combined (Фаза 6 §3, 2026-05-26; основной путь + канало-агностичен с захода A 2026-06-30)** | `knowledge-specialists-combined` | **DeepSeek V4 Pro** → OpenAI via proxy `gpt-5.4` → Ollama `qwen3.5:9b`. Б+ объединённый вызов: один LLM-запрос извлекает 8 типов сущностей (decisions / ideas / insights / experiments / regulations / knowledge_categories / skill_traits / helpfulness_traits) вместо 8 раздельных. ~3.7× дешевле при сопоставимом качестве (eval `backend/scripts/eval/judge-specialists-bplus-vs-g.ts`). Воркер — `SpecialistsCombinedWorker`, очередь `core.specialists-combined`. **С захода A (2026-06-30) — ВКЛ по дефолту, основной путь** (рубильник AdminSetting `knowledge.specialists_combined_enabled`, kill-switch ON; не A/B): **канало-агностичен** (chat/chatbox/Bitrix, не только встречи — контекст из `RawEvent`), воспроизводит 4 побочки (rebuild профиля/навыков, гигиена решений, ProcessTemplate через раздельный process-detector), питается few-shot реестром всех 57 типов + скелет-картой + нахлёстом окон/gleaning. См. §«Переписывание извлекающего слоя — заход A» ниже и [[../02_architecture/knowledge-core]] §«Переписывание извлекающего слоя — заход A». |
...
- **Заход B (отложен):** общий агент эмитит `tasks[]` (combo достаёт задачи/обещания из разобранного разговора → `TaskDraftMaterializerService`) — зависит от трекер-машинерии ТЗ задач (`task-extraction-pipeline-unification` Ф1–Ф4, ещё не реализованы). См. [[../04_не-сделано/README]].
