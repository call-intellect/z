/**
 * TZ-1 Фаза 5 (daily-value-engine) — минимальный структурный экспорт витрины
 * value-recap в «слайды» (заголовок + буллеты). Полноценный генератор
 * презентаций (PPTX/слайды-картинки) НЕ делаем — это отдельная инфраструктура
 * (см. реестр не-сделанного). Здесь — печатаемый/слайд-ready payload.
 *
 * Честность (Р6): только твёрдые данные из payload; soft-цифры со словом
 * «оценка» + знаменателем; count решений в паре с «% доведённых». Никаких ₽.
 */

import type { ValueRecapSlideDto } from '../dto/value-recap.dto';
import type { ValueRecapPayload } from '../services/value-recap.scoring';

/** Построить набор слайдов из payload витрины. Пустой при отсутствии payload. */
export function buildValueRecapSlides(
  ...
  // Слайд 1 — снятая рутина (ведущая ось).
  slides.push({
  ...
  // Слайд 2 — команда (soft, «оценка» + знаменатель).
  slides.push({
