...
async function scenarioS3(): Promise<S3Row[]> {
  ...
  console.log('Берём префикс N токенов + уникальный suffix (~50 токенов) в конце.');
...
async function main(): Promise<void> {
  ...
  if (maxCachedS1 > 5000) {
    ...
    console.log(`? Кэш ограничен ${maxCachedS1} токенами — гипотеза про потолок подтверждена.`);
  }
