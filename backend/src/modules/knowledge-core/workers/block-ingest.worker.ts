...
import { type EntityType, Prisma, type RawEvent, type SignalType } from '@prisma/client';
  ...
  private async process(job: Job<RawEventJobData>): Promise<void> {
    ...
    try {
      ...
      if (this.cfg.knowledgeCore.ideaDirectPathEnabled && ideaEmbeddingByBlockId.size > 0) {
        ...
        for (const it of existingIdeas)
          for (const bid of it.sourceBlockIds) coveredBlockIds.add(bid);
