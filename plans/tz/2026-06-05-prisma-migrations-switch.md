...
- [x] **Ф1b. Reconcile-миграция** — `20260605075900_reconcile_participant_invitation_status/migration.sql`: идемпотентно актуализирует существующие БД до схемы (чинит `Participant.invitationStatus` enum-тип + дотягивает Ф0-колонки; на свежей БД no-op). Только объекты schema.prisma — postgres-init.sql не трогается. **Имя — timestamp-префикс**, чтобы сортироваться ПОСЛЕ `0_init` (имя `0001_*` сортировалось бы ДО `0_init`, т.к. `_` > цифр → P3005).
