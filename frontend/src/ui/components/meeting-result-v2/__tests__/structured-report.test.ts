...
describe("structuredFieldLabel", () => {
  ...
  it("маппит новые ключи отчёта (B1.1)", () => {
    ...
    expect(structuredFieldLabel("competitors_mentioned")).toBe(
    ...
    expect(structuredFieldLabel("churn_risk_quote")).toBe(
      "Цитата риска оттока",
    );
    expect(structuredFieldLabel("recurring_problems")).toBe(
      "Повторяющиеся проблемы",
    );
    expect(structuredFieldLabel("unexplained_gaps")).toBe(
      "Без объяснённой причины",
    );
...
describe("structuredValueToMarkdown", () => {
  ...
  it("массив объектов → «- main — meta»", () => {
    expect(structuredValueToMarkdown([{ title: "A", assignee: "Иван" }])).toBe(
      "- A — Иван",
    );
  });
...
describe("structuredReportToPlainText", () => {
  it("убирает markdown-разметку (# и -)", () => {
    const txt = structuredReportToPlainText({ decisions: ["Б"] }, "Отчёт");
