...
export function ListClient({ slug }: { slug: string }) {
  ...
  if (!project) {
    return (
      <div className="text-sm text-fg-tertiary">Проект не найден.</div>
    );
  }

  return <IssueList issues={issues} group emptyText="В этом проекте пока нет задач" />;
}
