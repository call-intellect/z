...
const DEFAULT_STUCK_ANALYZING_MIN = 15;
...
export class ChatboxAnalyzeCron {
  ...
  async sweep(): Promise<void> {
    try {
      ...
      const stuckMin = await this.resolveStuckAnalyzingMin();
      ...
      const sessions = await this.prisma.chatboxChatSession.findMany({
        where: {
          ...
          OR: [
            ...
            { analysisStatus: 'analyzing', updatedAt: { lt: stuckBefore } },
          ],
        },
      ...
      try {
        ...
        stuckCount = await this.prisma.chatboxChatSession.count({
          where: { analysisStatus: 'analyzing', updatedAt: { lt: stuckBefore } },
        });
        this.metrics?.setChatboxStuckAnalyzing(stuckCount);
      } catch (gaugeErr) {
        this.logger.warn(
          { err: gaugeErr instanceof Error ? gaugeErr.message : String(gaugeErr) },
          'analyze-sweep: не удалось обновить gauge pending/stuck-сессий',
        );
      }

      this.logger.debug(
        `analyze-sweep: picked=${sessions.length} enqueued=${enqueued} stuck=${stuckCount}`,
      );
    } catch (err) {
      this.logger.error(
        { err: err instanceof Error ? err.message : String(err) },
  ...
  private async resolveStuckAnalyzingMin(): Promise<number> {
    try {
      const raw = await this.adminSettings.get<number>(
        'chatbox.analyze.stuckAnalyzingMin',
        DEFAULT_STUCK_ANALYZING_MIN,
      );
      const value = Number(raw);
      if (!Number.isFinite(value) || value < 1) return DEFAULT_STUCK_ANALYZING_MIN;
      return Math.floor(value);
    } catch {
      return DEFAULT_STUCK_ANALYZING_MIN;
    }
  }
}
