...
> **Это РУЧНОЙ разовый диагностический инструмент разработчика, НЕ часть прод-конвейера.** Он не крон,
> не воркер, не на горячем пути обработки встреч и сам по себе на проде НЕ запускается. Боевой выбор модели
> уже сделан фазой E (маршрут `meeting-extract-actions`/`decision-extract`/`idea-extract`/`insight-extract`
> → `deepseek-v4-pro`) — продакшен просто использует Pro, без всякого A/B в рантайме. Harness нужен лишь
> чтобы ВРУЧНУЮ, по запросу, перепроверить «Flash vs Pro» на реальном промпте, если в будущем встанет вопрос
> «а не вернуть ли Flash / не взять ли другую модель». Запускает его инженер осознанно (и для боевого
> deepseek-эндпоинта — с подтверждения владельца), а не автоматика.
