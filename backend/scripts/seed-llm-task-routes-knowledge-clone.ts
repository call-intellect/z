...
const SEEDS: TaskRouteSeed[] = [
  ...
  },
  {
    ...
    playbookSection:
      '§2.1 block-distill (same chain as extract; merge — JSON in, JSON out) + β-2 sub-TZ §10',
    chain: [
      { tier: 'primary', providerName: 'deepseek', model: 'deepseek-v4-flash' },
      {
        tier: 'secondary',
...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `=== seed-llm-task-routes-knowledge-clone START (updateExisting=${updateExisting}) ===`,
  );
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log('=== seed-llm-task-routes-knowledge-clone DONE ===');
}

main()
  .catch((err) => {
    // eslint-disable-next-line no-console
    console.error('seed-llm-task-routes-knowledge-clone FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
