...
/**
 * KieService unit-тесты: проверяем диспатч по префиксу модели (claude-/gpt-/gemini-),
 * парсинг трёх разных форматов ответа KIE, retry на 429/5xx, и graceful failure
 * на неизвестных префиксах / 4xx.
 *
 * Mocking — через `globalThis.fetch` (как в vox.service.spec.ts).
 * Retry-лестница в сервисе — [500, 1000, 2000]; для 429/5xx тестов используем
 * `vi.useFakeTimers()` + `vi.runAllTimersAsync()`, чтобы не блокировать suite.
 */
...
describe('KieService.complete — диспатч по префиксу модели', () => {
  ...
  it('gemini-* → Gemini direct-format (модель в URL, в body нет `model`)', async () => {
    ...
    // КРИТИЧНО: gemini direct — model в URL, в body model НЕ передаётся
...
describe('KieService.complete — retry/error handling', () => {
  ...
  it('HTTP 4xx (не 429) → LlmError сразу, без retry', async () => {
    ...
    await expectation;
    // ровно 1 вызов — 4xx не ретраится
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });
  ...
  it('HTTP 429 → 3 retry → 4 попытки → LlmError с httpStatus=429', async () => {
    ...
    // ловим reject заранее, чтобы fake-timers не зацикливались на pending promise
    ...
    await vi.runAllTimersAsync();
    await expectation;
    // retryDelaysMs = [500, 1000, 2000] → 1 первичный вызов + 3 retry = 4
