...
**В периметре (5 механизмов + прямые зависимости):** извлечение задач из разговоров/встреч/чата →
...
переходы статуса, method-capture, reconcile; **качество ТЕКСТА задачи/подзадач** (заголовок/описание —
структурный текст vs дословная речь — механизм 5, §5а).
...
кастом-поля, вебхуки, импорт Яндекс/Битрикс. Упоминаются там, где пересекаются с 5 механизмами.
...
## 5а. Механизм 5 — КАЧЕСТВО ТЕКСТА задачи (заголовок / описание / подзадачи)

> Добавлено 2026-07-04 по находке владельца на проде (задача MTG-14, проект «Из встреч»): заголовок
> переформулирован чисто, а **описание = дословная расшифровка речи** постановщика («Хорошо, ладно, это мы
> проверим, чё, ну, как-как передаётся…»). Разведка кода подтвердила системно.

### 5а.1 Что становится ОПИСАНИЕМ (сырьё, не переформулировка) — Т11
| Путь | Описание ← | Сырьё? | file:line |
|---|---|---|---|
| **Живой** SpecialistsCombined → materialize | `extractedDescription = sourceQuote` | **сырая цитата** | `task-draft-materializer.service.ts:185` |
| Combined-схема извлекателя | поля `description` **НЕТ** (только `title` + `sourceQuote`) | — | `specialists-combined.prompt.ts:220-234` |
| Легаси per-block | `extractedDescription = draft.sourceQuote` | **сырая цитата** | `specialist-3-15-tasks.service.ts:291` |
| Встреча next-step | `extractedDescription = args.description?.trim()` (обычно null); сырой next-step → `extractedTitle`/`rawContent` | — | `intake.service.ts:417-419` |
| Материализация intake→Issue | `description = override ?? extractedDescription ?? rawContent` → `Issue.description` И `descriptionStripped` | **проносит сырьё** | `intake.service.ts:692-701` |
| Мёртвый `TaskExtractionService` | схема ИМЕЕТ отдельное `description` (`tasks-structured.ts:11`) | но **путь не подключён** | `ai.module.ts` (нет вызовов) |

**Итог:** оба живых извлекателя кладут в описание `sourceQuote` (дословную речь) → `Issue.descriptionStripped`
= сырьё. Это и есть MTG-14.

### 5а.2 Заголовок и подзадачи
- **Заголовок — чистый везде** (императив, глагол+объект): combined `specialists-combined.prompt.ts:614`,
  легаси `task-extract.prompt.ts:32`. Проблема **только в описании**.
- **Подзадачи/чек-лист:** LLM выдаёт короткие переформулированные шаги (`subtasks:[{title}]`,
  `specialists-combined.prompt.ts:615,231`) → `checklistJson` → `IssueChecklistItem.text` как есть
  (`intake-checklist-materialize.util.ts:22`). Отдельного шага чистки НЕТ — качество на совести LLM.

### 5а.3 Провенанс (куда уходит сырая цитата — не потерять при фиксе)
- `IntakeIssue.rawContent` = `` `${title}\n\nЦитата: ${sourceQuote}` `` (`task-draft-materializer.service.ts:172`) — **сырьё всегда тут**.
- `TaskSource.quote` (легаси-путь линковки, `specialist-3-15-tasks.service.ts:246`).
- `Issue.previewQuote` (`schema.prisma:9642`) — на пути intake→Issue **пустует (null)** → свободный «дом» для дословной цитаты.
- Готовый образец промпта-переформулятора (чистый человеческий текст из сырья) — `issue-progress-draft.prompt.ts:33-52`.

### 5а.4 Замысел фикса (Т11) — детально в [ТЗ](../tz/2026-07-04-task-text-reformulation.md)
Добавить структурное `description` **в сам извлекатель** (переиспользуя идущий LLM-вызов, без доп. стоимости):
`description` рядом с `title` в combined-схеме/тулзе/промпте → map в сервисе → `materialize`:
`extractedDescription = draft.description ?? sourceQuote`. `sourceQuote` остаётся провенансом (`rawContent`,
опц. `Issue.previewQuote`). Downstream (`intake.triage`) не трогать — чистое описание поедет на Issue само.
Требование к тексту: короткая структурная суть (что/зачем/контекст) без слов-паразитов, факты сохранены, без
выдумок (антигаллюцинация).
...
10. **Текст задачи — структурный, не сырой** (Т11): заголовок И описание — короткая формулировка по сути
    (что/зачем/контекст), без слов-паразитов/расшаркиваний/эмодзи; подзадачи — чистые шаги; факты (срок/имя/
    сумма) сохранены, ничего не выдумано; дословная речь остаётся в провенансе.
...
| **Т11** | **Описание задачи = дословная речь** (`extractedDescription=sourceQuote`), а не структурный текст; подзадачи — на совести LLM без чистки. Кейс MTG-14. | §5а | Baseline: `Issue.descriptionStripped` = сырая реплика со словами-паразитами. **Фикс:** [ТЗ переформулирования](../tz/2026-07-04-task-text-reformulation.md). |
...
сливается/не закрывается). Одиннадцать гипотез Т1–Т11 — приоритетные цели стенда; банк сценариев (§файл
