...
function computeRoleScore(args: {
  ...
  const score = args.completeness * 0.4 + cardScore * 0.3 + args.probeClosedRatio * 0.3;
  return Math.max(0, Math.min(1, score));
}

describe('MaturityScorerService — формула Role', () => {
  it('даёт 0 при нулевых входных', () => {
    expect(computeRoleScore({ completeness: 0, cardCount: 0, probeClosedRatio: 0 })).toBe(0);
  });
  ...
  it('взвешивает completeness c коэффициентом 0.4', () => {
    expect(computeRoleScore({ completeness: 0.5, cardCount: 0, probeClosedRatio: 0 })).toBeCloseTo(
      0.2,
      6,
    );
  });
  ...
  it('взвешивает probe c коэффициентом 0.3', () => {
    expect(computeRoleScore({ completeness: 0, cardCount: 0, probeClosedRatio: 0.5 })).toBeCloseTo(
      0.15,
      6,
    );
  });
