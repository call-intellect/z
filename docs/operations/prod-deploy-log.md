...
- **Прод-действие:** `docker compose up -d --build backend` + rebuild frontend, **затем** `docker compose exec backend bun run scripts/apply-prod-deploy.ts --mode update` (доставит сид потолка 5/сутки). Миграций/patch/backfill нет.
