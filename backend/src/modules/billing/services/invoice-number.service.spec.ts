/**
 * Unit-тесты InvoiceNumberService — формат `Z-YYYY-NNNNNN`.
 */
...
describe('InvoiceNumberService.format', () => {
  ...
  it('padding до 6 знаков', () => {
    ...
    expect(svc.format(123_456, new Date('2026-01-01T00:00:00Z'))).toBe(
      'Z-2026-123456',
    );
  });

  it('billingNumber >= 1_000_000 — естественное расширение padding', () => {
    expect(svc.format(1_234_567, new Date('2026-01-01T00:00:00Z'))).toBe(
      'Z-2026-1234567',
    );
  });

  it('берёт UTC-год (а не локальный)', () => {
    // Москва +03 — 31 декабря 23:00 UTC = 1 января 02:00 МСК.
    // Если бы брали локальный год — было бы 2027; берём UTC → 2026.
    expect(svc.format(7, new Date('2026-12-31T23:00:00Z'))).toBe('Z-2026-000007');
    // На границе UTC: 1 января 00:00 UTC → уже 2027.
