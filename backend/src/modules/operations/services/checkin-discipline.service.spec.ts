...
/**
 * ТЗ Ф8.7 (cabinet-redesign-rhythms) — unit-тесты `getCheckinDiscipline`.
 *
 * Мокаем `prisma.dailyCheckIn.groupBy` (expected — все строки, completed —
 * только `completedAt != null`) и `prisma.person.findMany` (имена). Сети и
 * времени нет — детерминированно. Покрываем:
 *   (а) обычные данные утро/вечер сдано/пропущено → totals + byPerson;
 *   (б) нет данных → нули, completionRate=null;
 *   (в) флаг DAILY_CHECKIN_ENABLED=false → enabled:false, нулевые totals.
 */
...
function buildSvc(opts: {
  ...
  // groupBy вызывается дважды: 1-й — expected (все), 2-й — completed.
  const groupBy = vi
    .fn()
...
describe('OperationsDashboardService.getCheckinDiscipline', () => {
  it('(а) обычные данные: утро/вечер сдано/пропущено → totals + byPerson', async () => {
    const { svc } = buildSvc({
      enabled: true,
      // Анна: утро 5 ожидаемо / 4 сдано, вечер 5 / 3.
      // Борис: утро 5 / 5, вечер 5 / 0.
      expected: [
        g('p-anna', 'morning', 5),
        g('p-anna', 'evening', 5),
        g('p-boris', 'morning', 5),
    ...
    // Totals: утро 10 ожид / 9 сдано / 1 пропуск; вечер 10 / 3 / 7.
  ...
  it('(б) нет данных → totals нули, completionRate=null, byPerson пуст', async () => {
    ...
    // Без personId — findMany не дёргаем.
  ...
  it('(в) флаг DAILY_CHECKIN_ENABLED=false → enabled:false, нулевые totals, groupBy не вызван', async () => {
    ...
    expect(dto.totals.morningExpected).toBe(0);
    expect(dto.totals.eveningExpected).toBe(0);
    // Выключенная фича не должна ходить в БД.
  ...
  it('детерминизм: при отсутствии имени Person → «Без имени», порядок по expected desc', async () => {
    const { svc } = buildSvc({
      ...
      // p-low отсутствует в persons → имя fallback.
    ...
    // Сортировка: больше expected — выше.
