...
function RegulationsListContent() {
  ...
  const [kindFilter, setKindFilter] = useState<"all" | RegulationKindApi>(
    initialKind,
  );
  const [statusFilter, setStatusFilter] = useState<"all" | RegulationStatusApi>(
    "all",
  );
  ...
  const handleSupersede = useCallback(async () => {
    ...
    try {
      ...
      if (e instanceof ApiError && e.code === "forbidden") {
        ...
        toast.error(humanizeApiError(e, "Не удалось заменить запись"));
      }
      throw e;
    }
  ...
  const toggleHistory = useCallback(async () => {
    ...
    try {
      ...
      toast.error(humanizeApiError(e, "Не удалось загрузить историю"));
    } finally {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-6 py-8">
      <header className="mb-4">
        <h1 className="text-2xl font-semibold">Правила, процессы и политики</h1>
                  ...
                  <header className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {detail.kind === "policy" && detail.severity ? (
                        <Chip
                          variant={SEVERITY_CHIP[detail.severity]}
                          size="sm"
                        >
