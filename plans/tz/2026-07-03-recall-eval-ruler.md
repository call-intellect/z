...
- `batch-recall-trace.ts`: цепочки прогоняются последовательно с накоплением истории **независимо от
  `LIMIT`** (убрать гейт `LIMIT===∞` на строке 167); одиночки — как есть.
- Валидатор банка отклоняет follow-up-вопрос (эллипсис/местоимение как первый токен) без `chain`.
- Приёмка: unit на разбор банка (chain из 2 turn → 2 записи с нарастающей историей; одиночный follow-up
  без chain → ошибка валидатора). Полный прогон цепочек — «под стендом позже».
