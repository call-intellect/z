...
/**
 * `OperationsTabs` — горизонтальные pill-табы для трёх страниц
 * операционного раздела:
 *   - `/dashboard/operations` — «Обзор»
 *   - `/dashboard/operations/daily` — «Сегодня»
 *   - `/dashboard/operations/weekly` — «Неделя»
 *
 * Фаза 5 ТЗ `plans/tz/2026-06-01-dashboards-wow-polish.md`. Стиль скопирован
 * с `PersonSubpagesNav` (парные токены, `bg-accent/15 + text-accent-fg`
 * для активного таба). Активный определяется через `usePathname()` по
 * точному совпадению либо по префиксу `/.../`.
 */
type TabItem = {
  href: string;
  label: string;
...
export function OperationsTabs() {
  ...
  return (
    ...
    >
      {ITEMS.map((item) => {
        // Корень `/dashboard/operations` подсвечиваем только когда мы ровно
        // на нём — иначе он перетянет активность у daily/weekly.
