...
import { BlockIngestWorker, classifyTypedFailReason } from './block-ingest.worker';

describe('classifyTypedFailReason — Ф5', () => {
  it('cypher does not exist → age_unavailable', () => {
    expect(
      classifyTypedFailReason(new Error('function cypher(unknown, unknown) does not exist')),
    ).toBe('age_unavailable');
  });

  it('код 42883 в сообщении → age_unavailable', () => {
    expect(classifyTypedFailReason(new Error('SQL error 42883'))).toBe('age_unavailable');
  });

  it('упоминание z_graph → age_unavailable', () => {
    expect(classifyTypedFailReason(new Error("graph 'z_graph' is not loaded"))).toBe(
      'age_unavailable',
    );
  });

  it('ag_catalog в сообщении → age_unavailable', () => {
    expect(classifyTypedFailReason(new Error('schema ag_catalog not in search_path'))).toBe(
      'age_unavailable',
    );
  });
  ...
  it('unique constraint в тексте → idempotent_skip', () => {
    expect(classifyTypedFailReason(new Error('duplicate key — unique constraint'))).toBe(
      'idempotent_skip',
    );
  });
  ...
  it('ValidationError (по имени класса) → validation_error', () => {
    ...
    expect(classifyTypedFailReason(new ValidationError('bad shape'))).toBe('validation_error');
  });

  it('сообщение про required → validation_error', () => {
    expect(classifyTypedFailReason(new Error('data.name is required'))).toBe('validation_error');
  });

  it('прочая ошибка → other', () => {
    expect(classifyTypedFailReason(new Error('connection reset by peer'))).toBe('other');
  });
});
...
describe('BlockIngestWorker.warnTypedFail — Ф5 классификация + метрика', () => {
  ...
  it('нормализует type-метку «decision (fallback)» → «decision» в метрике', () => {
    const { worker, metrics } = buildWorkerWithMetrics();
    (worker as any).warnTypedFail('decision (fallback)', 'текст', new Error('boom'));
    expect(metrics.incTypedEntityFailed).toHaveBeenCalledWith({
      type: 'decision',
      reason: 'other',
    });
