...
> 4. Новые эндпоинты `GET /me/income-chart` (12 точек), `GET /me/funnel?period=30d|90d|all`, `POST /me/promo-event` (метрики `ReferralPromoStrip`).
> 5. Промо-стрип `<ReferralPromoStrip />` в `AppShell` — мягкий призыв на whitelisted страницах (`/dashboard`, `/goals`, `/clones`, ...), копи разная для owner и member, dismiss на 30 дней.
...
1. **Партнёр создаёт реферальный профиль** в один клик — на странице `/referrals` отмечает чекбокс «соглашаюсь с офертой партнёрской программы» и жмёт «Создать ссылку». Backend требует только `contractAccepted: true` (Zod `literal(true)`), ИНН/`legalForm`/реквизиты на этом шаге опциональны. Сразу фиксируется `contractAcceptedAt = now()` и генерируется slug (8 символов без 0/o/l/1).
2. **Партнёр (опц.) добавляет реквизиты для вывода** — на той же странице (`PayoutDetailsCard`) заполняет ИНН, форму ведения деятельности (самозанятый / ИП / юрлицо), банковские реквизиты в `payoutDetails`. Без них программа работает (ссылка активна, клики считаются, payout'ы создаются как `pending`), но фактический вывод денег закроется только после заполнения.
3. **Партнёр верифицирует ИНН** через `InnLookupService` — это нужно для будущей выплаты. Без верифицированного ИНН **И** заполненных `payoutDetails` cron 10-го числа НЕ переведёт payout в `paid` (см. `ReferralPayoutCron`).
...
|---|---|---|---|---|---|---|
| 1 | Создание профиля в один клик | `POST /referrals/me`: `ReferralsService.create({ ownerUserId, contractAccepted, inn?, legalForm?, payoutDetails? })`; обязателен только `contractAccepted=true` (Zod `literal(true)` + сервисный guard), реквизиты опц.; slug через `customAlphabet('abcdefghijkmnpqrstuvwxyz23456789', 8)` (nanoid) — символы без 0/o/l/1 для устной передачи; уникальность ownerUserId через `@unique`. `contractAcceptedAt = now()` ставится сразу. | `backend/src/modules/referrals/services/referrals.service.ts:150..184` (`create`), `backend/src/modules/referrals/controllers/referrals.controller.ts:111..133` (`POST /me`), `dto/referrals.dto.ts:55..64` (`CreateReferralBodySchema`) | `POST /api/v1/referrals/me` | `Referral(slug, contractAcceptedAt, opt inn, opt legalForm, opt payoutDetails)` | ✅ |
...
| 3 | Принятие оферты (legacy) | `POST /referrals/me/accept-contract`: ставит `Referral.contractAcceptedAt = now` идемпотентно. В новом флоу (Волна 2 / ТЗ referrals-cabinet-revamp) оферта принимается при `create` через `contractAccepted=true`. Эндпоинт оставлен для обратной совместимости с профилями, у которых `contractAcceptedAt == null`. Без `innVerifiedAt && contractAcceptedAt && payoutDetails!=null` cron 10-го числа НЕ переведёт payout в `paid` (см. шаг 9). | `backend/src/modules/referrals/services/referrals.service.ts:300..312` (`acceptContract`), `referrals.controller.ts` | `POST /api/v1/referrals/me/accept-contract` | `Referral.contractAcceptedAt` | ✅ legacy |
