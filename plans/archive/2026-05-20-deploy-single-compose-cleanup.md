...
> Реализовано целиком и подтверждено фактическим кодом: единый root docker-compose.yml без профилей, порты через .env, dev-compose, медиа отдельным compose, 3 nginx-конфига, удалён deploy/backend|frontend, удалён мёртвый к
