...
const SEEDS: TaskRouteSeed[] = [
  {
    taskType: 'operations-monthly-digest',
    playbookSection:
      'Месяц компании — свод 4 недель в месячный брифинг, 1 вызов/месяц/Org, capable модель.',
    chain: [
...
async function main(): Promise<void> {
  ...
  console.log(`=== seed-llm-task-routes-month-company START (updateExisting=${updateExisting}) ===`);
  ...
  console.log('=== seed-llm-task-routes-month-company DONE ===');
}

main()
  .catch((err) => {
     
    console.error('seed-llm-task-routes-month-company FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
