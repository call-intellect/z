...
## Регламенты должности в клоне роли (Способ C, 2026-06-23)

**Источник:** ТЗ [`plans/tz/2026-06-22-clone-regulation-grounding-method-c.md`](../../plans/tz/2026-06-22-clone-regulation-grounding-method-c.md) (3 фазы, ветка `feature/2026-06-22-clone-regulation-grounding-method-c`). Схема (`ExecutablePersona.applicableRegulationsSnapshot` + индекс `Instruction(tenantId, scope)`) — [[../02_architecture/data-model]] §«Указатель регламентов в клоне роли»; новый сервис — [[../02_architecture/module-map]] §«RoleRegulationRetrievalService»; крутилки — [`docs/operations/feature-flags.md`].

**Проблема (до):** клон роли опирался ТОЛЬКО на неявный опыт носителей (`SkillTrait`/`RolePrinciple`/`PracticeSkill`). Записанные **регламенты/инструкции/политики/процессы** должности — карточки `Regulation`/`Instruction`/`Policy`/`Process` из «Базы знаний» — в ответ клона НЕ попадали (`buildForRole` их не читал). Две разъединённые подсистемы.

**Способ C — связка через retrieval + указатель-снапшот:**
- **Правило важнее привычки.** При ответе (`askRole`/`askRoleV2`, person-scope НЕ затронут) `RoleRegulationRetrievalService.retrieveForRole` подтягивает по смыслу вопроса (`embedQuery` → cosine `<=>` по 4 таблицам, порог + topN) применимые к должности правила и кладёт их в **отдельный приоритетный блок `<applicable_regulations>`** промпта `clone-respond`. Ранжирование: `Policy(blocking)` → `Policy(mandatory)` → `Regulation`/`Process`/`Instruction` (обязательный контекст) → `Policy(advisory)` → личный опыт persona/traits. `severity` есть только у `Policy`; для остальных дефолт — «записанное правило весомее привычки, но не перебивает явный блокирующий запрет».
- **Указатель-снапшот.** `buildForRole` при сборке клона заполняет `ExecutablePersona.applicableRegulationsSnapshot` (`[{kind,id,name,severity?,scope}]`, без полного текста, топ по `lastConfirmedAt`/`updatedAt`) — он рендерится компактным перечнем заголовков `<regulations_index>`: «клон знает, что у него есть N правил для сверки». Это дешёвый фундамент под будущего проверяльщика (proactive/postfact — отдельный задел [`plans/analysis/2026-06-22-regulation-checker-proactive-and-postfact.md`](../../plans/analysis/2026-06-22-regulation-checker-proactive-and-postfact.md)).
- **Иерархия видимости (scope).** Правило применимо к клону, если его `scope ∈ {role:<id>, org, department:<id>}` (departmentId резолвится по `Role.departmentId`; снапшот Ф2 — role-only, иерархия Ф3 — только в retrieval). Точный строковый фильтр `scope = 'role:<id>'` (exact, не `contains`) через хелпер `parseRoleScope`. Нормализация `scope`→FK на `Role` сознательно НЕ сделана (задел, реестр не-сделанного).
- **prompt-cache сохранён.** Оба блока — в КОНЦЕ переменной user-части `CLONE_RESPOND_USER_TEMPLATE` (после `practiceSkills`); стабильный SYSTEM `buildCloneRespondSystemPrompt` НЕ тронут. Правил нет → блок не рендерится (клон отвечает как прежде).

**Крутилки** (AdminSetting, группа «Регламенты клона» в `KnowledgeCoreSettingsClient`, не ENV/код): `clone.regulations.retrieval.top_n` (6), `clone.regulations.retrieval.min_similarity` (0.30 cosine-distance), `clone.regulations.snapshot.max_items` (20), `clone.regulations.scope.include_org` (true). Сид `seed-admin-setting-clone-regulations.ts`.
