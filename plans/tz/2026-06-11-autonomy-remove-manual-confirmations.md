...
## Фаза 1 (W1) — LLM-арбитр конфликтов `[ ]`
...
## Фаза 2 (W2) — probe: гейт ценности + «лестница владельца» + NUDGE→дайджест `[ ]`
...
## Фаза 3 (W3) — curation: пороги, аудит-выборка, AI-судья `[ ]`
...
## Фаза 4 (W4) — intake: авто-приём задач из всех каналов `[ ]`
...
Не начато. Один документ, 5 фаз (W0→W4), порядок = порядок реализации. Каждая фаза самостоятельна и оставляет систему рабочей. Код-почва здесь; **тексты промптов (арбитр, судья, специалисты, probe-formulate, триаж) правим отдельным шагом «улучшаем промпты» вместе** — в фазах они первой редакцией. Политика «где человек / где агент сам» — в анализе §9.
