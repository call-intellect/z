...
- `/chat-v2`, `/assistant` — **redirect → `/chat` (2026-06-25, «Единый помощник» Ф1/Ф5).** Орфаны-входы в chat-v2/assistant убраны: единственный вход к ответам — Мастер на `/chat`. `/chat-v2` сохраняет `?conversationId=` при редиректе (`chat-v2/page.tsx`); `ChatV2Client.tsx`/`AssistantClient.tsx` пока в дереве, но не маршрутизируются (полное схлопывание 3 движков отколото в [`2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md`](../../plans/tz/2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md), нужна визуальная приёмка). Селектор «помощник / клон должности» (2026-06-15) живёт в панели Мастера (слим-вкладки «Помощник компании | Клоны ролей»).
- `/memory` — **дверь к реестрам (2026-06-25), НЕ кусочный поиск.** Кусочный `MemorySearch` (искал по памяти без агента и вываливал обрывки-чанки) **удалён**; `memory/page.tsx` — хаб-сетка ссылок на реестры (Решения / Оцифровано / Темы / Сущности / Таблицы / Идеи / Сигналы) + подсказка «Спросите у Мастера». Глупого «Спросить у Коры с кусками» в интерфейсе больше нет.
