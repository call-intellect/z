/**
 * Ф2 / Б3 (2026-06-05) — честное состояние «бот не настроен» на /me/channels.
 *
 * `mapTelegramChannelEntry` должен ПЕРВЫМ делом смотреть на
 * `channel.configured`: если глобальный Telegram-бот без токена
 * (`configured===false`), статус = `channel_not_configured`, чтобы UI не
 * вёл в тупик. `botUsername` пробрасывается во view для deep-link.
 */
