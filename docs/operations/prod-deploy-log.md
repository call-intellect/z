...
- **Шаг 6 — Patch (маршрут LLM compile → deepseek-v4-pro):** в `patch-mass-migrate-to-deepseek-pro.ts` TARGETS добавлен `executable-persona-compile` — патч уже в STEPS (`phase:'patch'`, `args:['--update-existing']`, `everyDeploy`), поэтому на каждом деплое переводит primary сборки клона `deepseek-v4-flash`→`deepseek-v4-pro` (уважая admin-правки). Причина: flash спотыкался на structured/thinking (`Thinking mode does not support tool_choice`) → иногда обнулял/деградировал клон при сборке. ⚠ Сам `seed-llm-task-routes-skill-and-clone.ts` (канонический источник, primary→pro + secondary→gpt-5.4) в аггрегатор НЕ входит — прод получает изменение через этот patch.
