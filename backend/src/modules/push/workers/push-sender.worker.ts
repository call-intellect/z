...
import { CORE_QUEUE_NAMES, type PushSendJobData } from '../../core-queue/queues';
...
export class PushSenderWorker implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    this.worker = new Worker<PushSendJobData>(
      ...
      async (job) =>
        this.pipe.job(SystemLogPipeline.NOTIFICATIONS, 'push.sender', job, () => this.process(job)),
      {
    ...
    this.worker.on('completed', (job) => {
      this.logger.debug({ jobId: job.id, userId: job.data.userId }, 'push-send: job completed');
    });

    this.logger.debug(`PushSenderWorker запущен (${CORE_QUEUE_NAMES.PUSH_SEND}, concurrency=5)`);
  }
  ...
  async process(job: Job<PushSendJobData>): Promise<void> {
    ...
    const url =
      data && typeof data === 'object' && typeof data.url === 'string' ? data.url : undefined;
