...
| 10 | Авто-advance статуса по трекеру | `IdeaStatusAutoAdvanceService` (`@OnEvent('tracker.event_occurred')`, фильтр `type='issue.status_changed_to_done'`): при закрытии задачи с `goalId` продвигает статус идей той же цели (не `shipped/rejected/archived`) через `nextIdeaStatusOnTaskClose`. Гейт `ideas.feed.enabled` | `backend/src/modules/knowledge-core/services/idea-status-auto-advance.service.ts:28` | `EventEmitter` `tracker.event_occurred` | `Idea.status` | ✅ |
| 11 | Авто-связка идея→решение | `Specialist36Service.reconcileIdeaForDecision` / `markRealizedByDecision` — вызывается из `specialist-3-3-decisions.service.ts:500` при обработке решения: KNN-поиск подходящей идеи и пометка «реализована решением» | `backend/src/modules/knowledge-core/services/specialist-3-6-ideas.service.ts:429,464` | из pipeline решений (3-3) | `Idea` | ✅ |
