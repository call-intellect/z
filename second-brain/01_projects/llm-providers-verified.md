...
- **Budget guard (pre-dispatch).** Новый `BudgetGuardService` ([ai/services/budget-guard.service.ts](../../backend/src/modules/ai/services/budget-guard.service.ts)) вызывается из `LlmRouter.call` ПЕРЕД dispatch (после dataClass-фильтра). Оценивает MTD-расход тенанта: сумма `AiUsageLog.costRub` за UTC-месяц (один источник-колонка — согласованность с алертом), in-memory-кэш TTL `llm.budget.mtd_cache_ttl_sec` (code-fallback 60с). Блокирует (`LlmBudgetExceededError`) **только** при флаге `llm.budget.enforce_enabled` (AdminSetting, **дефолт OFF**) И `OrgBudgetCap.capKind='hard'` И MTD≥cap; иначе **observe** — метрика `llm_budget_exceeded_total{mode}` (`observe`|`enforce`|`downgrade`) + WARN «would block», dispatch продолжается. **Best-effort fail-open**: любая ошибка в guard (включая недоступность FX) НЕ роняет LLM-вызов — ложная блокировка хуже редкого пропуска. `tenantId=null` (system-jobs) не лимитируется. **Дефолт OFF = ноль изменений поведения** до явного включения владельцем (развилка Р-1 ТЗ: block/degrade/alert — путь A построен за флагом, решение перед включением).
- **Третий режим `capKind='downgrade'` (2026-07-04, ТЗ [`llm-budget-downgrade-tier`](../../plans/tz/2026-07-04-llm-budget-downgrade-tier.md)).** Закрывает развилку Р-1 выше (вариант «degrade»): при `over` и `capKind='downgrade'` `LlmRouterService.call()` НЕ бросает `LlmBudgetExceededError` и НЕ просто наблюдает — переупорядочивает уже отфильтрованную по dataClass цепочку `filtered` (тот же список моделей для taskType, что и tier-fallback-на-ошибках) по возрастанию цены (`MODEL_PRICES[model].inputPer1M+outputPer1M`), запуская dispatch с самой дешёвой; первая попытка получает `fallbackReason='budget_downgrade'` в `AiUsageLog` (отличает от обычного `null`/error-fallback). Этот режим НЕ гейтится `llm.budget.enforce_enabled` (тот флаг гейтит только `hard`) — активен сразу, как только у Org явно выбран `downgrade`. Плюс — платформенный дефолт-лимит `llm.budget.default_monthly_cap_rub` (AdminSetting, 0=безлимит): подставляет ТОЛЬКО сумму для Org без своей строки, режим (`capKind`) остаётся `'soft'`/тем что уже есть — сам по себе ничего не блокирует. `BudgetAlertCron` расширен на обход всех активных Org (не только с явной строкой) — иначе платформенный дефолт никогда бы не долетал уведомлением.
