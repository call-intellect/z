...
export class AdminBotsController {
  constructor(@Inject(AdminBotsService) private readonly svc: AdminBotsService) {}
  ...
  setTelegramWebhook(@Body(new ZodValidationPipe(SetWebhookSchema)) dto: SetWebhookDto) {
  ...
  @Get('max')
  @ApiOperation({
    summary: 'Статус глобального MAX-бота: accessToken (masked), webhook URL, rate-limit.',
  })
  getMax() {
  ...
  setMaxWebhook(@Body(new ZodValidationPipe(SetWebhookSchema)) dto: SetWebhookDto) {
  ...
  @Get('email-inbox')
  @ApiOperation({
    summary: 'Read-only снимок ENV-настроек IMAP-инбокса (host/port/user/folder/cron).',
  })
  getEmailInbox() {
  ...
  @ApiOperation({
    summary: 'Попытаться установить IMAP-соединение и распечатать greeting (TODO 501).',
  })
  testEmailInbox() {
