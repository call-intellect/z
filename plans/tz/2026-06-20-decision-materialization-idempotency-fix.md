...
>
> **ОБНОВЛЕНО 2026-06-20 (исторический контекст выше устарел).** Ветка `feature/knowledge-core-master` **полностью влита в `dev`** (PR #45, через `feature/merge-all-2026-06-17`), и с тех пор `dev` ушёл на ~190 коммитов вперёд (graph.service/combined переписаны ещё раз). Никакой «параллельной сессии» и риска merge-конфликта больше нет. **Ф1–Ф3/Ф6 реализованы прямо на `dev`** (актуальные версии файлов), Ф4 — решено оставить `@unique`. Координация на «ветке kcore» неактуальна.
