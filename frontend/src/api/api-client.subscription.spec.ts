/**
 * Тесты interceptor'а subscription_required в ApiClient.
 *
 * Проверяется:
 *  (1) 403 с кодом `subscription_required` → dispatch CustomEvent + бросает ApiError.
 *  (2) 403 с другим кодом → НЕ dispatch event + бросает ApiError.
 *  (3) 200 OK → возвращает данные, без event'ов.
 *
 * ТЗ: plans/tz/2026-05-28-paywall-no-trial.md §4.3.
 */
...
describe('ApiClient — subscription_required interceptor', () => {
  ...
  it('200 OK → возвращает данные, без event', async () => {
    ...
    const result = await client.get<{ id: string; name: string }>('/api/v1/meetings');
