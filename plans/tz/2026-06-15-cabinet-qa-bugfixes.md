...
3. **[x] Ф3 — «В задачи» из очереди работает** (🔴) — РЕАЛИЗОВАНО (commit Ф3), но **с другим дизайном** (см. ниже): вместо пикера — дефолт-проект «Входящие» (решение владельца). Перенос задачи в другой проект → `plans/tz/2026-06-15-issue-move-to-project.md`.
...
7. **[ ] Ф7 — Интент: вопросы сотрудников ≠ обещания** (🟡) — ВЫНЕСЕНО отдельным ТЗ `plans/tz/2026-06-15-intent-questions-are-not-commitments.md` (калибровка LLM, не строчный баг).
