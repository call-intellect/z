...
const SEEDS: TaskRouteSeed[] = [
  {
    ...
    playbookSection:
      '§2.1 block-distill / decision-supersede-detect (verdict-only arbiter) + KC-Temporal ТЗ §W1.2',
    chain: [
...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log('=== seed-llm-task-routes-temporal DONE ===');
}

main()
  .catch((err) => {
    // eslint-disable-next-line no-console
    console.error('seed-llm-task-routes-temporal FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
