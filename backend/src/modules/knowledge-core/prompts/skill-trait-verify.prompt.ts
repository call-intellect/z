...
export const SKILL_TRAIT_VERIFY_SYSTEM_PROMPT = withAsrNote(
  withEdgeCasePolicy(
    [
      ...
      '',
      'ФОРМАТ ОТВЕТА (СТРОГО). Весь твой ответ — это РОВНО ОДИН валидный JSON-объект по схеме `skill_trait_verify_v1` и ничего больше. Первый символ ответа — «{», последний — «}».',
      'НЕЛЬЗЯ: markdown-ограждения (```), преамбулы и рассуждения вне JSON («В данном случае…», «Проанализируем…», «Ответ:»), любой текст до «{» или после «}».',
      'Твоё рассуждение целиком помещается в поле reason ВНУТРИ JSON. Шаблон ответа:',
      '{"grounded": true, "reason": "Цитаты 1 и 2 показывают собственный подход — откладывать сроки до сбора данных."}',
    ].join('\n'),
  ),
);
