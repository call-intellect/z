...
export async function judgeG(
  q: BankQuestion,
  run: RunResult,
  ctx: { retrievedTexts: string[]; statusFacts: string[]; absentFacts: string[] },
): Promise<LensG> {
  const system = [
    ...
    'НЕ клейми выдумкой: (а) общие рассуждения и советы; (б) явно помеченную аналогию/экстраполяцию; (в) факт, который ЕСТЬ в поднятом контексте ниже; (г) честный отказ.',
