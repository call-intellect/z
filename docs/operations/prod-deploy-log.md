...
- **Шаг 11 — Docker rebuild** — `docker compose up -d --build backend` (anyOf-nullable схема block-ingest, chat-summary эпизода из `payload.rollingSummary`, retry эмбеддингов, свип `::vector(1536)` → `::vector` в 19 файлах src+scripts — без него ЛЮБАЯ запись эмбеддинга падает на 768-колонках).
