/**
 * Snapshot-тест сборки промта `executable-persona-compile.prompt.ts` v2
 * (TZ clone-method ИНТ.1 — секционная сборка persona из слоёв метода).
 *
 * ⚠ НЕ про качество LLM-вывода. Snapshot фиксирует:
 *   - текст `EXECUTABLE_PERSONA_COMPILE_V2_SYSTEM_PROMPT` (constant — guard от
 *     случайных правок: 5 секций, инвариант «правило ПРОЦЕССА, не ярлык» и
 *     гипотезный тон должны быть стабильны; плюс SYSTEM cache-friendly —
 *     правка ломает prompt-кэш);
 *   - сборку `EXECUTABLE_PERSONA_COMPILE_V2_USER_TEMPLATE` для полного входа
 *     (все 5 блоков) и инвариант «пустой блок не рендерится вовсе».
 *
...
describe('executable-persona-compile v2 — snapshot сборки промта', () => {
  it('system prompt стабилен', () => {
    expect(EXECUTABLE_PERSONA_COMPILE_V2_SYSTEM_PROMPT).toMatchSnapshot(
      'system',
    );
  });
    ...
    const user = EXECUTABLE_PERSONA_COMPILE_V2_USER_TEMPLATE({
      ...
      traits: [
        trait({
          category: 'решения',
          statement:
            'обычно сначала собирает данные по 2–3 источникам, потом решает',
        }),
        trait({
      ...
      processMarkers: [
        trait({
          category: 'варианты',
          statement:
            'обычно перед рекомендацией перечисляет варианты и критерий выбора',
        }),
      ],
    });
  ...
  it('имя промпта — v2', () => {
    expect(EXECUTABLE_PERSONA_COMPILE_V2_PROMPT_NAME).toBe(
      'executable_persona_compile_v2',
    );
  });
});
