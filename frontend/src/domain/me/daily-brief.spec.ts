/**
 * Unit-тесты маппера брифа `mapDailyBrief` (ApiDto→DomainModel).
 *
 * ТЗ 2026-06-11 mobile-cora-exec-manager, Ф0. Детерминированно, без сети.
 * Главное — позитивная рамка Р4 (держишь слово N из M, под угрозой/перенести)
 * и cold-start `isEmpty`.
 */
...
describe('mapDailyBrief — под рукой сегодня / под угрозой', () => {
  it('onDeckToday = задачи+обещания НЕ overdue; atRiskItems = overdue', () => {
    const dto = brief({
      myTasks: [item({ title: 'T1', overdue: false }), item({ title: 'T2', overdue: true })],
...
describe('mapDailyBrief — переименования и типы', () => {
  it('dueDateIso → Date, null → null; counterpartyName пробрасывается', () => {
    const dto = brief({
      myTasks: [
        item({ dueDateIso: '2026-06-11T09:00:00.000Z', counterpartyName: 'Иван' }),
  ...
  it('insightCoOccurrence опционален: отсутствует на бэке → null', () => {
    ...
    const withInsight = mapDailyBrief(
      brief({
        insightCoOccurrence: { statement: 'дедлайн жмёт', colleaguesCount: 3, escalated: false },
      }),
    );
...
describe('mapDailyBrief — cold-start isEmpty', () => {
  ...
  it('есть хинт → isEmpty=false', () => {
    expect(mapDailyBrief(brief({ hint: 'Загляни в чек-ин' })).isEmpty).toBe(false);
  });
