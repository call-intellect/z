...
/**
 * Pulse Wave 2.3 — тепловая карта настроений по людям для COO-дашборда.
 *
 * Источник данных: `OperationsTeamTemperatureApi.byPerson` — массив
 * `{personId, personName, green, yellow, red, total}` за окно (default 7 дней).
 *
 * Визуал: имя слева, ячейки чек-инов посередине (по одной ячейке на каждый
 * чек-ин — red/yellow/green), агрегатный score справа. Score рассчитывается
 * как `(green − red) / total ∈ [−1; +1]`, мапится в `success/warning/danger`
 * тон. Показываем максимум 20 человек (sorting уже на бэке по `red DESC`).
 */
