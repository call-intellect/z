...
5. **Справочник** *(collapsible, default свёрнут, storageKey `sidebar.reference.open`)* — `/company`, `/departments`, `/domains`, `/maturity`, `/documents`, `/roles`, `/clones`, `/customers`, `/vendors`, `/events`, `/experiments`, `/brand-voice`, **`/tables` («Таблицы»)**. Внутри — вложенная подгруппа «Будет в следующей фазе» с γ-пунктами (`/processes`, `/policies`, `/metrics`). *(2026-06-04: `/structure` отсюда убран — стал «Команда» в группе «Каждый день».)* *(2026-06-23: пункт «Клиенты» (`/customers`) добавлен рядом с «Поставщики».)* *(2026-07-03: пункт «Таблицы» (`/tables`) возвращён в «Справочник» — закрывает находку прод-аудита «нет пункта меню» для умных таблиц; ветка `work/2026-07-02`.)*
...
- `/chat` (десктоп) — **дом Мастера (concierge), 2026-06-28.** `MasterChatHome` (`frontend/app/(authenticated)/chat/MasterChatHome.tsx` + `MasterConversation.tsx` + редьюсер `src/ui/chat/master-chat-events.ts`) на движке concierge: слева список диалогов (`conciergeApi.listConversations`, названия из `titlePreview`/`summary`), история по выбору (`getConversation`), стрим ответа (SSE + polling-fallback), markdown, цитаты passthrough, действия+undo+table-preview, переключатель «Помощник / Клон» (`AssistantTargetSelect`), `asOf`. Заменил прежний `ChatV2Client` как лицо чата — chat-v2 теперь только внутренний ответчик Мастера. Схлопывание 3 движков (chat-v2 / concierge / legacy) на один Мастер — ТЗ [`2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md`](../../plans/tz/2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md) (КФ1–КФ5, коммиты `7d111a94`..`c5b7ec10`); визуальная приёмка прода — pending (см. [[../04_не-сделано/README]]).
- `/chat-v2`, `/assistant` — **redirect → `/chat` (2026-06-25, «Единый помощник» Ф1/Ф5).** Орфаны-входы в chat-v2/assistant убраны: единственный вход к ответам — Мастер на `/chat`. `/chat-v2` сохраняет `?conversationId=` при редиректе (`chat-v2/page.tsx`). Сироты-движки удалены (КФ5, 2026-06-28): `ChatV2Client`(+spec) / `OrgChatPanel` / `AssistantClient` / `ConciergeChat` / `ConciergeClonesTab`(+spec) / `ConciergeSlot` / `chat-v2/ChatPanel`. Селектор «помощник / клон должности» (2026-06-15) живёт в панели Мастера (`AssistantTargetSelect`).
- `/memory` — **карта «Второй мозг» (2026-07-03) + дверь к реестрам.** `memory/page.tsx` — табы **[Карта | Все реестры]**. **Карта** (`BranchMapClient.tsx`, главный вид) — 12 областей компании (стратегия/клиенты/продажи/маркетинг/продукт/операции/команда/финансы/технологии/производство/партнёрства/право) плитками с счётчиками (темы/регламенты/процессы/документы/решения) и сигналом-светофором; читает `GET /api/v1/knowledge/branches`. **«Все реестры»** — прежняя хаб-сетка ссылок (Решения / Оцифровано / Темы / Сущности / Таблицы / Идеи / Сигналы) + дверь **«Спросить у Мастера»** (Р3, ведёт на `/chat`). Кусочный `MemorySearch` остаётся удалён. ТЗ [`2026-07-02-second-brain-by-branches`](../../plans/tz/2026-07-02-second-brain-by-branches.md).
- `/memory/[branch]` — **экран области (2026-07-03).** `BranchDetailClient.tsx` — деталь одной из 12 областей: summary «где мы по области» + темы + деривированные регламенты/процессы/документы/решения со ссылками в существующие разделы; читает `GET /api/v1/knowledge/branches/:branch`. Домен `src/domain/branch.ts`, API `src/api/branches.api.ts`.
- `/themes`, `/themes/[id]` — **живое пространство темы (2026-07-03).** Тему теперь **заводит человек**: `/themes` (`ThemesClient.tsx`) — кнопка «+ Новая тема» (`CreateThemeDialog.tsx`, создание фразой personal/team) + бейдж «моя тема» для своих. `/themes/[id]` (`ThemeDetailClient.tsx`) — **живая читаемая страница темы**: суть/лента блоков (авто-наполнение) / кто в теме / решения с кнопкой «завести задачу» (мост в трекер) / задачи / документы / регламенты; поповер **«почему»** (`addedVia`/`score`/`reason`) и unpin (удаление лишнего → исключение). API `src/api/themes.api.ts` (+create/rename/archive/pin/unpin/commitmentToTask), домен `src/domain/theme.ts` (+origin/visibility/isMine, addedVia/score/reason). ТЗ [`2026-07-02-living-topic-space`](../../plans/tz/2026-07-02-living-topic-space.md).
...
**`/month` — герой «Месяц компании» (2026-06-30):** на `/month` (rhythm `month`) над `DashboardCanvas` владельцу показан герой `MonthCompanyHero` (`frontend/src/ui/components/dashboard/month-company/*`): `MonthVerdictCover` (4 оси + тренд по неделям) → «Читать письмо месяца» (`MonthLetter`) → `MonthGoalCompass` (переиспользует общий `GoalCompass3D`) + `MonthGoalPace` (факт/план/ETA/ведущий сигнал) → `MonthDecisions` («что решить собственнику») → `MonthNextFocus` («фокус месяца») → таблица план/факт за месяц (переиспользует `WeeklyPerPersonWidget` с месячным окном; колонка «Вклад в цель» = range-sum `goalContributionNet` по окну). Слои `src/api/monthly-digest.api.ts` → `src/domain/operations-monthly-digest.ts` → `src/hooks/useMonthCompany.ts`; читает `GET /dashboard/operations/monthly-digest/latest`. Таб «Месяц» в rhythm-switcher уже вёл на `/month`. См. [[director-dashboard]] §«Месяц компании». Источник — `plans/tz/2026-06-29-month-company-monthly-brief.md`.

**Навигация по датам + архив отчётов (2026-06-30):** к 3 героям (день/неделя/месяц) подключён общий `PeriodNavigator` (`frontend/src/ui/components/dashboard/shared/PeriodNavigator.tsx`) — stepper по ритму («вперёд» disabled на latest), клик-подпись → попап «Недавние отчёты» (архив с цветной точкой `stateHint`) + «Выбрать дату…» (нативный picker). Хелперы `src/domain/period.ts` (shiftDate/mondayOf/shiftWeek/shiftPeriodYm/shiftPeriod/formatPeriodLabel/currentPeriod/comparePeriods/nearestAvailablePeriod) + `PeriodEmptyState` (нет данных → ближайший/«К последнему»; первый запуск → обучающий). Каждый герой держит `selectedPeriod` (default «последний завершённый» из `available-periods.latest`), грузит данные по периоду (SWR-ключ включает период) + кнопка «К последнему». `value-recap` переведён с локального `PeriodSelector` на общий `PeriodNavigator`. См. [[frontend-contexts-hooks]] §«PeriodNavigator», [[director-dashboard]] §«Месяц компании».
...
| `/chat` | `MobileAskClient` («Спросить») — с 2026-06-28 тонкая обёртка над `MasterScopedChat` (scope:org, движок Мастера: markdown/цитаты/действия/опц. голос-ВВОД+TTS/suggestedPrompts) | `MasterChatHome` (дом Мастера на concierge; раньше рендерил `ChatV2`) |
...
- **2026-06-28 (chat-surface convergence — один Мастер как лицо чата, КФ1–КФ5):** 3 движка чата (chat-v2 / concierge / legacy) схлопнуты на **один — Мастер (concierge)**; chat-v2 остаётся внутренним ответчиком. Десктоп `/chat` = `MasterChatHome` (`frontend/app/(authenticated)/chat/` + редьюсер `src/ui/chat/master-chat-events.ts`): список диалогов слева (`conciergeApi.listConversations`, названия из `titlePreview`/`summary`), история по выбору, SSE+polling-стрим, markdown/цитаты passthrough/действия+undo+table-preview, переключатель «Помощник / Клон» (`AssistantTargetSelect`), `asOf`. Переиспользуемый `src/ui/chat/MasterScopedChat.tsx` (+`MasterCitations.tsx`) — scoped-чат Мастера; `IssueChat` (scope:issue) и мобильный `MobileAskClient` (scope:org) стали тонкими обёртками над ним. Двери: nav-пункт «Спросить» (/chat) открыт лидерским ролям (Р1); `/memory` → дверь «Спросить у Мастера» (Р3); FAB `ConciergeFloatingButton` → кнопка-навигация в /chat (поповер демонтирован). Cmd+K «?» → `conciergeApi.askOnce` (был chatV2). Сироты КФ5 удалены: `ChatV2Client`(+spec)/`OrgChatPanel`/`AssistantClient`/`ConciergeChat`/`ConciergeClonesTab`(+spec)/`ConciergeSlot`/`chat-v2/ChatPanel`. Не вошло: `MeetingChatPanel`-миграция и pin/archive/feedback дома — под-ТЗ (см. [[../04_не-сделано/README]]). Источник: [plans/tz/2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md](../../plans/tz/2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md) (КФ1–КФ5, коммиты `7d111a94`..`c5b7ec10`). См. [[frontend-contexts-hooks]] · [[concierge-agent]] · [[api-layer]].

- **2026-06-25 («Единый помощник» Ф1/Ф5 — один вход, «Память»=дверь):** `/chat-v2` и `/assistant` → **redirect `/chat`** (`chat-v2/page.tsx` сохраняет `?conversationId=`, `assistant/page.tsx`); кусочный `MemorySearch` на `/memory` **удалён** — `memory/page.tsx` стала хаб-дверью к реестрам (сетка ссылок + «Спросите у Мастера»); видимые «Concierge»→«Мастер»; `eventTypeLabel` (`domain/conversational.ts`) — фолбэк `latin_snake`→«Уведомление». FAB прямого чата + единый колокольчик `PendingActionsBell` уже были сделаны параллельной сессией (2026-06-20). **Полное схлопывание 3 движков чата (chat-v2/concierge/legacy) и 4 поверхностей (`/chat` десктоп, мобильный Ask, `IssueChat`, `MeetingChatPanel`) на один Мастер — отколото** в [`2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md`](../../plans/tz/2026-06-25-edinyy-pomoshnik-chat-surface-convergence.md) (нужна визуальная приёмка, см. [[../04_не-сделано/README]]). Источник: [plans/tz/2026-06-25-edinyy-pomoshnik-arhitektura.md](../../plans/tz/2026-06-25-edinyy-pomoshnik-arhitektura.md) (Ф1/Ф5, коммит `8dac5210`).
