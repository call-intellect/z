...
import type { WeeklyDigestLetterSectionApi } from "@/api/weekly-digest.api";
import { CHART, GRAD } from "@/ui/components/dashboard/modern";
import { cn } from "@/ui/shadcn/lib/utils";

const ICON_ON_GRADIENT = "oklch(0.99 0.005 280)";

export function WeekLetter({
  sections,
  fallbackProse,
}: {
  sections: WeeklyDigestLetterSectionApi[] | null;
  ...
  return (
    <div>
      ...
      >
        {open ? "Свернуть письмо" : "Читать недельное письмо"}
        <ChevronDown
          size={18}
          aria-hidden
          ...
          {sections && sections.length > 0 ? (
            <div className="flex flex-col">
              {sections.map((section, idx) => (
                <section
                  key={section.key || `${idx}`}
                  className={cn("py-6", idx > 0 ? "border-t" : "")}
                  style={
                    idx > 0 ? { borderColor: "var(--glass-border)" } : undefined
                  }
                >
