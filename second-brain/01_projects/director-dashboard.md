...
- **Единый drill-down на «День/Неделя/Месяц компании» (2026-07-03, universal-entity-drilldown-provenance-parity):** блокер/риск/insight/идея-кластер/конфликт проваливаются к первоисточнику **по факту наличия источника** (drawer «Откуда это»), а не по типу: блокер → `sourceBlockId`→drawer entityType=`block`; риск/insight → `sourceBlocksCount>0`→entityType=`insight`; идея-кластер → `/ideas`; конфликт person↔person → entityType=`friction` (id=`EntityLink.id`). `SignalRow` расширен `onClick`/`href` — стрелка/аффорданс только при наличии источника; счётчики и агрегаты статичны. Конфликт двухуровнев (приходит с бэка): owner/admin — дословная реплика, прочие — тема + встреча. Список конфликтов дашборда (`getTeamFrictions`) НЕ отдаёт `sourceBlockIds` наружу (anti-leak, регресс-тест). Виджеты: `DayBlockers`/`WeekBlockers`/`MonthBlockers` + `DaySignalsGrid`/`WeekSignalsGrid`/`MonthSignalsGrid`.
...
### День компании v2 (2026-07-01) — письмо COO + полный вход с атрибуцией + виджеты

Надстройка над «Днём компании» (ТЗ `plans/tz/2026-06-30-day-company-report-v2.md`, Ф1–Ф8, ветка `work/2026-06-29`). Модель/пайплайн/агент НЕ менялись — переписаны промпт, наполнение пакета, виджеты, тайминг, маршрут.

- **Письмо COO (промпт v2 `day-company-v2`):** 12 секций проза+cites (`intro,main,done,not_done,reporting,blocked,clients,ideas,attention,actions,delta,reflection`); имена людей и клиентов **прямо** (отчёт приватный, читатель один — владелец); петля со вчера (динамика осей); финальный свободный блок «Взгляд операционного директора» (`reflection`, 5–6 предложений интерпретации, без новых фактов/чисел); «что сделано»/«не сделано» — обобщённая проза без поштучных списков; «что помешало» суммирует причины (узкие места + повторяющиеся боли + конфликты). **Сущность «решения» удалена везде** (промпт/схема/`letterJson` key `decisions`). Числа дисциплины (план/факт, %) считает СИСТЕМА, LLM только вставляет. Строгая JSON-схема (`.strict()` синхронно Zod + json_schema).
- **Новые слои входа (`buildDayPackage` наполнен):** `employeeVoice` (голос сотрудника из графа по авторству — 3 пути: блоки/мысли через `IdeaBlockEvidence(authorPersonId,sourceTimestamp)`, идеи через `Idea.createdByUserId`, риски через `Insight.sourceBlockIds`), `rawConversations` (переписка Битрикс/чатбокс целиком за день с подписью «кто говорил» + метка сотрудник/клиент, страховочная обрезка по `operations.daily_digest.raw_char_budget`=40000), `signals` (блокеры/риски-по-причине/идеи-кластеры), `conflicts` (за день из `EntityLink conflicted_with`, забор `getTeamFrictions(since=начало дня)`), `reporting` (план↔факт по людям из `DailyCheckIn`, числа системой), `yesterdayOpenSignals` (петля со вчера). Единый резолв `userId`/`externalId`→Person — сервис `PersonRefResolverService` (клиент отличён от сотрудника, не найден → «без автора»). Новый индекс `IdeaBlockEvidence(tenantId,authorPersonId,sourceTimestamp)`.
- **Виджеты day-only перегруппированы:** `DayBlockers` (блокеры отдельной красной секцией «что прямо мешает») + `DaySignalsGrid` (риски сгруппированы по `causeCategory` в «Коммуникация и люди»/«Инструменты»/«Процессы»; идеи одним виджетом кластерами; конфликты «Команда: трения» — пары + confidence). Плоский дубль идей убран. Week/Month НЕ тронуты.
- **Маршрут и тайминг:** маршрут `operations-daily-digest` = DeepSeek Pro (`deepseek-v4-pro`) → GPT (`openai-via-proxy/gpt-5.4-mini`) → KIE (`kie/gemini-3.1-pro`), жёсткий `maxTokens` снят (большой контекст под полный вход). Крон сдвинут `@Cron('0 3 * * *')` → `@Cron('0 7 * * *', Europe/Moscow)` = **07:00 МСК** (после сбора чек-инов 05:00 — виден полный день). Усилена разметка `team_friction` в `block-ingest` (иначе ось «Команда» пуста). Отложено (`04_не-сделано`): Probe во вход, сдвиг продюсера целей (компас как есть), снос `CheckInConflictDetectorCron`.
...
### Неделя/Месяц компании v2 (2026-07-01) — паритет с «Днём компании v2»

Экраны «Неделя компании» (`/dashboard` таб «Неделя») и «Месяц компании» (`/month`) подняты до уровня «Дня компании v2» (ТЗ `plans/tz/2026-07-01-week-month-company-v2.md`, Ф1–Ф8, ветка `work/2026-06-29`). Модели/пайплайны/агенты НЕ менялись — переписаны промпты, наполнение пакета, виджеты; **миграций БД нет** (все новые данные в существующем JSON-поле `metricsJson`).

- **Письмо COO-голосом v2 (промпты `week-company-v2`/`month-company-v2`):** 12 секций проза+cites (`intro,main,done,not_done,reporting,blocked,clients,ideas,attention,actions,delta,reflection`), имена людей/клиентов прямо, петля с прошлым периодом (динамика осей), финальный «взгляд операционного директора» (`reflection`). Строгая zod (`z.enum` ключей письма отклоняет невалидные). Недельный синтез теперь читает **полные дневные письма** (`letterJson` дневных снапшотов), а не только вердикты; месячный вход = сжатые недельные.
- **Сигналы за окно периода снапшочены в `metricsJson`** (не live-top): недельный И месячный дайджест несут `risksByCause` (`InsightListItem[]`, риски по причине), `ideaClusters` (`IdeaCluster[]`), `teamFrictions` (`TeamFriction[]`), `blockers` (`Blocker[]`) — собираются helper'ом `collectWindowSignals` за `[weekStart..weekEnd]` / `[1-е..последнее число месяца]`. `OperationsDashboardService.getTeamFrictions` получил верхнюю границу окна `to`, `getBlockers` — окно `window {from,to}` по `dateLocal`. Плитки на фронте рендерятся **из дайджеста за период** (`digest.metrics`), а не из «топ сейчас».
- **`decisions` месяца переименовано в `ownerForks`** сквозняком (промпт/схема/zod/сервис/DTO/фронт). UI-блок «Что решить собственнику» → **«Развилки месяца»** (`MonthDecisions.tsx` → `MonthForks.tsx`).
- **Ось «Команда» в вердикте краснеет от повторяющихся трений** за окно (`clampWeekVerdict`/`clampMonthVerdict`, хелпер `computeTeamFrictionClamp`); пороги — крутилки.
- **Виджеты:** `WeekBlockers`/`WeekSignalsGrid` и `MonthBlockers`/`MonthSignalsGrid` (форк дневных, данные из `digest.metrics` за период). Canvas-дубль скрыт для owner на неделе И месяце; уникальные month-only виджеты (`month-recap`/`achievements`/`weekly-dynamics`/`maturity`/`bus-factor`) перенесены в секцию «Итоги месяца» внутри `MonthCompanyHero`.
- **Крутилки (AdminSetting, code-fallback, Ship-On — работают до сида):** `operations.weekly_digest.raw_char_budget`=60000 (бюджет символов дневных писем во входе недельного промпта), `operations.digest.team_friction_min_confidence`=0.7, `operations.digest.team_friction_repeat_count`=2 (порог оси «Команда»). Сид — `backend/scripts/seed-admin-setting-week-month-v2.ts`. Новых feature-флагов НЕТ (kill-switch недели/месяца существуют), новых ENV НЕТ. Данные — [[../02_architecture/data-model]] §«WeeklyOperationsDigest»/«MonthlyOperationsDigest»; промпты — [[ai-jobs]] §«operations-weekly-digest»/«operations-monthly-digest».
