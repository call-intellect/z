...
- **Шаг 7 — Seed (НОВЫЙ, идемпотентный, STEPS `phase:'seed-base'`):** `scripts/seed-admin-setting-table-graphsync.ts` — **3 ключа** `table.graphsync.enabled`=`true` + `table.graphsync.min_confidence`=`0.5` + `table.agent.draft_ttl_days`=`14` (category `tables`, section `agent`, severity `low`). Защита admin-edited (`updatedBy !== 'system'`); повтор = no-op.
- **Шаг 8 — Backfill (НОВЫЙ, идемпотентный, STEPS `phase:'backfill'`, `skipBootstrap`):** `scripts/backfill-table-graphsync.ts` — (а) проставляет `Table.graphSync` из каталога системным таблицам ideas/promises/okr/hypotheses/**risks** (набор `GRAPHSYNC_SYSTEM_KEYS` авто-выводится из каталога); (б) `reconcileTenant` по всем Org: создаёт строки из живых Goal/Experiment/IdeaBlock — включая **риск-сигналы** (`risk`/`churn_risk`/`blocker`/`pain`/`resource_gap`/`*_friction` → «Реестр рисков», Описание←name) — гейт confidence≥0.5 (ниже → draft), дедуп по sourceObjectId, fill-empty. Поддержка `--org=<id>`. Идемпотентен (второй прогон = 0 created).
