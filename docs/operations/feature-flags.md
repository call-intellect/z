...
| `taskReconcile.enabled` / `TASK_RECONCILE_ENABLED` | 🟢 ВКЛ | Суточный reconcile петли «разговор → закрытие задачи» (cron 03:00 → протухание pending-кандидатов TaskClosureCandidate + пересчёт reopen-rate `task_closure_reopen_rate` + WARN-алёрт при превышении `taskClosure.reopenRateAlert` + переэмит пропущенных событием матчей). Выкл → крон НЕ трогает БД (ранний return до findMany). (task-dedup 2026-06-16 Ф3) |
| `taskDedup.enabled` (AdminSetting) | 🟢 ВКЛ | Гейт дедупа задач перед созданием (Ф1): синхронный embed кандидата + `findSimilarByVector` + LLM `task-dedup-arbiter` → suggest дубля (НЕ авто-merge). Выкл → задачи создаются без дедуп-подсказки. Пороги: `taskDedup.suggestThreshold` (0.88), `taskDedup.embedTimeoutMs`. (task-dedup 2026-06-16 Ф1) |
| `taskClosure.enabled` (AdminSetting) | 🟢 ВКЛ | Петля «разговор → кандидат закрытия» (Ф2): handler `task.completion_signalled` → семантический матч + LLM `task-closure-verify` → обратимый `TaskClosureCandidate` (НЕ авто-закрытие). Выкл → кандидаты на закрытие не создаются. Пороги: `taskClosure.matchThreshold` (0.85), `taskClosure.autoConfirmThreshold` (0.95). (task-dedup 2026-06-16 Ф2) |
...
| `knowledge_base.redesign.enabled` | 🟢 ВКЛ | Новая раскладка раздела «База знаний» (широкая оболочка + левое дерево + колонка чтения + TOC + тумблер ширины). Выкл → прежняя master-detail раскладка. Едет на фронт через `/regulations/summary.redesignEnabled`. (knowledge-base-redesign Ф5) |
...
| `ai.mainReport.primary` / `LLM_MAIN_REPORT_PRIMARY` | 🟢 `deepseek` | Основной провайдер ГЛАВНОГО отчёта встречи (`LlmFallbackService`): `deepseek` (Ship-On — кэш DeepSeek + per-agent pro-модель, каскад DeepSeek→MiniMax→OpenAI). Рубильник-откат: `=minimax` → прежний каскад MiniMax→OpenAI. AdminSetting-ключ (enum `minimax`/`deepseek`), ENV — fallback. (retest3 Ф5 #51/Р3 · config Шаг 8) |
| `mail.dryRun` / `MAIL_DRY_RUN` | 🔴 ВЫКЛ | Сухой прогон почты: `true` → письма не уходят реально, а логируются (отладка/инцидент). `false` (Ship-On — почта отправляется). Аварийный рубильник на случай инцидента в рассылке. AdminSetting-ключ (`cfg.mail.dryRun`, `z.boolean()` default false), ENV — fallback. (config Шаг 8) |
...
| `aiFeatures.regulationConsolidatorEnabled` | 🟢 ВКЛ | Крон-консолидатор накопленных дублей карточек регламентов/процессов/политик/инструкций ВНУТРИ одного типа (`RegulationConsolidatorCronService` каждые 30 мин → enqueue → `RegulationConsolidatorWorker`): LATERAL top-1 HNSW по embedding в пределах типа, арбитр дедупа `regulation-dedupe` → merge с `CardVersion(changeReason:'consolidate')`. Защита: не трогает карточку с `currentVersion.trustTier='human'` и пары с человеко-решением «разные» (CurationDecision reject/split). Выкл → крон no-op, дубли не консолидируются. AdminSetting-ключ (code-fallback true, без ENV). (ТЗ 2026-06-20 регламенты Ф5a) |
...
| `probe.qualityJudgeEnabled` | 🟢 ВКЛ | LLM-судья качества формулировки уточняющего вопроса (probe): после `probe-formulate` в `ProbeDispatcherWorker` вопрос проверяется судьёй (`probe-quality-judge`) — не пустой/не расплывчатый/без кода-латиницы/один вопрос/≤400 симв.; при браке и валидном rewrite (детерминированный маркер-чек) шлётся один улучшенный регенерат. Best-effort: судья упал → шлём исходный (метрика `probe_quality_judged_total{verdict}`). Выкл → вопрос отправляется как сформулирован, без проверки. AdminSetting-ключ (code-default true). (ТЗ probe-system-phase2-completeness Ф2) |
| `probe.engagementRoutingEnabled` | 🟢 ВКЛ | Выбор получателя уточняющего вопроса (probe) по engagement: в `ProbeDispatcherWorker` из кандидатов (после rate-limit) вопрос идёт самому отзывчивому — берётся максимальный engagement-снимок из Redis (`probe:engagement:<userId>`, пишет `ProbePriorityCron`); отсутствует снимок → нейтраль 0.5, tie-break детерминированный (меньший `userId`). Best-effort: Redis/настройка упали → нейтраль для всех → первый по сортировке. Выкл → прежний round-robin (первый кандидат). AdminSetting-ключ (code-default true). (ТЗ probe-system-phase2-completeness Ф3) |
| `probe.semanticDedupEnabled` | 🟢 ВКЛ | Семантический дедуп близких по смыслу уточняющих вопросов (probe): перед отправкой эмбеддинг вопроса (`questionEmbedding` в `probe_events`, HNSW-индекс) сверяется с недавними probe — если cosine ≥ порога в окне поиска, дубль не отправляется. Выкл → дедуп только по точному content-hash (близкие по смыслу повторы возможны). AdminSetting-ключ (code-default true). (ТЗ probe-system-phase2-completeness Ф4) |
| `probe.reaskEnabled` | 🟢 ВКЛ | Один переспрос (re-ask) неотвеченного уточняющего вопроса (probe): при истечении `ProbePriorityCron` создаёт переформулированный переспрос (`ProbeEvent` status='pending', `payload.reaskCount=1` + `originalProbeEventId`) и ставит его в очередь dispatcher'а; исходный закрывается как `expired` БЕЗ исхода ignored (он переспрошен, а не проигнорирован). Dispatcher по `reaskCount≥1` добавляет в КОНЕЦ USER `probe-formulate` мягкую пометку «это повторный вопрос». Переспрос создаётся напрямую, минуя suggest-гейты (в т.ч. topic-cooldown). Второй раз без ответа (`reaskCount≥1`) → `expired` + ignored. Выкл → истёкший probe сразу закрывается как ignored без переспроса. AdminSetting-ключ (code-default true). (ТЗ probe-system-phase2-completeness Ф5) |
| `probe.valueGateEnabled` | 🟢 ВКЛ | LLM-гейт ценности уточняющего вопроса (probe): в `ProbeDispatcherWorker` и `ProbeDigestCron` ПЕРЕД формулировкой `ProbeFormulationService.gate()` решает, стоит ли беспокоить человека (`probe-value-gate`, JSON `{ask, reason}`) — пустой пробел (нет объекта/внятной сути, ответ уже виден, объект — общее слово) роняется как `dropped_low_value` (метрика `probe_value_gate_total{verdict=skip}`), вопрос не отправляется. Сбой/невалидный JSON LLM → fail-open `{ask:true, reason:gate_error}` (нужное не глушим). Выкл → `{ask:true, reason:gate_disabled}` без LLM-вызова, вопрос формулируется всегда (старое поведение). AdminSetting-ключ (code-default true), ENV-fallback `PROBE_VALUE_GATE_ENABLED=true`. (ТЗ probe-smart-questions-module Ф4) |
| `probe.digestFormulateEnabled` | 🟢 ВКЛ | Формулировка вопросов батч-дайджеста probe через общий LLM-сервис: в `ProbeDigestCron.collectAndSend` для item без `payload.formulatedQuestion` идёт цепочка `gate()→formulate()→judgeQuality()`, результат сохраняется в `probeEvent.payload.formulatedQuestion`; `ask=false` исключает item из дайджеста (`dropped_low_value`, метрика skip). Любой сбой/таймаут LLM по item → best-effort фолбэк на детерминированный путь Ф1 (`deriveDigestQuestion`/`humanizeProbeFallback`), дайджест не падает. Выкл → детерминированный путь Ф1 без LLM. AdminSetting-ключ (code-default true), ENV-fallback `PROBE_DIGEST_FORMULATE_ENABLED=true`. (ТЗ probe-smart-questions-module Ф5) |
...
| `ASSISTANT_INBOUND_ASYNC_ENABLED` | 🟢 ВКЛ | Ранний ACK входящих помощника: webhook Telegram/MAX отвечает 200 сразу, тяжёлую обработку (concierge tool-loop) уносит в BullMQ-очередь `assistant.inbound` (воркер `AssistantInboundWorker`, attempts:1, jobId-дедуп). Чинит «5 дублей ответа» (медленный 200 → ретраи доставки). Выкл → синхронная обработка как раньше (аварийный откат, если очередь деградировала). Дедуп по `update_id`/`mid` (Redis SET NX) работает НЕЗАВИСИМО от флага. ENV-рубильник (`cfg.bot.assistantInboundAsyncEnabled`, zBool default true). (ТЗ assistant-calendar-master Ф1) |
...
| `ASSIGNMENT_NOTIFICATIONS_ENABLED` | 🟢 ВКЛ | Уведомление исполнителю при назначении задачи (`issue.assigned`). OFF гасит ТОЛЬКО отправку этого уведомления, действий владельца не требует. ENV-рубильник (`cfg.tracker.assignmentNotificationsEnabled`, zBool default true). (ТЗ assistant-assign-task-notify Ф4) |
| `concierge.enabled` / `CONCIERGE_ENABLED` | 🟢 ВКЛ | Помощник-консьерж (вход в единый мозг помощника). Выкл → помощник недоступен. AdminSetting-ключ (resolveSync, ENV-fallback, code-default true). (config-knobs-to-admin-settings) |
| `knowledge.axisClassifyEnabled` / `AXIS_CLASSIFY_ENABLED` | 🟢 ВКЛ | Классификатор осей блока графа знаний. Выкл → блоки не размечаются по осям. AdminSetting-ключ (resolveSync, ENV-fallback, code-default true). (config-knobs-to-admin-settings) |
| `curation.consistencyCheckerEnabled` / `CONSISTENCY_CHECKER_ENABLED` | 🟢 ВКЛ | Проверка согласованности знаний (поиск противоречий между карточками). Выкл → конфликты не выявляются. AdminSetting-ключ (resolveSync, ENV-fallback, code-default true). Дедуп-окно — крутилка `curation.consistencyCheckerDedupTtlSeconds` (14400). (config-knobs-to-admin-settings) |
| `curation.completenessScannerEnabled` / `COMPLETENESS_SCANNER_ENABLED` | 🟢 ВКЛ | Сканер полноты карточек знаний (находит незаполненные обязательные поля). Выкл → сканер не работает. AdminSetting-ключ (resolveSync, ENV-fallback, code-default true). (config-knobs-to-admin-settings) |
| `tracker.goalAlignmentLowEnabled` / `GOAL_ALIGNMENT_LOW_ENABLED` | 🟢 ВКЛ | Детектор слабой связи задач с целями. Выкл → детектор не работает. AdminSetting-ключ (resolveSync, ENV-fallback, code-default true). (config-knobs-to-admin-settings) |
| `tracker.activityDigestEnabled` | 🟢 ВКЛ | AI-сводка изменений по задаче (catch-up «что произошло», on-demand `GET /issues/:id/activity-digest`). Выкл → эндпоинт отдаёт 200 с «функция выключена», без вызова LLM. AdminSetting-ключ (getDynamic, code-default true), без ENV. (tracker-card-redesign-and-progress Ф8b) |
| `tracker.automationsEnabled` | 🟢 ВКЛ | Движок пользовательских автоматизаций трекера (правила «если — то», `AutomationEngineService` на `tracker.event_occurred`). Выкл → ни одно правило не применяется (движок выходит сразу). AdminSetting-ключ (getDynamic, code-default true, severity `high`), без ENV. (tracker-card-redesign-and-progress Ф10) |
| `tracker.recurrenceEnabled` / `TRACKER_RECURRENCE_ENABLED` | 🟢 ВКЛ | Воркер материализации повторяющихся задач (`RecurrenceMaterializeCron`). Выкл → повторения не создают новые задачи (cron выходит сразу). AdminSetting-ключ (getDynamic, ENV-fallback, code-default true, severity `high`). (tracker-card-redesign-and-progress Ф11) |
...
`MAIL_INBOX_ENABLED` (задачи из писем) · `CLONE_V2_ENABLED` · `BITEMPORAL_ENABLED` / `BITEMPORAL_SUPERSEDE_ENABLED` (версионирование знаний во времени) · `PROMPT_EVOLUTION_ENABLED` (авто-эволюция промптов, GEPA) · `orchestrator.enabled` / `ORCHESTRATOR_ENABLED` (оркестратор суб-агентов — незрелый; kill-switch, **дефолт OFF** честно) · `router.llmFallbackEnabled` / `ROUTER_LLM_FALLBACK_ENABLED` (LLM-фолбэк роутера специалистов — kill-switch, **дефолт OFF** честно).

> **`orchestrator.enabled` и `router.llmFallbackEnabled` — kill-switch с дефолтом OFF (честно).** Оба переведены в AdminSetting (resolveSync, ENV-fallback, code-default `false`) ТЗ config-knobs-to-admin-settings — теперь редактируемы из админки, но дефолт остаётся OFF: фичи незрелы. Лимиты при будущем включении — крутилки `orchestrator.{maxSubagentsPerRun,runTimeoutMinutes}` (5/15) и `router.{fallbackNegativeTtlSeconds,fallbackCacheTtlSeconds}` (60/86400).
...
| `probe.replyClassifyMinConfidence` | 0.6 | Порог уверенности (0–1): входной классификатор каналов засчитывает свободный текст без reply (Telegram) / без reply вовсе (MAX) ответом на открытый probe только при confidence ≥ порога интента `probe_reply`. Ниже → сообщение идёт обычным маршрутом (помощник/заметка). AdminSetting (super_admin), code-fallback `0.6` | Распознавание свободного ответа на probe (ТЗ probe-phase2 Ф1) |
| `probe.semanticDedupThreshold` | 0.92 | Cosine-порог семантического дедупа probe: новый вопрос считается дублем близкого, если сходство эмбеддингов ≥ порога. AdminSetting (super_admin), code-fallback `0.92` | Семантический дедуп probe (ТЗ probe-system-phase2-completeness Ф4) |
| `probe.semanticDedupWindowHours` | 72 | Окно поиска (ч) близких по смыслу probe при семантическом дедупе: сверяются только probe за последние N часов. AdminSetting (super_admin), code-fallback `72` | Семантический дедуп probe (ТЗ probe-system-phase2-completeness Ф4) |
