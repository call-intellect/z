...
**Латентные вопросы системы сохранения, вскрытые разбором кода (вынесены в §10):** (1) ✅ **AGE-асимметрия — НЕ дыра, намеренно по ADR 2026-06-23** (реляционка = источник правды графа, AGE отложен до фазы 2); проверять целостность РЕЛЯЦИОННОГО графа (S-E), а не «почему AGE пуст»; (2) 🔴 компаньоны партиции `Entity.mergedIntoTenantId`/`Person.entityTenantId` не проставляются → Prisma-relation рвётся (S-M3) — похоже на дыру от рефактора партиционирования; (3) 🔴 `entity-resolver` может слить две person-Entity, `applyMerge` не re-point'ит `Person.entityId` → клоны схлопываются (S-C3); (4) анти-fan-out cap=4 (крутилка) молча роняет низкоприоритетных специалистов (S-R5) — осознанный tradeoff, следить за метрикой; (5) tasks — ТОЛЬКО через combined, при OFF задач нет (§7-1). Итог: под анализ+ТЗ идут прежде всего (2) и (3); (1)/(4)/(5) — намеренные, только под наблюдение.
...
- **🆕 Дыры системы сохранения (вскрыты разбором кода 2026-07-01, проверяются в §6.1-S — доложить владельцу отдельным вердиктом):**
  - **AGE-граф асимметричен реляционке — ✅ НЕ ДЫРА, намеренно по ADR.** `IdeaBlockLink` в AGE `z_graph` не пишется никогда; `EntityLink` от почасового `entity-graph-builder.cron` — prisma-only (`EntityLinkService.upsertRichEdge`, минуя `GraphService`); в AGE лишь `derived_from` + типизированные сущности + ручные `addEdge`. **Разъяснено по первоисточнику:** ADR [`2026-06-23-knowledge-graph-ingestion-audit/99-synthesis.md`](2026-06-23-knowledge-graph-ingestion-audit/99-synthesis.md) §6/Q4 — выбран Вариант C: **реляционка (Postgres + pgvector) = источник правды графа; AGE-граф-обход отложен до «фазы 2 по данным» (не включали)**. AGE намеренно вторичен и best-effort (коммит `ee0bb910`: cypher в транзакции ронял бизнес-строки → развязали post-commit + kill-switch `graph.ageEnabled`; [`age-deployment-decision.md`](../../second-brain/02_architecture/age-deployment-decision.md)). Реляционный граф наполняется корректно (ТЗ [`2026-06-23-knowledge-graph-ingestion-rebuild.md`](../tz/2026-06-23-knowledge-graph-ingestion-rebuild.md) + `47d4122c` гибрид рёбер + [`2026-06-25-knowledge-graph-hygiene.md`](../tz/2026-06-25-knowledge-graph-hygiene.md)). **Проверять надо целостность РЕЛЯЦИОННОГО графа (S-E1/S-E2/S-E4/S-E5/S-E6), а не «почему AGE пуст».** Остаточный вопрос — только «читает ли кто-то AGE-подграф»: если нет — мёртвый вес (уборка), не потеря знаний. `cypher-builder.ts:5` (whitelist без 9 SBA-типов) актуален лишь если фазу-2 когда-нибудь включат.
