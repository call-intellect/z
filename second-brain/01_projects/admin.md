...
- **Секция «Подключение» в форме провайдера (2026-07-10):** сегмент-переключатель «Напрямую / Через прокси» вместо Switch; живой предпросмотр «Куда пойдёт запрос» — эффективный base (при прокси из `GET /admin/llm-providers/connection-meta`, формула = `resolveEffectiveConnection`) + суффикс протокола (`/chat/completions`, `/responses`, `/v1/messages`, …); подсказки baseUrl зависят от протокола (для `anthropic-messages` — адрес БЕЗ `/v1`, SDK добавляет `/v1/messages` сам); при прокси baseUrl помечен «не используется», подсказка ключа объясняет, что префикс прокси добавится автоматически (маска префикса из меты). Формула прокси протокол-зависимая (`PROTOCOLS_APPENDING_OWN_PATH` = `anthropic-messages`, `kie-native` — сами строят путь с `/v1`, поэтому к базе прокси `/v1` не дописывается: `{root}/{proxyPath}`); Anthropic через прокси = `proxyPath=anthropic` → `{root}/anthropic/v1/messages`, ключ в `x-api-key` с префиксом (нужен openai-proxy с anthropic-маршрутом, добавлен 2026-07-10 — см. [plans/tz/2026-07-10-anthropic-proxy-route.md](../../plans/tz/2026-07-10-anthropic-proxy-route.md)); при выборе anthropic-протокола с прокси proxyPath префиллится «anthropic», warning остаётся только при пустом proxyPath (корневой /v1 ведёт на OpenAI). Кнопка **«Сохранить и проверить»** — save → `:id/smoke-test` → toast результата. Предпросмотр моделей при создании работает и через прокси (`useProxy`/`proxyPath` в `discover-preview`), причём при прокси `baseUrl` не нужен (DTO optional). Обкатка владельцем вечером 2026-07-10: маршрут на прокси — **Select** «OpenAI/Anthropic/grsai/KIE» (нестандартный слаг существующей записи — отдельный пункт), поле прямого адреса в прокси-режиме **скрыто** (сохраняемый `baseUrl` подставляется из upstream маршрута), предпросмотр показывает и итоговый ключ (`Bearer`/`x-api-key` + маска префикса); «Сделать по умолчанию» вычищает из `llm.router.defaultChain` записи несуществующих провайдеров; `openai-chat`-адаптер авторетраит 400 «use max_completion_tokens» новых моделей OpenAI.
