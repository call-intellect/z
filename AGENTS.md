...
- **serena** — семантический анализ кода (LSP-based: поиск символов, навигация, рефакторинг), инструменты `mcp__plugin_serena_serena__*`. Плагин установлен и включён, запускается через `uvx` (`git+https://github.com/oraios/serena`). **Проект `z` зарегистрирован** (2026-07-08): конфиг в `.serena/project.yml`, язык `typescript`, кодировка `utf-8`; `cache/` и `project.local.yml` серена игнорит своим `.serena/.gitignore`. **Гочча: серена НЕ регистрирует репо сама** — пустой список проектов `[]` / «No active project» лечится вызовом `activate_project` с путём `/Users/tozix/dev/work/z`. Если в сессии нет `mcp__plugin_serena_serena__*` — сервер упал на старте по одной из двух причин: **(а)** `spawn uvx ENOENT` — нет `uv` на PATH процесса ZCode (`brew install uv`); **(б)** `connection timed out after 30000ms` — холодный старт превысил 30-с MCP-таймаут; лечится прогревом кэша `uvx --from git+https://github.com/oraios/serena serena --version` и перезапуском ZCode. Онбординг-память серены (`write_memory`) **дублирует** `second-brain/` — не веди там архитектурное описание, только точечные подсказки LSP.
