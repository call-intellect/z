...
### B7 — Ф8 свайп внутри экрана (опц., низкий приоритет) `[ ]` — ОТЛОЖЕНО строкой в `04_не-сделано` (явно «низкий приоритет, опц.»; навигация работает bottom-tabs; свайп — вторичный UX-enhancer, требует новой зависимости `embla-carousel-react`)
...
### C1 — Ф9/D8 уведомление «встреча без записи — анализа не будет» `[x]` (ТЗ написано: `plans/tz/2026-06-11-meeting-no-recording-notification.md`; реализация ЖДЁТ «да» владельца на канал — рекомендация `conversational meeting.no_analysis`)
...
### C2 — Ф10 pluralRu консолидация (опц.) `[ ]` — ОТЛОЖЕНО строкой в `04_не-сделано` (owner Р7: фоновая уборка, нулевая видимая ценность, риск регрессии в `contribution.ts`; не делаем рискованный рефактор в конце большого свипа)
...
### C3 — Ф8/D5 мини-дизайн слияния «Входящие+Подтверждения» (опц.) `[x]` (design-doc написан: `plans/analysis/2026-06-11-inbox-actions-merge-mini-design.md`; находка: «Входящие» перегружено в 3 местах; рекомендация — переименование, не слияние; ждёт решения владельца)
