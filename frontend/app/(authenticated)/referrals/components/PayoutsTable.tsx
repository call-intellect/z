 ...
 *
 * Редизайн B10: переведена на `ModernTable` (стеклянная карточка нового
 * языка дашбордов). Тон статуса — через парные токены `STATUS_TONE`
 * (тот же примитив, что `StatusPill`), но с осмысленной подписью статуса
 * выплаты (Ожидает / Выплачено / Отменено), а не обобщённым «ок/риск».
 */
...
/**
 * Плашка статуса выплаты в стиле `StatusPill` (парные токены `STATUS_TONE`),
 * но с осмысленной русской подписью статуса вместо «ок/внимание/риск».
 */
...
function toneFromColor(c: 'green' | 'amber' | 'red'): 'ok' | 'warning' | 'risk' {
