...
**Источник:** ТЗ [`plans/tz/2026-07-03-llm-cost-dashboard.md`](../../plans/tz/2026-07-03-llm-cost-dashboard.md) + архитектура [`plans/architecture/2026-07-03-llm-cost-dashboard.md`](../../plans/architecture/2026-07-03-llm-cost-dashboard.md). Подключает первого читателя к `AiCostDaily` (наполняется `DailyCostAggregatorCron` с 24 мая 2026 — до этого ТЗ ни один экран её не читал) и консолидирует 8 разрозненных экранов расхода на LLM в один.

- **`backend/src/modules/admin/economics/llm-cost-module-map.ts`** — статическая таксономия `taskType → LlmCostModule` (4 модуля: `memory_graph`/`extraction`/`agent`/`other`), 176 явных записей + `resolveLlmCostModule()` с `?? 'other'` фолбэком для taskType, появившихся после аудита.
- **`backend/src/modules/admin/economics/llm-cost-dashboard.service.ts`** + **`.controller.ts`** — `LlmCostDashboardService` (6 методов: `overview`/`modelDetail`/`moduleDetail`/`companies`/`companyDetail`/`taskTypeDetail`), все запросы через `prisma.aiCostDaily.groupBy`/`aggregate` (не `$queryRaw` — модель маленькая, типобезопасный `groupBy` достаточен); `sharePct` считается в TS после запроса, не в SQL. `LlmCostDashboardController` — `@ApiExcludeController`, `CookieAuthGuard+SuperAdminGuard+SuperAdminAuditInterceptor` (без `TenantGuard` — суперадмин кросс-tenant), зарегистрирован в `admin.module.ts`. DTO — `backend/src/modules/admin/dto/llm-cost-dashboard.dto.ts` (Zod). Эндпоинты — [[../01_projects/api-layer]] §«Расход на LLM».
- **`backend/scripts/backfill-ai-cost-daily-gap.ts`** — разовый бэкафилл `AiCostDaily` за 2026-05-09..2026-05-23 (период до появления ночного крона, раньше 9 мая исходных данных `AiUsageLog` физически нет), идемпотентен (`@@unique` на `AiCostDaily` + pre-check по `count`), зарегистрирован в `apply-prod-deploy.ts STEPS` (`phase:'backfill'`, `skipBootstrap:true`).
- **Фронт** — `frontend/app/(admin)/admin/analytics/llm-cost/{page,LlmCostDashboardClient}.tsx` + `frontend/src/api/admin-llm-cost.api.ts` + `frontend/src/domain/admin-llm-cost.ts`. Уровень выбирается query-параметрами `?view=overview|model|module|companies|company&id=&period=` на одном роуте, не 5 отдельными app-router путями (публичный контракт навигации, на него ссылаются хирургические правки ниже). Переиспользует существующие `AreaTrend`/`AdminSparkline`/`AdminSection`/`useAdminQuery`/`AdminCsvDownloadButton` — новых UI-примитивов не заводили.
- **Консолидация 8 старых экранов:** полный редирект — `/admin/analytics/economics`, `/admin/analytics/functions` (список); хирургия, денежная часть вырезана / остальное осталось — `/admin/analytics/orgs` (`admin-orgs.service.ts::listOrgs()` больше не считает `aiUsageLog.groupBy`, поля `costUsdInPeriod`/`callsInPeriod` убраны), `/admin/economics/orgs/[id]` (расчёт расхода и топ-задачи убраны в пользу ссылки; `UnitEconomicsService.getOrg()`/`OrgEconomicsController` self-service `/admin/org/economics` НЕ тронуты — сознательно вне scope), `/admin/analytics/functions/[taskType]` (дублирующий редактор цепочки моделей — вызывавший тот же `putChain`, что и «Роутинг моделей», — убран; блок «Последние вызовы» остался); мёртвый код удалён насовсем — `admin-usage.service.ts::getUsersUsage()`/`getFunctionsUsage()`, `UnitEconomicsService.getGlobal()`+роут `unit-economics/global`, 3 redirect-заглушки (`/admin/economics`, `/admin/usage/functions(+[taskType])`, `/admin/usage/users`). UI — [[../01_projects/admin]] §«/admin/analytics/llm-cost».
