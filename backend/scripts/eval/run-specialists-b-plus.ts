...
const REPORT_PATH = path.resolve(
  SCRIPT_DIR,
  '../../test/eval/specialists-experiment/reports/variant-b-plus.json',
);
...
interface Block {
  ...
  signalType: string;
  tags: string[];
  evidence: { quote: string; speaker: string };
...
function blocksToContext(blocks: Block[]): string {
  return blocks
    .map(
      (b) =>
        `[BLOCK:${b.id}] (signalType=${b.signalType}, persons=${b.mentionedPersons.join(',') || '-'})\n  ${b.name}\n  В: ${b.criticalQuestion}\n  О: ${b.trustedAnswer}\n  Цитата (${b.evidence.speaker}): «${b.evidence.quote}»`,
    )
    .join('\n\n');
}
...
async function main() {
  ...
  const resp = (await client.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: 'user',
        content: `Все блоки встречи «${fixture.meetingTitle}» (${fixture.blocks.length} шт):\n\n${blocksToContext(fixture.blocks)}\n\nВажно: верни через submit_all_8_entities.`,
      },
    ],
    max_tokens: 32000,
    tools: [TOOL],
  ...
  const counts =
    output && typeof output === 'object' && !error
      ? {
