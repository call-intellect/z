...
- **`ChatboxChat`** — чат (тред), зеркало 1:1: денорм `channelType`/`customerExternalId`/`responsibleExternalId`, `status` (enum active/closed), `isGroup Boolean @default(false)` (производный флаг группового чата — `true`, если в чате >1 различного CLIENT-отправителя; проставляется синком, миграция `20260624160000_chatbox_chat_is_group`), `externalCreatedAt/UpdatedAt`, `lastMessageAt`, `messageCount`. Relations `sessions`/`messages`. Индексы по `customerExternalId`/`lastMessageAt`/`status`.
