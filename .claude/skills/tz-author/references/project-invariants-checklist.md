...
## Флаги — Ship-On (выкатываем включённым; CLAUDE.md принцип 8)
- [ ] Готовая фича выкатывается ВКЛЮЧЕННОЙ. «Дефолт OFF, владелец включит потом» и фаза «понаблюдаем → включим» — ЗАПРЕЩЕНЫ (не готова включиться → не готова к выкату).
- [ ] Если флаг нужен — это ровно один из двух видов: (а) аварийный рубильник (kill-switch): фича ON, рубильник лишь для экстренного выключения при инциденте, действий владельца не требует; (б) решение владельца: изменение, необратимо меняющее что видят/могут люди или тратящее/блокирующее деньги — выкатывать только с заданным владельцем параметром (матрица доступа, сумма лимита) и сразу включать; нет параметра → не выкатывать.
- [ ] Любой флаг зарегистрирован строкой в `docs/operations/feature-flags.md` (флаг · тип · состояние · если решение владельца — что ждём). Флаг «на всякий случай» не вводить.
...
| Новый флаг (kill-switch / решение владельца) | `docs/operations/feature-flags.md` | Шаг 1 |
