...
describe('PersonsController (IDOR fence)', () => {
  ...
  it('happy list: tenantId из @CurrentOrg в сервис', async () => {
    ...
    expect(persons.list).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'org-A' }));
  });

  it('byId 403 cross-tenant', async () => {
    const { ctrl } = build({ canRead: false });
    await expect(ctrl.byId('p-foreign', userA, 'org-B')).rejects.toBeInstanceOf(ForbiddenException);
  });
});
