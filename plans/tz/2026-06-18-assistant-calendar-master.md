...
## Фаза 2 — «Сейчас» (дата/время/TZ) в контекст помощника (баг «завтра») `[ ]`
...
## Фаза 5 — Различение «контрагент / о ком встреча» vs «место» (главный запрос владельца) `[ ]`
...
## Фаза 6 — Онлайн-пометка: видеокомната только по явному «онлайн» `[ ]`
