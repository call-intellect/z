...
const SEEDS: SettingSeed[] = [
  {
    key: 'betaOps.monthlyDigestEnabled',
    value: true,
    category: 'operations',
    section: 'monthly_digest',
    severity: 'medium',
    description:
      'Kill-switch месячного крона «Месяц компании» (1-го числа ~06:00 МСК). False → крон в no-op без рестарта.',
  },
  {
    key: 'betaOps.monthlyDigestLocalHour',
    value: 6,
    category: 'operations',
    section: 'monthly_digest',
    severity: 'low',
    description: 'Час (МСК, 0-23) генерации «Месяца компании» 1-го числа.',
  },
];
...
async function main(): Promise<void> {
  console.log('=== seed-admin-setting-month-company START ===');
  ...
  console.log('=== seed-admin-setting-month-company DONE ===');
}

main()
  .catch((err) => {
    console.error('seed-admin-setting-month-company FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
