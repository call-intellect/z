...
### ⚠️ Замер recall (2026-07-04, «Стрела») — данные есть, лифта НЕТ (честно)
...
**Валидация на «Стреле» (dev, 2026-07-04):** проба выбора таблиц `_probe-table-context.ts` — **17/21 правильные таблицы** (было 4/21, почти все — не той таблицы); «Обещания» больше НЕ перехватывает риск/blocker-вопросы (blockers→«Реестр рисков», goals→«Цели и метрики»); вагые/фактовые вопросы → пусто (fail-closed). `entityId` покрытие: **risks 27/27, ideas 17/17, promises 43/43 = 100%** (hypotheses/okr null — у Goal/Experiment нет entity в графе, валидный null). typecheck+lint+build+168 тестов зелёные. **Остаток (owner-gate):** стенд-A/B recall (Проба B, статзначимость) → снять прод-гейт; relation/rollup/views/мульти-person — отдельная волна (не соревнуемся в ширине конструктора; см. анализ §4).
