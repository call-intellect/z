...
export function SkillTraitConceptsClient() {
  ...
  const detailSwr = useSWR(
    ...
    selectedId ? () => adminSkillTraitConceptsApi.detail(selectedId) : null,
  );

  const isForbidden =
    listSwr.error instanceof ApiError && listSwr.error.code === "forbidden";
  const errorMessage =
  ...
  return (
    ...
    >
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto text-xs text-fg-tertiary">Найдено: {total}</div>
      </div>
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3 text-sm">
          <div>
            ...
            будет помечен «слит в другой», а все его {concept.traitCount}{" "}
            наблюдений перейдут к выбранному.
          </div>
          <div>
            <label className="text-xs text-fg-secondary">
              Целевой смысловой блок
            </label>
