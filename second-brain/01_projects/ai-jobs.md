...
| **operations (β-8.3, 2026-05-25; расширен «День компании» 2026-06-29)** | `operations-daily-digest` | Seed — `backend/scripts/seed-llm-task-routes-beta-8-3.ts`. **С «Дня компании» (2026-06-29):** capable-вызов `deepseek-v4-pro` (responseFormat `json_schema` strict, `reasoningEffort high`, `maxTokens 8000`) вместо прежней markdown-сборки — ОДИН вызов выдаёт строгий JSON: вердикт дня (4 оси) + письмо-проза + дневной компас (`DailyDigestService.generate` + `daily-digest.prompt.ts`). Парс устойчивый (`extractDayCompanyResponse`: `toolCalls[0].input` И text, снятие обёртки `{result:{...}}`; Zod снисходительный с `.catch`). Post-LLM `clampVerdict` (критич. клиентский сигнал ⇒ ось «Клиенты» ≠ ok). При провале LLM/JSON — «сухой» fallback (NULL новых полей). Крон `OperationsDailyDigestCron` перенесён `0 22 * * *` → `0 3 * * *` (03:00 UTC, после ночных синков). Поля persist в `DailyOperationsDigest.{verdictJson,letterJson,goalAlignmentDayJson}` (см. [[../02_architecture/data-model]], [[director-dashboard]] §«День компании»). |
