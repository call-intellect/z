...
| Ф6 — Мобильное приложение | 🟡 ЧАСТИЧНО (backend DONE, app scaffold) | R21, R22, R34 | **Ф6a (backend, DONE+verified):** `PushToken` (apns/fcm/rustore/webpush) + транспорт-агностичный `PushService` (нет кредов транспорта → no-op, ядро не падает — R34; FCM не фундамент), APNs-JWT/FCM-v1/RuStore-каркас, push-канал conversational (ФЗ-41 без тела), `POST /account/delete` (App Review 5.1.1v), `UserBlock`+`MessageReport` (UGC 1.2), флаг `CHAT_PUSH_ENABLED`, крутилки. vitest 479, typecheck/build/bootstrap зелёные. **Ф6b (app scaffold, tsc-clean):** `kora-mobile/` (Expo SDK 56, expo-router, нижние табы Сообщения·Поддержка·Кора·Я, экраны Messages/Chat/Support/Kora/Settings, push-регистрация, offline-outbox, удаление аккаунта+block+report, store-compliance STORE.md). npm install + tsc --noEmit зелёные. **⚠ OWNER-GATED (нельзя в этой среде):** нативная сборка/eas, сертификаты APNs/FCM/RuStore, store-аккаунты+формы privacy/Data-safety, reliability-gate пуша на устройстве, иконки/сплэш. Внесено в `04_не-сделано`. |
| Ф7 — Доводка | ✅ DONE | R23–R26 | Ф7a: автоудаление `MessageRetentionService` (крутилка `message_retention_days`, 0=выкл; pre-ingest в граф ПЕРЕД soft-delete — R23) + HR-подписки (EventEmitter `membership.created/removed` → `ConversationMember(source='auto')` в обязательном канале — R24). Ф7b: опросы `Poll/PollOption/PollVote` (`closePoll`→`IdeaBlock(signalType='decision')`), huddles `HuddleService.startHuddle` (Meeting+LiveKit+`huddleConversationId`+системное сообщение), закрытие huddle→запись+резюме в чат (R26, расширен Ф5a-hook). Фикс: классификация push-ENV (Ф6a) в `env-classification`. GIN-поиск (R25) — в Ф4a. vitest messaging+meetings 295, env-guard 3/3, typecheck/build зелёные. |
