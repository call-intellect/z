...
/**
 * SBA β-7 — REST `/api/v1/documents/:id/use-cases`.
 *
 * Отдельный контроллер (а не extension `DocumentsController`), чтобы не
 * создавать DI-цикл `DocumentsModule → BrandVoiceModule`. RBAC: action `write`
 * на `document` (admin/owner Org; manager — self), плюс автоматическое
 * обновление gauge `brand_voice_corpus_size` если убрали/добавили
 * 'brand_corpus'.
 */
@ApiTags('documents')
...
export class DocumentUseCasesController {
  ...
  async patch(
    ...
    // RBAC: writer на document. Self-policy (manager self) проверим через
    // resourceOwnerId — но Document.uploaderId — Person.id, а не User.id,
    // поэтому self-проверка тут не применима без mapping. Достаточно
    // owner/admin (manage всех документов Org).
    ...
    if (!ok) {
      throw new ForbiddenException({
        ...
        error: {
          code: 'forbidden',
          message:
            'Управлять use-case-метками документов могут только владелец/администратор',
        },
      });
    }
    try {
