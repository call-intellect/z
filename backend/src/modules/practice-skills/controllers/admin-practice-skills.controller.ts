...
function toDto(s: PracticeSkill): PracticeSkillDto {
  return {
    ...
    trigger: s.trigger,
    steps: Array.isArray(s.steps) ? (s.steps as unknown as PracticeSkillStepDto[]) : [],
    examples: Array.isArray(s.examples) ? (s.examples as unknown as PracticeSkillExampleDto[]) : [],
    redFlags: Array.isArray(s.redFlags) ? (s.redFlags as unknown as string[]) : [],
    status: s.status,
