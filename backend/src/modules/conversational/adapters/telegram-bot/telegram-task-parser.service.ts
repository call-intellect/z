...
export class TelegramTaskParserService {
  ...
  async classifyReply(args: {
    ...
    try {
      result = await this.llm.call({
        ...
        dataClass: 'internal',
        sourceRef: args.relatedIssueId ? { type: 'issue', id: args.relatedIssueId } : null,
      });
    } catch (err) {
      this.logger.warn(
        {
  ...
  private async callTaskExtract(args: {
    ...
    const systemPrompt = guardOn ? withInjectionGuard(args.systemPrompt) : args.systemPrompt;
    const userMessage = guardOn ? wrapUserData(args.userMessage) : args.userMessage;
    try {
      const result = await this.llm.call({
      ...
      const confidence = typeof json.confidence === 'number' ? clampConfidence(json.confidence) : 0;
      return {
  ...
  private async createIntakeFromParsed(args: {
    ...
    const suggestedProjectId = await this.resolveProjectId(tenantId, parsed.suggestedProjectHint);
      ...
      const created = await this.prisma.intakeIssue.create({
        data: {
          ...
          expiresAt: computeExpiresAt(
            this.config?.pendingActions.intakeTtlDays ?? INTAKE_TTL_DAYS_FALLBACK,
          ),
        },
        select: { id: true },
      });
    ...
    const autoTriageEligible = confidence >= TelegramTaskParserService.AUTO_TRIAGE_THRESHOLD;
  ...
  private async resolveAssigneeId(
    ...
    try {
      ...
      const exact = candidates.find((c) => c.name.toLowerCase() === trimmed.toLowerCase()) ?? null;
      const single = candidates.length === 1 ? candidates[0] : (exact ?? null);
  ...
  private async loadOrgContext(tenantId: string, _userId: string): Promise<OrgContext> {
    try {
...
Тон: дружелюбный, без официоза, без эмодзи внутри списка. Не используй markdown ** (звёздочки) — только HTML-теги Telegram. Не превышай 3000 символов.`;
  ...
  private renderDigestFallback(payload: TelegramDigestPayload): string {
    ...
    if (payload.inProgress.length > 0) {
      ...
      for (const i of payload.inProgress.slice(0, 8)) {
        parts.push(`• <code>${escapeHtml(i.identifier)}</code> «${escapeHtml(i.title)}»`);
      }
      parts.push('');
    }
    ...
    if (payload.sprint) {
      ...
      const hyp = s.hypothesisText ? truncate(s.hypothesisText, 80) : '—';
      parts.push(`🎯 <b>Спринт «${escapeHtml(s.cycleName)}»</b> — гипотеза: ${escapeHtml(hyp)}.`);
      const signalsLine =
        s.signals.length > 0
      ...
      parts.push(`▶️ Следующий шаг: ${s.nextAction ? escapeHtml(s.nextAction) : '—'}.`);
    }
    return parts.join('\n').slice(0, 3000);
  }
  ...
  private heuristicReplyClassify(text: string): TelegramReplyClassification {
    ...
    if (/(\+ ?1 ?ден|завтра|послезавтра|передвин|сдвин|перенес)/i.test(lower)) {
      return { kind: 'status_command', action: 'postpone_one_day' };
    }
    if (/(не сделаю|отмен|откажусь|отказ|не успе[юе]|не возьму)/i.test(lower)) {
...
export interface TelegramTaskParseResult {
  ...
  reason: 'ok' | 'empty_text' | 'llm_failed' | 'intake_create_failed';
}
