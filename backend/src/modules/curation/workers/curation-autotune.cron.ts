...
import { CurationService, KILL_SWITCH_PROVISIONAL_THRESHOLD } from '../services/curation.service';
...
export class CurationAutotuneCron {
  ...
  async runForOrg(
    ...
    for (const stat of auditStats.items) {
      ...
      const currentProvisional = fresh.provisionalThresholdByType?.[stat.resourceType];
      if (
  ...
  private decideDirection(overrideRate: number, maxOverride: number): 'up' | 'down' | null {
  ...
  private async applyAdjustment(args: {
    ...
    const currentAuto = fresh.autoThresholdByType?.[resourceType] ?? fresh.autoThreshold;
    const deepForType =
      fresh.deepReviewThresholdByType?.[resourceType] ?? fresh.deepReviewThreshold;
    const lowerBound = Math.max(thresholdMin, deepForType);

    const raw = direction === 'down' ? currentAuto - step : currentAuto + step;
