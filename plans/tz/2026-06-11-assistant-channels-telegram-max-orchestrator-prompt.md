...
## Что реализуем
ТЗ: `plans/tz/2026-06-11-assistant-channels-telegram-max.md` (status: ready-to-implement). Идея: усилить существующего AI-помощника (`concierge`) и сделать Telegram/MAX **каналами-окнами** к нему — один мозг, много окон. 6 фаз, **один прод-релиз** (Ship-On, решение владельца В2). НЕ дублировать tool-логику в адаптерах (второй мозг — запрещено).
