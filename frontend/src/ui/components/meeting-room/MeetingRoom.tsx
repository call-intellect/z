...
function VideoArea() {
  ...
  const carouselTracks = tracks.filter(
    (track) => !isSameTrack(track, focusTrack),
  );
    ...
    if (hasSubscribedShare && autoPinnedRef.current === null) {
      ...
      !screenShareTracks.some(
        (track) =>
          ...
          autoPinnedRef.current?.publication.trackSid,
      )
    ) {
    ...
    <LayoutContextProvider value={layoutContext}>
      {focusTrack ? (
        ...
        <GridLayout
          ...
          className={
            isSolo
              ? "lk-grid-layout kora-video-grid kora-video-solo"
              : "lk-grid-layout kora-video-grid"
          }
        >
          <ParticipantTile />
        </GridLayout>
