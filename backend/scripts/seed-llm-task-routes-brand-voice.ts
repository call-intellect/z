...
const SEEDS: TaskRouteSeed[] = [
  {
    ...
    playbookSection:
      '§β-7 — высокая точность нужна (style профиля влияет на весь генерируемый контент).',
    chain: [
      {
        tier: 'primary',
...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log('=== seed-llm-task-routes-brand-voice DONE ===');
}

main()
  .catch((err) => {
    // eslint-disable-next-line no-console
    console.error('seed-llm-task-routes-brand-voice FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
