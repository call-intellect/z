...
- **Шаг 7 — Seed (идемпотентный, УЖЕ в STEPS `phase:'seed-base'`):** `scripts/seed-admin-setting-task-solution.ts` — 7 ключей (kill-switch сущности + 5 крутилок + `taskSolution.refineEnabled` — kill-switch LLM-уточнения материализатора, цикл 1 2026-07-07: гейт содержательности + рост клонов соисполнителей), защищает admin-edited (`updatedBy !== 'system'`). **Цикл 2 2026-07-08: `taskSolution.minSignalChars` 40→15** (обновится на не-admin-edited орг). Доедет `docker compose exec backend bun run scripts/apply-prod-deploy.ts --mode update`.
