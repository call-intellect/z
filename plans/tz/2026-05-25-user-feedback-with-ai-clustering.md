...
- [ ] **Фаза 2 — Пользовательские эндпоинты и rate-limit.** `POST /feedback`, `GET /feedback/my`, `GET /feedback/my/limit`. `FeedbackRateLimitGuard` с Redis. Юнит-тесты на guard и сервис. Swagger.
- [ ] **Фаза 3 — Frontend пользовательский.** `/feedback` страница, форма, история, счётчик лимита. Проверить ручками в браузере.
- [ ] **Фаза 4 — Промпт + LLM-router taskType.** Добавить `feedback.cluster` в registry. Seed fallback-цепочки. Zod-схема валидации. Юнит-тест с фикстурой.
- [ ] **Фаза 5 — Digest-сервис + воркер + cron.** `feedback-digest.service.ts` с реальной логикой, fallback по моделям, транзакция. BullMQ-processor, cron 01:00 UTC. Redis-lock. Юнит + интеграционные тесты. Ручной запуск через `/admin/feedback/digest/run`.
- [ ] **Фаза 6 — Админские эндпоинты.** GET список/детали блоков, GET items, PATCH rename, POST merge / archive / unarchive. Swagger. Юнит-тесты.
- [ ] **Фаза 7 — Frontend админский.** `/admin/feedback` дашборд, фильтры, таблица. `/admin/feedback/[topicId]` детали, items, разворот сообщения.
- [ ] **Фаза 8 — Действия в админке.** Диалоги rename / merge / archive, кнопка «Запустить обработку сейчас». Ручная проверка флоу.
- [ ] **Фаза 9 — Second-brain и наблюдаемость.** Заполнить все заметки в second-brain. Добавить prom-метрики `feedback_digest_runs_total`, `feedback_digest_messages_processed`, `feedback_digest_new_topics`, `feedback_digest_failed_runs`. Логи через `pino` с structured-полями.
