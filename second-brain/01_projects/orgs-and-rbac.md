...
- `/settings/organization` — только вкладка «Информация» (owner-only) с 2026-06-04. **Управление участниками и приглашениями переехало в раздел «Команда» (`/structure`)** — ростер `GET /orgs/:id/team-roster`, inline-смена роли / удаление / приглашение / перевыпуск / отзыв, карточка сотрудника `/structure/persons/[id]` (включая персональные override доступа). См. [[frontend-pages]] §«Команда».
  - **Дедуп + статус ростера (2026-06-10, ТЗ [`meeting-stuck-and-team-roster-fixes`](../../plans/tz/2026-06-10-meeting-stuck-and-team-roster-fixes.md) Ф1).** `listTeamRoster` теперь дедуплицирует строки по нормализованному (`trim().toLowerCase()`) непустому email — аккаунтная карточка (`userId`) и ручная карточка («Добавить сотрудника», `userId=null`) на один email схлопываются в одну строку (база — носитель аккаунта, должность/отдел/человеческое имя дополняются из ручной). Статус строки: `invitationStatus = membership ? 'accepted' : (invitations[0]?.status ?? 'none')` — активный участник (владелец/админ, вступивший не через приглашение) теперь «активен», а не «не приглашён». Дедуп на чтении — страховка; реальные дубли в БД схлопывает backfill + partial unique `persons_tenant_email_active_uniq` (см. [[../02_architecture/data-model]] §Person).
