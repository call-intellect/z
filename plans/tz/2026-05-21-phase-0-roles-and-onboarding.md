...
**Файл:** `plans/tz/2026-05-21-phase-0a-data-model-and-graph-infra.md` _(draft, создан 2026-05-21)_
...
**Покрывает строки матрицы:** 1–25 + 43–62 (группа H) + 83 (расширение `/api/v1/search` на сущности группы А — backend-часть).

---

### 0b — Document-ingest pipeline + extraction типизированных сущностей

**Файл:** `plans/tz/2026-05-21-phase-0b-document-ingest.md` _(draft, создан 2026-05-21)_
...
**Покрывает строки матрицы:** 26–31 + 63–66 (расширения extraction и entity resolution) + 69 (text.adapter из аналитики ЛК §11) + 82 (выдача readonly-провенанса группы Б в API документа).

---

### 0c — Мастер знакомства + личный кабинет Фазы 0 (frontend)

**Файл:** `plans/tz/2026-05-21-phase-0c-onboarding-wizard-frontend.md` _(draft, создан 2026-05-21)_
...
**Покрывает строки матрицы:** 32–36 + 67–68 (заделы под γ в навигации) + 70–83 кроме 79 (расширения скопа из аналитики ЛК §11; 79 — frontend-часть; 69 уходит в 0b; 82 — UI часть; 83 — UI часть).

---

### 0d — RoleProfileAgent MVP

**Файл:** `plans/tz/2026-05-21-phase-0d-role-profile-agent.md` _(draft, создан 2026-05-21)_
...
**Покрывает строки матрицы:** 37–42 + 79 (часть про логику 409 на стороне API/worker для rebuild).
...
### K. Расширения скопа из аналитики ЛК (`2026-05-21-user-cabinet-design.md` §11)

Решения из аналитики ЛК, которые расширяют Фазу 0 относительно изначального скопа этого зонтичного ТЗ. Все добавлены сознательно после ревью аналитики — закрывают реальные пробелы (старый долг `/dump`, member-landing, pre-marketing γ-разделов, multi-org для пилотов-консультантов, действия на сущностях после wizard'а).

| № | Решение / артефакт | Источник | Где | Статус |
|---|---|---|---|---|
| 69 | Новый адаптер `text.adapter.ts` в `backend/src/modules/ingest/adapters/` — обработка свободного текста из `/dump`. Pipeline: `block-ingest` → `block-distill` → `entity-resolver`, как у `document.adapter`, без парсинга бинарных файлов | аналитика ЛК §11, решение 6 | 0b | [ ] |
| 70 | Страница `/dump` (минимальная): textarea «Расскажи всё, что хочешь зафиксировать» + кнопка «Сохранить» + feedback «Записал». Без голоса, файлов, AI-feedback — это всё в γ | аналитика ЛК §11, решение 6 | 0c | [ ] |
| 71 | Страница `/me` (минимальная): моя должность + мой отдел + мои документы + мои встречи. Все секции — агрегация по существующим API. Доступна всем (для admin — не landing, но работает) | аналитика ЛК §11, решение 4; §7.2 | 0c | [ ] |
| 72 | Единый компонент `<ComingSoonPage section="processes\|regulations\|policies\|metrics">` в `frontend/src/ui/components/coming-soon/` — preview для γ-разделов. Счётчик «N собрано» виден admin/owner/super_admin и скрыт при N=0. В γ заменяется реальной страницей на тех же URL | аналитика ЛК §11, решение 3; §7.7 | 0c | [ ] |
| 73 | Multi-org switcher в [`Header.tsx`](../../frontend/src/ui/components/app-shell/Header.tsx) — dropdown с активной Org (truncated 20 симв) + chevron + список других Org пользователя. Soft-reload через `mutate` ключевых SWR-кешей при переключении. Скрыт внутри wizard'а | аналитика ЛК §11, решение 5; §13.3 | 0c | [ ] |
| 74 | Пересборка [`Sidebar.tsx`](../../frontend/src/ui/components/app-shell/Sidebar.tsx) на 3 смысловые группы: «Компания» / «Оперативка» / «Настройки». URL не меняются. Свёрнутая подгруппа «Будет в следующей фазе» с 4 disabled-γ-пунктами | аналитика ЛК §5, решение 1; §9 | 0c | [ ] |
| 75 | Переименование лейблов в Sidebar: «Мои встречи» → «Встречи»; «Дамп мысли» → «Дамп»; «AI-чат» → «Помощник компании» (правило русского UI). Глоссарий пополняется новым термином «Помощник компании» | аналитика ЛК §9 п.5; §10 | 0c | [ ] |
| 76 | Состояние `disabled-γ` для `SidebarNavLink`: новый пропс `comingSoon?: boolean`, иконка часов (`Clock4` 16px), `opacity-60` + `text-muted-foreground`, переход на preview-страницу. Не путать с tariff-locked (иконка замка). Не путать с `gateFeature` | аналитика ЛК §5, решение 3; §13.5 | 0c | [ ] |
| 77 | Действия на сотруднике в `/structure?tab=persons`: «Пригласить» (создаёт `Membership` + email через `orgs/invitations`), «Переназначить должность» (закрывает старую `executes_role` `validTo=now`, создаёт новую), «Удалить из компании» (soft-delete `Person` + закрытие связей). Статус приглашения как колонка | аналитика ЛК §7.3; решение по §8 шаг 3 | 0c | [ ] |
| 78 | Действие «Загрузить должностную инструкцию» на табе «Должности» в `/structure?tab=roles` — эквивалент шага 4 wizard'а, но для отдельной должности после прохождения wizard'а. Создаёт `Document` + связь `JobDescription → derived_from → Document` | аналитика ЛК §7.3 | 0c | [ ] |
| 79 | Кнопка «Пересобрать карту» в `/roles/:id`: 60-секундный debounce на UI + 409 `Conflict` от API если job уже в очереди или выполняется. Idempotency на worker — по `roleId+buildVersion` | аналитика ЛК §7.5 | 0c (UI) + 0d (логика 409) | [ ] |
| 80 | Wizard однократен: доступен только при `Department.count = 0` в Org. После прохождения переход на `/onboarding/company/*` редиректит на `/dashboard`. Состояние wizard'а — по реально созданным сущностям, без отдельной модели `OnboardingState` | аналитика ЛК §8; §13.1 | 0c | [ ] |
| 81 | Wizard живёт в [`frontend/app/(authenticated)/onboarding/company/`](../../frontend/app/(authenticated)/onboarding/) (рядом с существующим `change-password/`) — пять подмаршрутов `step-1` … `step-5` без AppShell (`hideShell` в layout, как `change-password`) | аналитика ЛК §13.4 | 0c | [ ] |
| 82 | Граница UI группы Б в Фазе 0: на странице документа `/documents/:id` сущности группы Б показываются **только как readonly-провенанс** (имя + тип + confidence), без ссылок на отдельные страницы сущностей и без CRUD. Полноценный UI группы Б — Фаза γ | аналитика ЛК §7.4 | 0b (выдача данных) + 0c (UI) | [ ] |
| 83 | CommandPalette ⌘K расширяется на новые сущности группы А: `Role`, `Department`, `Person`, `Document`, `RoleProfile`. Через существующий `/api/v1/search` (backend в 0a, frontend в 0c) | аналитика ЛК §4.4; §15 п.10 | 0a (API) + 0c (UI) | [ ] |
...
## 6. Принятые решения по открытым вопросам

Все 11 открытых вопросов закрыты решениями. Спорные эксперименты (#2, #8) — закрываются эмпирически в первый день соответствующего sub-TZ.

| № | Вопрос | Решение | Где зафиксировано |
|---|---|---|---|
| 1 | Парсер документов | `pdf-parse` + `mammoth` для MVP (in-process, без sidecar). Markdown — `marked`. Apache Tika — отложено; включаем если на 1-м пилоте упрёмся в качество таблиц или сканов | sub-TZ 0b |
| 2 | Apache AGE в managed-Postgres | **Preflight 0a.1.** Гипотеза: Yandex Managed не поддерживает AGE → ставим self-hosted PostgreSQL на проде (один кластер, DevOps-опыт есть). Dev — Docker `apache/age:PG16_latest` | sub-TZ 0a (preflight) |
| 3 | Cron `RoleProfileAgent` | `@Cron('0 */4 * * *')` — раз в 4 часа. Если LLM-стоимость растёт — on-demand only через POST `/rebuild` | sub-TZ 0d |
| 4 | CSV-импорт в wizard | Nice-to-have, не в MVP. Включаем при bottleneck >10 минут на 1-м пилоте | sub-TZ 0c §12.1 |
| 5 | Пустая карта должности | Пустой `summaryCache` + статус `forming`; `RoleProfileAgent` пропускает Role до накопления `N=5` блоков идей с пометкой `role_relevant` | sub-TZ 0d |
| 6 | Прогресс-эвенты документа | Polling 2 сек на детальной странице; на списке — `GET /api/v1/documents/processing` если >20 «в работе». Page Visibility API — останавливать на blur. SSE/WS — отложено | sub-TZ 0c §6.3 + sub-TZ 0b |
| 7 | Иерархия отделов в UI | Плоский список. `parentDepartmentId` существует в схеме, UI и enforcement — γ | sub-TZ 0c §6.2 |
| 8 | Разбивка extraction на проходы | **Эксперимент на старте 0b.** Сравниваем точность одного промпта vs последовательных (блоки → сущности → связи) на 5–10 реальных артефактах. Гипотеза: для встреч >30 минут — несколько проходов выгоднее | sub-TZ 0b (первая неделя) |
| 9 | LLM колеблется Process/Regulation/Policy | Сохраняем как `Process` с `confidence<0.6` + `ambiguousTypes` в metadata. `CuratorAgent` в Фазе δ разбирает | sub-TZ 0b |
| 10 | γ-разделы в навигации | Visible-but-disabled для всех; для admin — счётчик «N собрано», скрыт если N=0 | sub-TZ 0c §6.7 |
| 11 | Mission/Vision/Strategy автоизвлечение | Не в MVP. Слот существует, наполняется в γ через UI. Флаг `EXTRACTION_ENABLE_TOP_LEVEL=false` блокирует автоизвлечение в Фазе 0 | sub-TZ 0b |
...
- [ ] Все 83 строки матрицы прослеживаемости имеют статус `[x]` (секции A–K).
...
| **Нет fact-level ACL — финансовые/HR/стратегические встречи в Org с режимом `open` видны всем manager'ам** (включая `IdeaBlock`, `AiResult`, помощник компании выдаст любому через RAG) | высокая | Подробный разбор — [`plans/analysis/2026-05-21-cabinet-roles-value-and-access-model.md`](../analysis/2026-05-21-cabinet-roles-value-and-access-model.md) §5.2. В Фазе 0 закрываем **процедурно**: (1) для SMB 50+ чел — переключать Org в `strict` или ограничивать пилот руководящей командой; (2) sales-материалы не обещают fact-level защиты до Фазы γ; (3) помощник `/chat` фильтрует контекст по `visibilityMode` (зафиксировать тестом). Полноценное решение — `sensitivity` enum + `confidentialityAcl` в Фазе γ; `FactAcl` в Фазе β |
