/**
 * Unit-тесты центрального маппера статуса встречи и логики «развязки записи
 * от AI-статуса» (Фаза 11 «развязка записи от AI-статуса», addendum §B).
 *
 * Контекст: в `MeetingStatus` добавлен терминальный `ai_failed` — упала только
 * AI-ветка (транскрибация/отчёт), но запись в порядке. Видео должно
 * показываться по наличию записи, НЕ по статусу.
 */
...
describe('meetingStatusView', () => {
  ...
  it('ai_failed → русский лейбл «отчёт не удался» и тон warning (НЕ danger)', () => {
    ...
    // Парные токены: bg-chip-…-bg + text-chip-…-fg, без жёстких hex/slate.
  ...
  it('все chipClass — парные токены без жёстких цветов', () => {
    for (const s of MEETING_STATUSES) {
      ...
      // Никаких сырых hex или slate-классов.
...
describe('развязка записи от AI-статуса — показывать ли баннер «AI-отчёт не готов»', () => {
  // Чистый предикат, повторяющий логику AiFailedBanner на странице результата:
  // баннер показываем при ai_failed, либо при failed когда запись есть.
