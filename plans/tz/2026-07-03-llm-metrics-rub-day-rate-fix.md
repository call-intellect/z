...
Реализовано полностью, обе фазы, в один проход (два независимых суб-агента параллельно), приёмка проведена лично (не на веру): typecheck/lint backend+frontend — 0 ошибок, `ai-models.service.spec.ts` — 18/18 зелёных (включая 3 новых теста, в т.ч. ключевой негативный `costRub=100 ≠ costUsd*rate=90.5 → используется 100`), сид прогнан дважды локально (`created=2` → `created=0`, идемпотентность подтверждена), значения в dev-БД после сида совпадают с кодовыми дефолтами (`false`/`90`).

Коммиты:
- `f3aae431` — Фаза 1 (backend + frontend + тесты + оба plans-документа).
- `9ba907dd` — Фаза 2 (регистрация двух крутилок).

Не запушено — по правилу проекта push только по отдельному подтверждению владельца.

Побочные находки по ходу, не в scope этого ТЗ (зафиксированы отдельными строками в `second-brain/04_не-сделано/README.md`, не потеряны):
- `llm.budget.mtd_cache_ttl_sec` — тот же паттерн orphan-крутилки, не зарегистрирован.
- `AiCostDaily`/`DailyCostAggregatorCron` — готовая, но нигде не читаемая посуточная агрегация расходов — сильный кандидат под будущий многоуровневый дашборд расходов, чтобы не строить агрегацию с нуля.

Что НЕ сделано / не проверено:
- Прод-данные не проверялись (не было доступа) — гипотеза «курс двигался за период → цифра расходилась» подтверждена как логическая ошибка кода, но не подтверждена конкретным историческим примером на реальных прод-цифрах.
- `docs/operations/prod-deploy-log.md` — новую строку не добавлял: `seed-admin-settings.ts` уже зарегистрирован в `apply-prod-deploy.ts` `STEPS` (`phase: 'seed-base'`) предыдущими фичами, новый прогон подхватит наши 2 строки автоматически, отдельного шага в логе не требуется (нет новой миграции/новых ENV/новых очередей).
