...
## Фаза 0 — P0: починить краш создания задач (РАЗБЛОКИРОВКА) `[ ]`
...
## Фаза 1 — Три класса: идея(предложение) / задача(поручение) / решение(выбор) `[ ]`
...
## Фаза 2 — Actionable-решение → задача (сквозная гарантия, две оси) `[ ]`
...
## Фаза 3 — Закрытие из разговора (P7) + журнал хода («середина») `[ ]`
