...
| **`recording.faststart` (ТЗ 2026-06-03 recording-reliability, Фаза 3)** | webhook `egress_ended`(composite) через `AiQueueService.enqueueRecordingFaststart`, если `RECORDING_FASTSTART_ENABLED=true` | `faststart.worker` (`FaststartWorker`, `recordings/workers/`) | 1 | Переупаковка composite MP4 в faststart (`ffmpeg -c copy -movflags +faststart`) → перезалив по тому же S3-ключу. jobId=`faststart_<meetingId>` (dedup). Дефолт OFF (нужна прод-проверка + ffmpeg в образе). См. [[recording]], [[ai-jobs]]. |
