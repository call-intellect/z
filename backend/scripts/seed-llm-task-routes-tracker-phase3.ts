...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `=== seed-llm-task-routes-tracker-phase3 START (updateExisting=${updateExisting}) ===`,
  );
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log('=== seed-llm-task-routes-tracker-phase3 DONE ===');
}

main()
  .catch((err) => {
    // eslint-disable-next-line no-console
    console.error('seed-llm-task-routes-tracker-phase3 FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
