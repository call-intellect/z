...
Аналогия: наш `second-brain/` — личный блокнот разработчика с дисциплиной. Кора для клиента — AI-секретариат из 10 специалистов (в `knowledge-core/services`), общий куратор-редактор и архивариус, которые сами слушают каналы, сами раскладывают по полкам, сами рисуют связи, сами ловят противоречия, сами задают уточняющие вопросы и сами отвечают, когда у них спрашивают.
...
| 3.9 Experiments | Гипотезы/эксперименты (running → completed/dropped) с lessons | `knowledge-core/services/specialist-3-9-experiments.service.ts` |
| 3.14 Goals | Цели и стратегическое выравнивание | [[../01_projects/goals-and-strategic-alignment]] |
| 3.15 Tasks | Задачи из блоков (`action_item`) → трекер (IntakeIssue → Issue) | [[../01_projects/tracker]] |

> Нумерация с дырами: **3.8 Helpfulness** живёт отдельным модулем `modules/specialist-3-8-helpfulness` (SBA Wave 2, вне `core.specialist-routing`); **3.10–3.13 не существуют**.
...
LiveKit-встреча / Telegram / email / трекер / документ / in-app
   ...
   reframing.cron                ← раз в сутки 3:00, чистит слабые связи + decay
        │
        ▼
   core.specialist-routing — fan-out по 10 специалистам
        ├─ 3.1 Regulations
        ├─ 3.2 Knowledge Clone
        ├─ 3.3 Decisions
