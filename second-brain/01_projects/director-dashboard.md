...
## Дашборд-контроль на исполнении + честная метрика утечки (2026-06-27, Ф4)

Источник — ТЗ [`task-decision-execution-unified`](../../plans/tz/2026-06-27-task-decision-execution-unified-tz.md) Ф4. Идея: на дашборде решение — это **источник задач и недельный индикатор-утечка**, а не ежедневная очередь «доведи решение». Опирается на `Decision.impliesAction` (Ф2, см. [[../02_architecture/data-model]], [[ai-jobs]] §«Задача·решение·исполнение»).

- **`impliesAction` в метрике (R19):** `classifyImplementationStatus` += `impliesAction` — не-actionable решение («решили НЕ делать» / стратегия без конкретного дела) НИКОГДА не `stalled` (→ `not_started`). Знаменатель throughput (`baseWhere`) += `impliesAction: true` — считаем доведение только тех решений, что вообще влекут действие. `listDecisionsForMonth` (Value Recap / память) НЕ фильтруется по `impliesAction` — это ретро всех решений, а не знаменатель (поле лишь пробрасывается в classify).
- **`DecisionsWidget` (R20):** при ритме `today` рисует ОДИН индикатор-утечку «Решения без действия» (=`stalled.length`, после R19 = actionable-решения без заведённой задачи) + ссылку `/decisions`; список-очередь и throughput-процент остаются ТОЛЬКО в неделе/месяце (недельная аналитика). 0 застрявших → пустой-стейт «Все решения в работе».
- **Эндпоинты не тронуты (R21):** `decisions/throughput`, `decisions/stalled` остаются недельной аналитикой.
- **Старые решения** (`impliesAction=false` по дефолту миграции) выпадают из stalled/throughput до LLM-ре-экстракции — на почти-пустом проде приемлемо, бэкфилл вне scope (см. [[../04_не-сделано/README]]).
