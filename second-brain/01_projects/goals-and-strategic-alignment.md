...
- **Жизненный цикл:** AI-цель создаётся как `source='ai', promotionState='suggested'`; при `confidence≥` `goals.autoPromoteConfidence` (0.8) или повторном упоминании (dedup→promote существующей suggested) → `promotionState='active'`; иначе ждёт тапа owner'а. Cap = `goals.maxActiveGoalsPerHorizon` (7) active+suggested целей на горизонт (антиплодёж). Goal без embedding-колонки → KNN-дедуп через ILIKE-fallback.
- **F6 (2026-06-22) — цели больше не теряются молча.** (а) **no_owner-фолбэк:** нет owner-membership → `createdById` берётся как любой член Org (раньше `нет owner → skip` молча терял цель); (б) пороги извлечения вынесены в AdminSetting — `goals.minExtractConfidence` (0.4, ниже → отброс), `goals.autoPromoteConfidence` (0.8), `goals.maxActiveGoalsPerHorizon` (7); (в) тихие потери `no_owner`/`focus_cap` сделаны наблюдаемыми метрикой. ТЗ [`meeting-to-tracker-and-models-unified-fix`](../../plans/tz/2026-06-22-meeting-to-tracker-and-models-unified-fix.md) F6.
