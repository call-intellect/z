...
## Фаза 3 — BE: убрать решения из оперативных отчётов (дайджесты + «День/Неделя/Месяц компании») `[x]`
...
## Фаза 5 — FE: дашборд-поверхности решений `[x]`
...
## Фаза 6 — FE: дайджест-клиенты без решений `[x]`
...
## Фаза 7 — Р5: тихая пометка «необратимое» на карточке решения (память) `[x]`
...
## Фаза 8 — Синхронизация second-brain + прод-реестры `[x]`
