...
- **Эксперимент с конкретным действием → задача** (ТЗ [`2026-07-06-experiment-to-task`](../../plans/tz/2026-07-06-experiment-to-task.md), тот же контур «память + исполнение», что решение→задача и обещание→задача): `Experiment` (спец. 3-9) остаётся фактом памяти, но при конкретном действии combo/`block-ingest` вдобавок заводят задачу. Провенанс — `ExperimentTaskLink('derived')` (зеркало `DecisionTaskLink`), авто-деривится в `intake-auto-triage.worker` (`linkDerivedExperimentsForIssue`) по общим `sourceBlockIds`. Метрика `experiment_tasks_extracted_total{tenant_top,surface}`. Детали — [[ai-jobs]] §«Эксперимент с конкретным действием → задача».

## Задачи ВСТРЕЧИ: привязка к встрече + назначенец гостя (2026-07-06)

ТЗ [`plans/tz/2026-07-06-meeting-tasks-linkage-fix.md`](../../plans/tz/2026-07-06-meeting-tasks-linkage-fix.md), анализ [`plans/analysis/2026-07-06-meeting-tasks-not-linked.md`](../../plans/analysis/2026-07-06-meeting-tasks-not-linked.md), архитектура [`plans/architecture/2026-07-06-meeting-tasks-linkage-fix.md`](../../plans/architecture/2026-07-06-meeting-tasks-linkage-fix.md) (ветка `work/2026-07-02`, 5 коммитов). Владелец задач встречи — **комбо** `SpecialistsCombinedService` (после захода B задачи встреч извлекает combo, не снесённый `MeetingExtractActionsService`). Дефект: combo по ingest отчёта встречи (`channel=meeting_report`) НЕ проставлял `meetingId` → задачи не попадали в карточку встречи (вкладка «Задачи» пуста, `Issue.linkedMeetingIds` не заполнялся), а назначенец гостя авто-уходил на постороннего сотрудника через skill-routing.

- **Ф1 — привязка через `meetingId`** (`c5878f21`): `TaskDraftMaterializerService` резолвит голый `meetingId` из `sourceId` формата `report_<id>` и проставляет `IntakeIssue.meetingId` → при промоуте протягивается в `Issue.linkedMeetingIds` (канон связи встреча↔задача, `where:{ linkedMeetingIds:{ has: meetingId } }`). `channel=meeting_report` трактуется как **meeting → always-promote** (`tracker.meetingTasksAlwaysPromote`): без skill-routing и owner-fallback, промоут всегда (см. §intake-auto-triage A2).
- **Ф2 — назначенец гостя, вариант А** (`e2a7c71b`): гость не член Org → исполнитель НЕ резолвится (skill-routing на сотрудника не делается), сырое имя из поля `assignee` комбо-промпта пишется в новую колонку `IntakeIssue.ownerHintRaw` → при accept протягивается в `Issue.ownerHintRaw` (миграция `20260706120000_add_owner_hint_raw`, аддитивная nullable, обе модели). Задача остаётся без исполнителя. Схема — [[../02_architecture/data-model]] §IntakeIssue / §Issue.
- **Ф2b — FE-пометка** (`2aa5b50a`): во вкладке «Задачи встречи» задача с `ownerHintRaw` показывается пометкой «по словам гостя» (исполнитель пуст).
- **Ф3 — снос фантомного taskType** (`ac94a2b3`): пассивный `meeting-extract-actions` (llm-router) окончательно удалён — combo единственный источник.
- **Ф4 — структурные логи** (`4abc5aad`): `assignee_resolve` / `meeting_linkage` / `combo_task_linkage` / `promote_assignee` — диагностика привязки и резолва исполнителя по прод-логам.
