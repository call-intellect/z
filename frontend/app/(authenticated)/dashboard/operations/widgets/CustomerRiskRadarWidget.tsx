    ...
    <GlassCard>
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {swr.data ? (
          <span className="text-xs" style={{ color: CHART.faint }}>
            критичных {swr.data.criticalCount} · под вниманием{" "}
            {swr.data.warningCount}
          </span>
        ) : null}
      </div>
      ...
      <div className="mt-4">
        {swr.isLoading ? (
          ...
          <p className="text-sm" style={{ color: CHART.dim }}>
            Клиентов под риском нет — сигналов оттока за период не
            зафиксировано.
          </p>
        ) : (
          <>
                ...
                >
                  ...
                  >
                    ...
                    {c.responsiblePersonName ? (
                      <span>ответственный: {c.responsiblePersonName}</span>
                    ) : null}
                  </div>
                  {c.hint ? (
                    <p className="mt-1.5 text-xs" style={{ color: CHART.dim }}>
                      {c.hint}
                    </p>
                  ) : null}
                </li>
