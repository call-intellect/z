...
const SEEDS: TaskRouteSeed[] = [
  {
    ...
    playbookSection: '§2.1 block-distill (similar complexity, structured JSON) + β-4 sub-TZ §10',
    chain: [
  ...
  },
  {
    ...
    playbookSection: '§2.1 small select-task (JSON-in/JSON-out arbiter) + β-4 sub-TZ §10',
    chain: [
      { tier: 'primary', providerName: 'deepseek', model: 'deepseek-v4-flash' },
      {
        tier: 'secondary',
...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log(`=== seed-llm-task-routes-insights START (updateExisting=${updateExisting}) ===`);
  // eslint-disable-next-line no-console
