...
/**
 * β-9 — приём приглашения по magic-link. Публичная страница, без auth.
 *
 * Сценарий: сотрудник пришёл по `/invite/<magicToken>` из письма
 * (или ссылки, которую переслал руководитель). Кнопка «Принять» — чтобы
 * подтвердить осознанно (защита от ситуаций «случайно кликнул»).
 *
 * После успеха backend выставит cookie `z_session` в Set-Cookie, и редирект
 * на `/dashboard` пускает уже залогиненного пользователя.
 */
export function AcceptInviteMagicClient({ magicToken }: { magicToken: string }) {
  ...
  return (
    <div className="mx-auto max-w-md py-16">
      ...
      <p className="mb-6 text-sm text-fg-secondary">
        Ваш руководитель приглашает вас в компанию в Коре — память вашей компании,
        которая помнит за всю команду. Нажмите «Принять», чтобы войти в свой кабинет.
      </p>
      <Button
        size="lg"
        className="w-full"
...
/**
 * Бэкенд бросает русские сообщения в `error.message`, но для известных кодов
 * можно дополнительно подсказать пользователю, что делать.
 */
