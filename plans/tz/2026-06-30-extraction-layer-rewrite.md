...
### [x] Ф6 — Скелет → шапка-карта, слой 1 (WP-E)
...
### [x] Ф9 — Усилить граф-детектор конфликтов + крутилки порогов (WP-G шаги 1-2)
...
### [x] Ф10 — me-tasks дедуп-guard + разовая проверка перед сносом (WP-J)
...
### [⏸️ ОТЛОЖЕНО в пост-прод] Ф11 — Снос обходчиков (каждый отдельным коммитом; без A/B) Б5/ВР8
> **Заход A НЕ выполняет снос.** (а) раздельные COMBINED_COVERED-специалисты и (б) regex-крон конфликтов — активный фолбэк рубильника; снос ТОЛЬКО после разовой прод-проверки паритета combo/граф-детектора (Ф10, прод-шаг). (в/г) снос meeting-extract/спайна = заход B (зависит от трекер-машинерии ТЗ задач). Задокументировано: `second-brain/04_не-сделано/README.md` (строки 2026-06-30).
...
### [x] Ф12 — Observability · прод-шаги · second-brain
...
## Итог
**Заход A реализован полностью (2026-06-30, ветка `work/2026-06-29`, коммиты `41736929`..Ф12).** Готово: Ф1 (рубильник→AdminSetting+zBool, крутилки→resolveSync, meeting-skeleton-маршрут; движок block-ingest = deepseek, Opus отменён — Вариант A), Ф4 (few-shot реестр 57 типов), Ф3 (combo воспроизводит 4 побочки), Ф2 (канало-агностичный combo), Ф5 (нахлёст+gleaning+дедуп), Ф6 (скелет→шапка-карта), Ф7б (scope регламентов+владелец+backfill), Ф8 (хроносверка sourceTimestamp), Ф9 (граф-детектор автор↔стороны), Ф10 (тесты дедуп-guard me-tasks), Ф12 (метрики+second-brain+prod-deploy-log+feature-flags). Все фазы: typecheck/lint/build зелёные, юнит/снапшоты зелёные. Миграций БД нет.

**Осталось (вне захода A, задокументировано в `04_не-сделано`):**
- **Ф11(а/б)** — снос раздельных специалистов и regex-крона: **пост-прод**, после разовой проверки паритета combo/граф-детектора (фолбэк рубильника до доказательства).
- **Ф7 + Ф11(в/г)** — combo эмитит `tasks[]` + снос meeting-extract/спайна = **заход B**, зависит от трекер-машинерии ТЗ задач (`task-extraction-pipeline-unification` Ф1-Ф4, не реализованы).
- **Разовая проверка глазами** (Ф10) + **EXPLAIN fact-supersede LATERAL** — прод-шаги после выката.
- **WP-H** (derive-by-signalType) — отложен (реестр).
