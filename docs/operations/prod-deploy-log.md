...
- **⚠️ Pre-flight 2 — добить backfill эмбеддингов вручную** (он же причина 23-минутных деплоев): `docker compose exec backend bun run scripts/backfill-embeddings-gemma-768.ts` до конца (часы; идемпотентен, `WHERE embedding IS NULL`; на 2026-07-09: Entity 0/8913, IdeaBlock 3543/5656). Пока не завершён — каждый деплой платит 600s таймаута в schema-фазе.
