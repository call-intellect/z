...
const SEEDS: TaskRouteSeed[] = [
  {
    ...
    playbookSection: 'Единый чат Ф5b «Что пропустил» — дешёвая сводка непрочитанного, markdown',
    chain: [
...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log(`=== seed-llm-task-routes-chat START (updateExisting=${updateExisting}) ===`);
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log('=== seed-llm-task-routes-chat DONE ===');
}

main()
  .catch((err) => {
    // eslint-disable-next-line no-console
    console.error('seed-llm-task-routes-chat FAILED:', err);
    process.exit(1);
  })
  .finally(async () => {
