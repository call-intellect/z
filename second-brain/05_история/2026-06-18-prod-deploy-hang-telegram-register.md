...
- **Ускорение выката (`b5303ca2` → `aa01ee36`):** ~⅔ `STEPS` (patch/backfill/migrate + seed-llm-routes) — одноразовые/идемпотентные, но гонялись каждый деплой. Добавлен журнал `public._deploy_applied_step` (как `_prisma_migrations`). Скип **по хэшу содержимого скрипта** (`sha256(файл + args)`), а не по имени — иначе отредактированный route-сид (добавили маршруты) заскипался бы и новые маршруты не засеялись. Изменился файл → прогон заново; не изменился → скип. Конфиг-реконсиляция (LLM-маршруты, Ship-On флаги, seed-llm-core/base/default) гоняется всегда (`everyDeploy:true` либо фаза вне once-списка). Форс — `--rerun-all` / `DEPLOY_RERUN_ALL=1`. Первый деплой после изменения прогонит всё один раз и запишет хэши; дальше steady-state ~45 шагов вместо 166.
